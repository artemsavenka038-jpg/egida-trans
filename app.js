...
function BindTripModal({plate,driver,trucks,clients,onPick,onClose}){
  ...
  const save=function(){
    ...
    onPick({truckId:sel?sel.id:'',truckPlate:sel?(sel.plate||''):'',driver:sel?(sel.driver||''):'',
      clientName:cname,
      temp:temp.trim(),at:nowLocalISO(),since:nowLocalISO(),
      expires:new Date(Date.now()+hrs*3600*1000).toISOString(),hours:hrs});
  };
...
function TempTab({trucks,clients,binds,onBind,readOnly}){
  ...
  const list=trailers.map(function(x){
    ...
    var s=bind?tempStatus(x.val,range,x.age):{st:'idle',label:'не привязан',color:'var(--text3)'};
    var alt=null;
    ...
    return Object.assign({},x,{bind:bind,range:range,s:s,alt:alt});
  }).sort(function(a,b){
