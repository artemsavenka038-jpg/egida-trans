...
function PlanGrid({plans,clients,orders,trucks,geoIndex,onSavePlan,onDelPlan,onLaunch}){
  ...
  const barFor=function(row){
    ...
    // полоса на 1 день вперёд; если есть обратка — ещё +1 день
    var endIdx=startIdx+1;
    if(pl.back) endIdx=startIdx+2;
    if(endIdx>6) endIdx=6; if(endIdx<startIdx) endIdx=startIdx;
    return {startIdx:startIdx,endIdx:endIdx,plan:pl,col:clientColor(pl.clientName),hasBack:!!pl.back};
  };
