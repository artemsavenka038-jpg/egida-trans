    ...
    open?ce('div',{style:{padding:'0 12px 12px'}},
      ...
      ce('div',{style:{border:'1px solid var(--border)',borderRadius:11,overflow:'hidden'}},
        forDay.map(function(p,i){ var overdue=new Date(p.when).getTime()<Date.now();
          var stCol=SC[p.truck.status]||'var(--text3)'; var stLabel=T('s_'+p.truck.status)||p.truck.status;
        ...
        ce('div',{style:{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(340px,1fr))',gap:12}},
          ...
          dayPlans.map(function(p){
            ...
            ce('div',{style:{marginTop:9,display:'flex',gap:6}},
              isDue?ce('button',{className:'fl-press',onClick:function(){ launchPlan(p); },style:{flex:1,padding:'9px',borderRadius:9,border:'none',cursor:'pointer',fontFamily:'inherit',fontSize:12.5,fontWeight:800,
                color:'#fff',background:'linear-gradient(135deg,var(--accent),var(--accent2))'}},'▶ Запустить рейс'):null,
