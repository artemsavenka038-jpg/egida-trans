...
function TempCard({r,onBind,onUnbind,onHistory,readOnly}){
  ...
  return ce('div',{style:{background:'var(--bg2)',borderRadius:14,padding:'12px 13px',
    ...
    bound?ce('div',{style:{display:'flex',flexDirection:'column',gap:5,paddingTop:6,borderTop:'1px solid var(--border)'}},
      ce('div',{style:{fontSize:11,color:'var(--text3)',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}},
        r.bind.truckPlate?((r.bind.truckPlate||'')+(r.bind.clientName?(' · '+r.bind.clientName):'')):'задано вручную'),
...
function TempTab({trucks,clients,binds,onBind,readOnly}){
  ...
  const shown=filter==='bad'?bad:(filter==='bound'?list.filter(function(x){return x.bind;}):list);
