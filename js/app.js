...
function PlanGrid({plans,clients,orders,trucks,geoIndex,onSavePlan,onDelPlan,onLaunch}){
  ...
  const barFor=function(row){
    // ищем план с самой ранней загрузкой в этой неделе
    ...
    // конец — по freeAt, если есть; иначе +2 дня (грубо)
    var endIdx=startIdx+2;
    if(pl.freeAt){ var fk=String(pl.freeAt).slice(0,10); var fi=weekKeys.indexOf(fk); if(fi>=0) endIdx=fi; }
  ...
  return ce('div',{style:{padding:'14px',maxWidth:1500,margin:'0 auto'}},
    ...
    ce('div',{style:{overflowX:'auto',border:'1px solid var(--border)',borderRadius:12}},
      ce('div',{style:{minWidth:900}},
        ...
        driverRows.map(function(row,ri){
          var bar=barFor(row);
          var mt=(trucks||[]).find(function(x){ return (x.driver||'').trim().toLowerCase()===(row.driver||'').trim().toLowerCase(); });
          var stLabel=mt&&mt.status&&mt.status!=='FREE'?(T('st_'+mt.status)!=='st_'+mt.status?T('st_'+mt.status):''):'';
          return ce('div',{key:ri,style:{display:'grid',gridTemplateColumns:'160px repeat(7,1fr)',borderBottom:'1px solid var(--border)',position:'relative',minHeight:44}},
            ...
            // статус водителя над полосой
            (bar&&stLabel)?ce('div',{style:{position:'absolute',top:2,pointerEvents:'none',fontSize:8.5,fontWeight:800,color:bar.col,
              left:'calc(160px + '+(bar.startIdx)+' * ((100% - 160px)/7) + 6px)'}},stLabel):null); }))),
