...
function unloadPoints(trucks){
  // собираем все предстоящие выгрузки из рейсов
  var pts=[];
  (trucks||[]).forEach(function(t){
    ...
    if(trips.length){
      trips.forEach(function(tr){
        pts.push({truck:t,plate:t.plate,driver:t.driver,when:tr.arriveUnload,
          addr:tr.to||t.unloadAddr||'',client:tr.clientName||t.clientName||'',freeAt:t.freeAt});
      });
    } else if(t.freeAt){
      // нет детального маршрута — берём освобождение как ориентир выгрузки
