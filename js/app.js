...
function RigSwapModal({trucks,mecTrailers,mecTractors,onApply,onClose}){
  ...
  const own=(trucks||[]).filter(function(t){ return !t.isHired&&t.plate; });
...
function DemandsTab({demands,trucks,clients,onSave,onDel,carrierOffers,onDelOffer,onArchiveOffer}){
  ...
  const own=(trucks||[]).filter(function(t){ return !t.isHired; });
...
function BindTripModal({plate,driver,trucks,clients,onPick,onClose}){
  const [q,setQ]=useState('');
  ...
  return ce(Modal,{title:'🔗 Привязать '+plate,onClose,wide:true},
    ...
    ce('div',{style:{background:'var(--bg3)',border:'1px solid var(--border)',borderRadius:11,padding:'11px 13px',marginBottom:10}},
      ...
      ce('select',{value:manualClient,onChange:function(e){setManualClient(e.target.value);},style:mecInp},
...
function TempTab({trucks,clients,binds,onBind,readOnly}){
  ...
  return ce('div',{style:{padding:'16px',maxWidth:1700,margin:'0 auto'}},
    ...
    bindFor?ce(BindTripModal,{plate:bindFor.plate,driver:bindFor.driver,trucks:trucks,clients:clients,
      onPick:function(b){ onBind(bindFor.plate,b); setBindFor(null); },onClose:function(){setBindFor(null);}}):null,
...
function AppMain() {
  ...
  return ce('div',{style:{minHeight:'100vh',color:'var(--text)'}},
    ...
    tab==='demands'&&ce(DemandsTab,{demands:demands,trucks:trucks,clients:clients,onSave:saveDemand,onDel:delDemand,
