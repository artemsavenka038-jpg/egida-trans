...
function PlanBoard({plans,clients,orders,trucks,onSavePlan,onDelPlan,onLaunch}){
  ...
  return ce('div',{style:{padding:'14px',maxWidth:1100,margin:'0 auto'}},
    ...
    days.map(function(d){
      var k=dk(d), rows=byDay(k), isToday=k===todayK;
        ...
        ce('div',{style:{display:'grid',gap:5}},
          rows.map(function(p){
            ...
            return ce('div',{key:p.id,style:{display:'flex',alignItems:'center',gap:10,padding:'8px 11px',borderRadius:9,flexWrap:'wrap',
              ...
              launched?ce('span',{style:{fontSize:11,color:'var(--green)',fontWeight:800}},'✓ в рейсе'):
...
function AppMain() {
  ...
  const updTruck = (id,patch)=>{ setTrucks(prev=>prev.map(t=>{ if(t.id!==id) return t; var m=Object.assign({},t,patch);
    if(patch&&patch.status==='FORWARD' && t.status!=='FORWARD'){ m.reachedUnload=false; }  // новый рейс — сбрасываем «был у выгрузки»
  ...
  const launchPlan = (p)=>{
    ...
    var patch={status:'FORWARD',
      ...
      note:p.note||t.note||''};
    updTruck(t.id,patch);
    // только ПОСЛЕ успешного создания рейса — удаляем план (машина теперь настоящая карточка)
    dbPut('/plans/'+p.id,null); setPlans(function(x){ return x.filter(function(y){ return y.id!==p.id; }); });
