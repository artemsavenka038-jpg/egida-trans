...
function DriverMarkModal({driver,mark,trucks,onSave,onClear,onClose}){
  const [type,setType]=useState((mark&&mark.type)||'off');
  ...
  const chip=function(active,col){ return {flex:1,textAlign:'center',padding:'9px',borderRadius:9,fontSize:13,fontWeight:700,cursor:'pointer',
    ...
    ce('div',{style:{display:'grid',gap:12}},
      ...
      ce('div',{style:{display:'flex',gap:9,marginTop:4}},
        ...
        ce('button',{onClick:function(){ onSave({type:type,from:from,to:to,plate:plate.trim(),note:note.trim()}); },className:'fl-press',
          style:{flex:1,padding:'12px',border:'none',borderRadius:11,color:'#fff',fontSize:14,fontWeight:800,cursor:'pointer',
          background:'linear-gradient(135deg,var(--accent),var(--accent2))'}},'Сохранить'))));
}
// ═══════════ x2 ПЛАНИРОВАНИЕ — недельная сетка водитель × день с полосой рейса ═══════════
function PlanGrid({plans,clients,orders,trucks,geoIndex,onSavePlan,onDelPlan,onLaunch,onAddOrder,onSetBar,role,logists,isBoss,gridBacks,onSetGridBack,onDelOrder,gridTuda,onSetGridTuda,driverMarks,onSetDriverMark}){
  ...
  return ce('div',{style:{padding:'14px',maxWidth:1500,margin:'0 auto'}},
    markDrv?ce(DriverMarkModal,{driver:markDrv,mark:(driverMarks||{})[markDrv],trucks:trucks,
      ...
      onClose:function(){setMarkDrv(null);}}):null,
    ...
    addCell?ce(PlanRowEditor,{row:{truckDriver:addCell.driver,loadDate:addCell.dateKey},clients:clients,orders:orders,trucks:trucks,dateKey:addCell.dateKey,
...
function PlanRowEditor({row,clients,orders,trucks,dateKey,onSave,onCancel,onDelete}){
  row=row||{};
