...
function TractorCard({t,onUpd,onDel}){
  ...
  return ce('div',{className:'fl-card',style:{background:'var(--bg2)',border:'1px solid var(--border)',borderRadius:14,padding:14,boxShadow:'var(--shadow)'}},
    ...
    ce('div',{style:{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:8}},
      ...
      fld('Пропуск РФ до',ce('input',dat('pass'))), null),
...
function MechTractors({tractors,onAdd,onUpd,onDel}){
...
function MechanicTab(props){
  ...
  return ce('div',{style:{padding:'16px',maxWidth:1700,margin:'0 auto'}},
    ...
    view==='tractors'&&ce(MechTractors,{tractors:tractors,onAdd:props.onAddTractor,onUpd:props.onUpdTractor,onDel:props.onDelTractor}),
...
function App() {
  ...
  useEffect(()=>{ dbGet('/settings/logists').then(function(x){ var a=normLogists(x); if(a&&a.length) setLogists(a); }).catch(function(){}); },[]);
  useEffect(()=>{ var f=function(){
    dbGet('/mechanic/tractors').then(function(o){ setMecTractors(mapVals(o)); }).catch(function(){});
    dbGet('/mechanic/trailers').then(function(o){ setMecTrailers(mapVals(o)); }).catch(function(){});
    dbGet('/mechanic/acts').then(function(o){ setMecActs(mapVals(o)); }).catch(function(){});
  ...
  const updMecTractor = (id,patch)=>{ setMecTractors(prev=>prev.map(x=>{ if(x.id!==id) return x; var m=Object.assign({},x,patch); dbPut('/mechanic/tractors/'+id,m); return m; })); tShow(T('saved')); };
  const addMecTractor = (plate)=>{ var id='mt_'+Date.now(); var obj={id,plate:(plate||'').trim(),odometer:'',toMileage:'',tireSteer:'',tireDrive:'',techInsp:'',tachograph:'',pass:'',note:''}; setMecTractors(prev=>prev.concat([obj])); dbPut('/mechanic/tractors/'+id,obj); tShow('Тягач добавлен'); };
  const delMecTractor = (id)=>{ setMecTractors(prev=>prev.filter(x=>x.id!==id)); dbPut('/mechanic/tractors/'+id,null); tShow('Тягач удалён'); };
  const updMecTrailer = (id,patch)=>{ setMecTrailers(prev=>prev.map(x=>{ if(x.id!==id) return x; var m=Object.assign({},x,patch); dbPut('/mechanic/trailers/'+id,m); return m; })); tShow(T('saved')); };
  const addMecTrailer = (plate)=>{ var id='mp_'+Date.now(); var obj={id,plate:(plate||'').trim(),econ:'',pairedTractor:'',toMileage:'',tireMileage:'',frc:'',unitService:'',note:''}; setMecTrailers(prev=>prev.concat([obj])); dbPut('/mechanic/trailers/'+id,obj); tShow('Прицеп добавлен'); };
  const delMecTrailer = (id)=>{ setMecTrailers(prev=>prev.filter(x=>x.id!==id)); dbPut('/mechanic/trailers/'+id,null); tShow('Прицеп удалён'); };
  const saveMecAct = (act)=>{ var id=act.id||('ma_'+Date.now()); var obj=Object.assign({},act,{id}); setMecActs(prev=>{ var ex=prev.some(a=>a.id===id); return ex?prev.map(a=>a.id===id?obj:a):prev.concat([obj]); }); dbPut('/mechanic/acts/'+id,obj); tShow('Акт сохранён'); };
  const delMecAct = (id)=>{ setMecActs(prev=>prev.filter(a=>a.id!==id)); dbPut('/mechanic/acts/'+id,null); tShow('Акт удалён'); };
  ...
  return ce('div',{style:{minHeight:'100vh',color:'var(--text)'}},
    ...
    tab==='mechanic'&&ce(MechanicTab,{tractors:mecTractors,trailers:mecTrailers,acts:mecActs,
      onAddTractor:addMecTractor,onUpdTractor:updMecTractor,onDelTractor:delMecTractor,
