...
const UNIT_SERVICE_DAYS=182, DATE_WARN_DAYS=30;
...
function normPlate(s){ if(!s) return ''; var x=String(s).toUpperCase(); x=x.replace(/[-–—]\s*\d\s*$/,''); return x.replace(/[^А-ЯЁA-Z0-9]/g,''); }
...
function MileagePasteModal({tractors,onApply,onClose}){
  ...
  return ce(Modal,{title:'📋 Массовая вставка пробегов',onClose,wide:true},
    ce('div',{style:{fontSize:12,color:'var(--text3)',marginBottom:9,lineHeight:1.6}},
      'Скопируйте пробеги из программы геолокации (или из Excel) и вставьте сюда — по одной машине в строке. Понимает форматы: «АТ 4774  1147593», «ДАФ р.н. АТ 4774 - 7   1.147.593», а также обратный порядок «1147593  АТ 4774». Номера сверяются с карточками тягачей.'),
    ...
    txt.trim()?ce('div',{style:{marginTop:12}},
      ce('div',{style:{display:'flex',gap:14,fontSize:12.5,fontWeight:800,marginBottom:8}},
        ce('span',{style:{color:'var(--green)'}},'Совпало: '+m.length),
        ce('span',{style:{color:u.length?'var(--amber)':'var(--text3)'}},'Не распознано: '+u.length)),
      m.length?ce('div',{style:{maxHeight:200,overflowY:'auto',border:'1px solid var(--border)',borderRadius:10,marginBottom:8}},
      ...
      u.length?ce('div',{style:{maxHeight:120,overflowY:'auto',fontSize:11.5,color:'var(--text3)',lineHeight:1.6}},
        u.map(function(x,i){ return ce('div',{key:i},'• '+x.raw+(x.plate?'  (номер '+x.plate+' не найден среди тягачей)':'  (номер не распознан)')); })):null):null,
...
let ROLES = LOGISTS.concat(['Механик','Руководитель']);
...
function applyLogists(list){ var a=normLogists(list); if(!a||!a.length) return; LOGISTS=a.map(function(l){return l.name;}); LOGIST_COLOR={}; a.forEach(function(l){ LOGIST_COLOR[l.name]=l.color; }); ROLES=LOGISTS.concat(['Механик','Руководитель']); }
...
function RolePicker({onPick,logists,trucks,onSaveLogists}){
  ...
  return ce('div',{style:{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',padding:20,color:'var(--text)'}},
    ...
    ce('div',{style:{width:'100%',maxWidth:420}},
      ...
      ROLES.map(function(r){ var isBoss=r==='Руководитель'; var isMech=r==='Механик'; var col=isBoss?'var(--green)':isMech?'var(--cyan)':(LOGIST_COLOR[r]||'var(--accent)');
        return ce('button',{key:r,className:'fl-press',onClick:()=>onPick(r),style:{width:'100%',marginBottom:11,padding:'16px 18px',
          ...
          ce('span',{style:{fontSize:23}},isBoss?'👔':isMech?'🔧':'👤'), r,
...
function MechDash({tractors,trailers,acts,onJump}){
  ...
  trailers.forEach(function(tr){
    ...
    items.push({plate:tr.plate||'—',cat:'Шины прицепа',s:kmStatus(odo,tr.tireMileage,TIRE_TRAILER_KM,TIRE_WARN_KM),jump:'trailers'});
...
function MechTractors({tractors,onAdd,onUpd,onDel,onBulk}){
  const [paste,setPaste]=useState(false);
  const add=function(){ var p=window.prompt('Гос.номер тягача:'); if(p&&p.trim()) onAdd(p.trim()); };
  return ce('div',null,
    paste?ce(MileagePasteModal,{tractors:tractors,onApply:onBulk,onClose:function(){setPaste(false);}}):null,
...
function TrailerCard({tr,tractors,onUpd,onDel}){
  ...
  const ts=kmStatus(odo,tr.tireMileage,TIRE_TRAILER_KM,TIRE_WARN_KM);
  ...
  return ce('div',{className:'fl-card',style:{background:'var(--bg2)',border:'1px solid var(--border)',borderRadius:14,padding:14,boxShadow:'var(--shadow)'}},
    ce('div',{style:{display:'flex',alignItems:'center',gap:8,marginBottom:10}},
      ...
      ce('button',{onClick:function(){ if(window.confirm('Удалить прицеп '+(tr.plate||'')+' из списка механика?')) onDel(tr.id); },className:'fl-press',
        style:{width:32,height:32,borderRadius:8,border:'1px solid color-mix(in srgb,var(--red) 45%,transparent)',background:'transparent',color:'var(--red)',cursor:'pointer',fontSize:15,flexShrink:0}},'🗑')),
    ce('div',{style:{display:'flex',flexWrap:'wrap',gap:8,marginBottom:12}},
      ce(StatusPill,{s:to,label:'ТО прицепа'}),ce(StatusPill,{s:ts,label:'Шины'}),ce(StatusPill,{s:frc,label:'FRC'}),ce(StatusPill,{s:unit,label:'ТО установки'}),
      ce(StatusPill,{s:ins,label:'Страховка'}),ce(StatusPill,{s:kas,label:'КАСКО'}),ce(StatusPill,{s:akb,label:'АКБ'})),
    ...
    ce('div',{style:{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:8}},
      ...
      fld('Пробег тягача на посл. замене шин',ce('input',num('tireMileage'))),
      fld('FRC действует до',ce('input',dat('frc'))),
      fld('Посл. ТО установки (термокинг)',ce('input',dat('unitService'))),
      fld('Страховка (ОСАГО) до',ce('input',dat('insurance'))),
      fld('КАСКО до',ce('input',dat('kasko'))),
...
function MechanicTab(props){
  ...
  const tabs=[['dash','📊 Сводка'],['tractors','🚚 Тягачи ('+tractors.length+')'],['trailers','🚛 Прицепы ('+trailers.length+')'],['acts','📋 Осмотры ('+acts.length+')']];
  return ce('div',{style:{padding:'16px',maxWidth:1700,margin:'0 auto'}},
    ...
    view==='tractors'&&ce(MechTractors,{tractors:tractors,onAdd:props.onAddTractor,onUpd:props.onUpdTractor,onDel:props.onDelTractor,onBulk:props.onBulkOdometer}),
...
function App() {
  ...
  window._lang = lang;
  const [tab,setTab] = useState(()=>{ try{ return localStorage.getItem('egida_role')==='Механик' ? 'mechanic' : 'park'; }catch(e){ return 'park'; } });
  ...
  useEffect(()=>{ var f=function(){
    dbFetch('/mechanic/tractors').then(function(r){ if(r.ok) setMecTractors(mapVals(r.val)); });
    dbFetch('/mechanic/trailers').then(function(r){ if(r.ok) setMecTrailers(mapVals(r.val)); });
    dbFetch('/mechanic/acts').then(function(r){ if(r.ok) setMecActs(mapVals(r.val)); });
  ...
  const updMecTractor = (id,patch)=>{ setMecTractors(prev=>prev.map(x=>{ if(x.id!==id) return x; var m=Object.assign({},x,patch); saveOr('/mechanic/tractors/'+id,m,T('saved')); return m; })); };
  const addMecTractor = (plate)=>{ var id='mt_'+Date.now(); var obj={id,plate:(plate||'').trim(),odometer:'',toMileage:'',tireSteer:'',tireDrive:'',techInsp:'',tachograph:'',pass:'',insurance:'',kasko:'',akbDate:'',note:''}; setMecTractors(prev=>prev.concat([obj])); saveOr('/mechanic/tractors/'+id,obj,'Тягач добавлен'); };
  const delMecTractor = (id)=>{ setMecTractors(prev=>prev.filter(x=>x.id!==id)); saveOr('/mechanic/tractors/'+id,null,'Тягач удалён'); };
  ...
  const updMecTrailer = (id,patch)=>{ setMecTrailers(prev=>prev.map(x=>{ if(x.id!==id) return x; var m=Object.assign({},x,patch); saveOr('/mechanic/trailers/'+id,m,T('saved')); return m; })); };
  const addMecTrailer = (plate)=>{ var id='mp_'+Date.now(); var obj={id,plate:(plate||'').trim(),econ:'',pairedTractor:'',toMileage:'',tireMileage:'',frc:'',unitService:'',insurance:'',kasko:'',akbDate:'',note:''}; setMecTrailers(prev=>prev.concat([obj])); saveOr('/mechanic/trailers/'+id,obj,'Прицеп добавлен'); };
  const delMecTrailer = (id)=>{ setMecTrailers(prev=>prev.filter(x=>x.id!==id)); saveOr('/mechanic/trailers/'+id,null,'Прицеп удалён'); };
  const saveMecAct = (act)=>{ var id=act.id||('ma_'+Date.now()); var obj=Object.assign({},act,{id}); setMecActs(prev=>{ var ex=prev.some(a=>a.id===id); return ex?prev.map(a=>a.id===id?obj:a):prev.concat([obj]); }); saveOr('/mechanic/acts/'+id,obj,'Акт сохранён'); };
  const delMecAct = (id)=>{ setMecActs(prev=>prev.filter(a=>a.id!==id)); saveOr('/mechanic/acts/'+id,null,'Акт удалён'); };
  ...
  const TABS=[
    ...
    ['log',T('tab_log')],
  ].concat((isMech||isBoss)?[['mechanic','🔧 Механик']]:[]);
  ...
  return ce('div',{style:{minHeight:'100vh',color:'var(--text)'}},
    (dbHealth&&dbHealth!=='ok')?ce('div',{style:{background:'color-mix(in srgb,var(--red) 16%,transparent)',
      borderBottom:'1px solid color-mix(in srgb,var(--red) 45%,transparent)',color:'var(--red)',
      padding:'10px 16px',fontSize:12.5,fontWeight:700,textAlign:'center',lineHeight:1.5}},
      '⚠ База данных не принимает запись ('+dbHealth+'). Всё, что вы меняете, НЕ сохраняется и пропадёт после перезагрузки. Скорее всего истёк срок правил доступа Firebase — их нужно продлить в консоли Firebase (Realtime Database → Rules).'):null,
    ...
    tab==='mechanic'&&ce(MechanicTab,{tractors:mecTractors,trailers:mecTrailers,acts:mecActs,
      onAddTractor:addMecTractor,onUpdTractor:updMecTractor,onDelTractor:delMecTractor,onBulkOdometer:bulkSetOdometer,
