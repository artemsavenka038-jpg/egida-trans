...
function PlanRowEditor({row,clients,orders,trucks,dateKey,onSave,onCancel}){
  ...
  return ce(Modal,{title:(row.id?'✎ Строка плана':'➕ Новая строка')+' · '+fmtD(row.loadDate||dateKey),onClose:onCancel},
    ...
    ce('div',{style:{display:'flex',gap:9,marginTop:16}},
      ce(Btn,{onClick:onCancel,color:'var(--text3)',wide:true},'Отмена'),
      ce('button',{onClick:function(){ if(!bad) onSave(Object.assign({},row,{loadDate:row.loadDate||dateKey,clientName:clientName.trim(),truckDriver:driver.trim(),truckPlate:plate.trim(),back:back.trim(),note:note.trim(),confirmed:confirmed})); },
        disabled:bad,className:'fl-press',style:{flex:1,padding:'12px',border:'none',borderRadius:11,color:'#fff',fontSize:14,fontWeight:800,cursor:bad?'default':'pointer',
        background:bad?'var(--bg3)':'linear-gradient(135deg,var(--accent),var(--accent2))',opacity:bad?0.5:1}},'Сохранить')));
}
function PlanBoard({plans,clients,orders,trucks,onSavePlan,onDelPlan,onLaunch,dayNotes,onSaveDayNote}){
  const base=new Date(); base.setHours(0,0,0,0);
  const [edit,setEdit]=useState(null);
  ...
  const days=[]; for(var i=0;i<10;i++) days.push(new Date(base.getTime()+i*864e5));
...
function AppMain() {
  ...
  // Геолокация машин из CAP (/odometer отдаёт координаты + скорость)
  useEffect(function(){
    var load=function(){
      capFetch('odometer').then(function(r){
        if(!r.ok) return;
