...
const addH  = (dt,h) => { const d=new Date(new Date(dt).getTime()+h*3600000); d.setMinutes(d.getMinutes()-d.getTimezoneOffset()); return d.toISOString().slice(0,16); };
const fmtDT = s => { if(!s) return '—'; const d=new Date(s); const loc=window._lang==='en'?'en-GB':'ru'; return d.toLocaleDateString(loc,{day:'2-digit',month:'2-digit'})+' '+d.toLocaleTimeString(loc,{hour:'2-digit',minute:'2-digit'}); };
...
const Modal = ({title,onClose,children,wide}) => ce('div',{
  ...
  padding:'22px 20px',width:'100%',maxWidth:wide?560:420,
...
function ForwardModal({truck,clients,onSave,onClose}) {
  ...
  const [dir,setDir] = useState(truck.status==='BACK'?'back':'forward');
  ...
  const save = ()=>{
    ...
    if(joinTwo){
      ...
      onSave({status:dir==='back'?'BACK':'FORWARD',location:fromCity,clientId:cid,clientName:(cl&&cl.name)||'',
  ...
  return ce(Modal,{title:T('m_route')+' — '+truck.plate,onClose,wide:true},
    ...
    ce('div',{style:Object.assign({},box)},
      ...
      ce('div',{style:{display:'flex',gap:6,flexWrap:'wrap'}},
        clients.map(c=>ce('button',{key:c.id,className:'fl-press',onClick:()=>setCid(c.id),
    ...
    ce('div',{style:box},
      ce('div',{style:lbl},T('from_city')),
      ce('div',{style:{display:'flex',gap:6,flexWrap:'wrap'}},
        CITIES.map(city=>ce('button',{key:city,className:'fl-press',onClick:()=>setFromCity(city),
          style:chip(fromCity===city)},city)))),
    ce('div',{style:box},
...
function BackModal({truck,orders,clients,onSave,onClose}) {
  ...
  const [dep,setDep] = useState(nowLocalISO());
  ...
  const save = ()=>{
    ...
    onSave({status:'BACK', backCargo:{cargo,from,to,departAt:dep,freeAt,
      orderId:(selectedOrder&&selectedOrder.id)||null, rate:(selectedOrder&&selectedOrder.rate)||null}, freeAt});
  };
      ...
      availOrders.length===0
        ...
        : availOrders.map(o=>{
            ...
            return ce('div',{key:o.id,className:'fl-press',onClick:()=>setSelectedOid(String(o.id)),style:{
              padding:'9px 11px',borderRadius:9,marginBottom:6,cursor:'pointer',
              ...
              o.rate&&ce('span',{style:{fontSize:11.5,color:'var(--green)',fontWeight:700}},o.rate.toLocaleString()+' '+T('cur_rub')));
          })),
...
function MsgModal({truck,clients,orders,onClose}) {
  ...
  if(fwdTrips.length>1){
    const lines=['🚛 '+truck.plate,'👤 '+truck.driver,'📞 '+truck.phone,''];
    ...
    fwdMsg=['🚛 '+truck.plate,'👤 '+truck.driver,'📞 '+truck.phone,'',
      truck.clientName?'🏢 '+T('lbl_client')+': '+truck.clientName:null,
      truck.loadAddr?T('lbl_loading')+': '+truck.loadAddr:null,
      truck.unloadAddr?T('lbl_unloading')+': '+truck.unloadAddr:null,
      truck.loadAt?'🕐 '+fmtDT(truck.loadAt):null,
      (cl&&cl.temp)?'🌡 '+cl.temp+'°C':null,
      notes?'📝 '+T('note_for_driver')+':\n'+notes:null
    ].filter(Boolean).join('\n');
  }
  const bc = truck.backCargo;
  const backMsg = bc?['🚛 '+truck.plate,'👤 '+truck.driver,'📞 '+truck.phone,'',
    '↩ '+T('lbl_cargo_back').toUpperCase(),
    bc.cargo?'📦 '+bc.cargo:null,
    bc.from?'📍 '+bc.from:null,
    bc.to?'🎯 '+bc.to:null,
    bc.departAt?'🕐 '+T('depart_word')+': '+fmtDT(bc.departAt):null,
    notes?'📝 '+notes:null
  ].filter(Boolean).join('\n'):null;

  const driverLink = window.location.href.split('?')[0]+'?pin='+truck.pin;
  ...
  return ce(Modal,{title:'👤 '+truck.driver,onClose,wide:true},
    ...
    notes&&ce('div',{style:{background:'color-mix(in srgb,var(--accent) 8%,transparent)',
      ...
      ce('div',{style:{color:'var(--text2)',whiteSpace:'pre-wrap',fontSize:13,lineHeight:1.55}},notes)),
...
function DriverPage({truck,clients,orders,onUpdate,onClose}) {
  ...
  const apply = (s)=>{
    setStatus(s);
    let patch;
    if(s==='FREE') patch={status:'FREE',freeAt:nowLocalISO(),clientId:'',clientName:'',loadAddr:'',unloadAddr:'',loadAt:'',backCargo:null,trips:[]};
    else patch={status:s,freeAt:truck.freeAt};
    onUpdate(patch);
  };
  const groups=[
...
function TruckCard({truck,clients,orders,onUpdate,onSwapRequest,onOpenDriver}) {
  ...
  const bc = truck.backCargo;
  const bord = truck.status==='PROBLEM'?'var(--red)':truck.onService?'var(--orange)':truck.needService?'var(--amber)':'var(--border)';
  ...
  return ce('div',{className:'fl-card fl-row-hover',style:{
    ...
    ce('div',{style:{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'4px 10px',fontSize:12,marginBottom:8}},
      ...
      bc&&truck.status==='BACK'&&ce('div',{style:{color:'var(--text3)',gridColumn:'1/-1'}},
    ...
    bc&&truck.status==='BACK'&&ce('div',{style:{background:'color-mix(in srgb,var(--cyan) 8%,transparent)',
    ...
    ce('div',{style:{display:'flex',gap:6,flexWrap:'wrap'}},
      ...
      ce(Btn,{onClick:()=>onOpenDriver(truck.id),color:'var(--text3)',sm:true},'📱')),
    ...
    modal==='msg'&&ce(MsgModal,{truck,clients,orders,onClose:()=>setModal(null)})
  );
}

// ── Moving card (On-route tab) ────────────────────────────────
function MovingCard({tr,clients,orders,onUpdate,onModal,modal,setModal}) {
  ...
  return ce('div',{className:'fl-card fl-row-hover',style:{background:'var(--bg2)',
    ...
    modal&&modal.type==='msg'&&modal.tid===tr.id&&ce(MsgModal,{truck:tr,clients,orders,onClose:()=>setModal(null)})
  );
}
...
function AssignModal({title,fieldLabel,free,onAssign,onClose}) {
  const [sel,setSel] = useState(free[0]?String(free[0].id):'');
  return ce(Modal,{title,onClose},
    ce(Field,{label:fieldLabel},
      free.length===0
        ? ce('div',{style:{fontSize:13,color:'var(--text3)',padding:8}},T('ai_none'))
        : ce(Select,{value:sel,onChange:setSel,
            options:free.map(t=>({value:String(t.id),label:t.plate+' — '+t.driver+(t.location?' ('+t.location+')':'')}))})),
    ce('div',{style:{display:'flex',gap:9,marginTop:8}},
      ce(Btn,{onClick:onClose,color:'var(--text3)',wide:true},T('btn_cancel')),
...
function App() {
  ...
  const assignTruckToOrder = (truckId,order)=>{
    ...
    setTrucks(prev=>prev.map(t=>{ if(t.id!==truckId) return t;
      var m=Object.assign({},t,{
        status:'LOADED',clientId:order.cid,clientName:order.client,loadAddr:order.from,unloadAddr:order.to,
  ...
  const assignTruckToClient = (truckId,cl)=>{
    ...
    setTrucks(prev=>prev.map(t=>{ if(t.id!==truckId) return t;
      var m=Object.assign({},t,{
        status:'LOADED',clientId:cl.id,clientName:cl.name,loadAddr:cl.loadAddr,unloadAddr:to,
        loadAt:la,freeAt:fa,
  ...
  const stats = useMemo(()=>({
    ...
    problems:trucks.filter(t=>t.status==='PROBLEM').length,
  }),[trucks]);
  ...
  const filtered = useMemo(()=>{
    ...
    return trucks.filter(t=>{
      ...
      if(stFilter==='PROBLEM'&&t.status!=='PROBLEM') return false;
  ...
  },[trucks,stFilter,search]);

  const movingTrucks = useMemo(()=>trucks.filter(t=>MOVING.includes(t.status)),[trucks]);
  ...
  return ce('div',{style:{minHeight:'100vh',color:'var(--text)'}},
    ...
    tab==='park'&&ce('div',{style:{padding:'16px',maxWidth:1700,margin:'0 auto'}},
      ...
      ce('div',{style:{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(330px,1fr))',gap:12}},
        filtered.map(t=>ce(TruckCard,{key:t.id,truck:t,clients,orders,onUpdate:updTruck,
    ...
    // ── MOVING TAB ──
    tab==='moving'&&ce('div',{style:{padding:'16px',maxWidth:1700,margin:'0 auto'}},
      ce('div',{style:{marginBottom:12}},
        ce('h2',{style:{margin:0,fontWeight:800,color:'var(--text)',fontSize:20}},T('moving_title')),
        ce('div',{style:{fontSize:12.5,color:'var(--text3)',marginTop:3}},T('moving_sub'))),
      ce('div',{style:{fontSize:12,color:'var(--text3)',marginBottom:10}},T('moving_found')+': '+movingTrucks.length),
      ce('div',{style:{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(340px,1fr))',gap:12}},
        movingTrucks.map(tr=>ce(MovingCard,{key:tr.id,tr,clients,orders,onUpdate:updTruck,modal:movingModal,setModal:setMovingModal}))),
      movingTrucks.length===0&&ce('div',{style:{textAlign:'center',padding:50,color:'var(--text3)'}},T('moving_empty'))),
    ...
    modal&&modal.type==='assignorder'&&(function(){
      ...
      return ce(AssignModal,{title:T('m_assign_truck'),fieldLabel:o.cargo+' · '+o.client,free,
