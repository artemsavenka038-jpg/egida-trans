...
function DemandEditor({d,clients,dayKey,onSave,onCancel}){
  d=d||{};
  ...
  return ce(Modal,{title:(d.id?'✎ Заявка':'➕ Новая заявка')+' · '+fmtD(d.date||dayKey),onClose:onCancel},
    ce('div',{style:{display:'grid',gap:10}},
      fld('Клиент',ce('input',{value:clientName,onChange:e=>setClientName(e.target.value),list:'fl-dem-clients',placeholder:'Санта, Серволюкс…',style:mecInp})),
      ...
      ce('div',{style:{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}},
        ...
        fld('Тип',ce('select',{value:kind,onChange:e=>setKind(e.target.value),style:mecInp},
    ...
    ce('div',{style:{display:'flex',gap:9,marginTop:16}},
      ce(Btn,{onClick:onCancel,color:'var(--text3)',wide:true},'Отмена'),
      ce('button',{onClick:function(){ if(!bad) onSave(Object.assign({},d,{date:d.date||dayKey,clientName:clientName.trim(),count:Number(count),kind:kind,time:time.trim(),note:note.trim()})); },
...
function DemandsTab({demands,trucks,clients,onSave,onDel,carrierOffers,onDelOffer,onArchiveOffer,onLaunch}){
  ...
  return ce('div',{style:{padding:'14px',maxWidth:1700,margin:'0 auto'}},
    adding?ce(DemandEditor,{dayKey:day,clients:clients,onSave:function(d){ onSave(d); setAdding(false); },onCancel:function(){setAdding(false);}}):null,
    edit?ce(DemandEditor,{d:edit,clients:clients,dayKey:day,onSave:function(d){ onSave(d); setEdit(null); },onCancel:function(){setEdit(null);}}):null,
...
function AppMain() {
  ...
  return ce('div',{style:{minHeight:'100vh',color:'var(--text)'}},
    ...
    tab==='demands'&&ce(DemandsTab,{demands:demands,trucks:trucks,clients:clients,onSave:saveDemand,onDel:delDemand,onLaunch:launchDemand,
