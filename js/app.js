...
function PlanRowEditor({row,clients,orders,trucks,dateKey,onSave,onCancel}){
  ...
  return ce(Modal,{title:(row.id?'✎ Строка плана':'➕ Новая строка')+' · '+fmtD(row.loadDate||dateKey),onClose:onCancel},
    ce('div',{style:{display:'grid',gap:10}},
      ...
      fld('Обратка (что назад / куда)',ce('input',{value:back,onChange:e=>setBack(e.target.value),list:'fl-plan-orders',placeholder:'АРХБУМ Подольск, Ивакино…',style:mecInp})),
      ce('datalist',{id:'fl-plan-orders'},(orders||[]).map(function(o,i){ return ce('option',{key:i,value:(o.client||'')+' '+(o.to||'')}); })),
...
function PlanBoard({plans,clients,orders,trucks,onSavePlan,onDelPlan,onLaunch,dayNotes,onSaveDayNote,onCopyDriver}){
  ...
  const days=[]; for(var i=-3;i<12;i++){ var dd=new Date(base.getTime()+i*864e5); var kk=dk(dd); if(i>=0 || (plans||[]).some(function(p){ return (p.loadDate||p.date)===kk; })) days.push(dd); }  // прошлые дни показываем только если в них есть планы
  ...
  return ce('div',{style:{padding:'14px',maxWidth:1100,margin:'0 auto'}},
    ...
    days.map(function(d){
      ...
      return ce('div',{key:k,style:{marginBottom:18}},
        ...
        rows.length===0?ce('div',{style:{fontSize:12.5,color:'var(--text3)',padding:'4px 2px 8px',fontStyle:'italic'}},'—'):
        ce('div',{style:{display:'grid',gap:5}},
          rows.map(function(p){
            var col=clientColor(p.clientName);
            var launched=!!p.launched;
            return ce('div',{key:p.id,style:{display:'flex',alignItems:'center',gap:10,padding:'8px 11px',borderRadius:9,flexWrap:'wrap',
              background:launched?'color-mix(in srgb,var(--green) 7%,transparent)':'var(--bg2)',
              border:'1px solid '+(launched?'color-mix(in srgb,var(--green) 30%,transparent)':(overdue&&!launched?'color-mix(in srgb,var(--amber) 35%,transparent)':'var(--border)')),
              borderLeft:'4px solid '+col}},
              // слева — время освобождения
              ce('span',{style:{minWidth:78,flexShrink:0,fontSize:11,fontWeight:700,textAlign:'center',lineHeight:1.25,
                color:p.freeAt?'var(--cyan)':'var(--text3)'}},
                p.freeAt?[ce('div',{key:'d'},new Date(p.freeAt).toLocaleDateString('ru',{day:'2-digit',month:'2-digit'})),
                          ce('div',{key:'t',style:{fontSize:12.5,fontWeight:800,color:'var(--text)'}},new Date(p.freeAt).toLocaleTimeString('ru',{hour:'2-digit',minute:'2-digit'}))]
                        :ce('span',{style:{fontStyle:'italic',fontSize:10}},'освоб.\n—')),
              // клиент цветной
              ce('span',{style:{fontWeight:800,fontSize:13.5,color:col,minWidth:110}},p.clientName||'—'),
              // водитель зелёным если подтверждён
              p.truckDriver?ce('span',{style:{fontSize:12.5,fontWeight:700,color:p.confirmed?'var(--green)':'var(--text2)',minWidth:120}},p.truckDriver+(p.truckPlate?(' · '+p.truckPlate):'')):
                ce('span',{style:{fontSize:12,color:'var(--text3)',minWidth:120,fontStyle:'italic'}},'нет водителя'),
              (function(){
                var mt=(trucks||[]).find(function(x){
                ...
                return (mt&&mt.isHired)?ce('span',{style:{fontSize:10,fontWeight:800,padding:'2px 8px',borderRadius:6,whiteSpace:'nowrap',
                  background:'color-mix(in srgb,var(--amber) 16%,transparent)',color:'var(--amber)'}},'🚚 привлечённое'+(mt.carrier?(' · '+mt.carrier):'')):null;
              })(),
              // обратка синим
              p.back?ce('span',{style:{fontSize:12,color:'var(--cyan)',flex:1,minWidth:100}},'↩ '+p.back):ce('span',{style:{flex:1}}),
              p.note?ce('span',{style:{fontSize:11.5,color:'var(--text3)',flexBasis:'100%',order:9}},'📝 '+p.note):null,
              ce('label',{onClick:function(e){e.stopPropagation();},style:{display:'flex',alignItems:'center',gap:5,fontSize:10.5,color:p.driverSent?'var(--green)':'var(--text3)',cursor:'pointer',whiteSpace:'nowrap'}},
                ce('input',{type:'checkbox',checked:!!p.driverSent,onChange:function(e){ onSavePlan(Object.assign({},p,{driverSent:e.target.checked})); },style:{width:13,height:13,cursor:'pointer'}}),
                'данные водителю'),
              ce('button',{className:'fl-press',title:'Скопировать текст для водителя',onClick:function(e){ e.stopPropagation();
                var cl=(clients||[]).find(function(c){ return c.name===p.clientName; });
                var lines=[];
                lines.push('🚛 Рейс на '+(p.loadDate||''));
                lines.push('Клиент: '+(p.clientName||''));
                if(cl&&cl.loadAddr) lines.push('📍 Загрузка: '+cl.loadAddr);
                var ua=(cl&&cl.unloadOptions&&cl.unloadOptions[0]&&cl.unloadOptions[0].addr)||'';
                if(ua) lines.push('🎯 Выгрузка: '+ua);
                if(cl&&cl.temp) lines.push('🌡️ Режим: '+cl.temp);
                if(p.back) lines.push('↩ Обратка: '+p.back);
                if(p.note) lines.push('📝 '+p.note);
                var txt=lines.join('\n');
                try{ navigator.clipboard.writeText(txt); }catch(_){}
                if(onCopyDriver) onCopyDriver(txt); },
                style:{padding:'5px 9px',borderRadius:7,cursor:'pointer',fontFamily:'inherit',fontSize:10.5,fontWeight:800,whiteSpace:'nowrap',
                border:'1px solid color-mix(in srgb,var(--cyan) 40%,transparent)',background:'color-mix(in srgb,var(--cyan) 10%,transparent)',color:'var(--cyan)'}},'📋 текст водителю'),
              launched?ce('div',{style:{display:'flex',alignItems:'center',gap:8,marginLeft:'auto'}},
                ce('span',{style:{fontSize:11,color:'var(--green)',fontWeight:800}},'✓ в рейсе'),
                ce('button',{className:'fl-press',onClick:function(){setEdit(p);},title:'Изменить и переназначить',style:{width:24,height:24,borderRadius:6,border:'1px solid var(--border2)',background:'var(--bg3)',color:'var(--text2)',cursor:'pointer',fontSize:11}},'✎'),
                ce('button',{className:'fl-press',onClick:function(){ if(window.confirm('Убрать строку из плана? Рейс в «Рейсах» останется.')) onDelPlan(p.id); },title:'Убрать из плана',style:{width:24,height:24,borderRadius:6,border:'1px solid var(--border2)',background:'transparent',color:'var(--text3)',cursor:'pointer',fontSize:11}},'✕')):
              ce('div',{style:{display:'flex',gap:5,marginLeft:'auto'}},
                ce('button',{className:'fl-press',onClick:function(){setEdit(p);},title:'Изменить',style:{width:28,height:28,borderRadius:7,border:'1px solid var(--border2)',background:'var(--bg3)',color:'var(--text2)',cursor:'pointer',fontSize:12}},'✎'),
                ce('button',{className:'fl-press',onClick:function(){ if(window.confirm('Отправить в рейс: '+(p.truckDriver||p.truckPlate||'машину')+' → '+(p.clientName||'')+'? Данные клиента подставятся автоматически.')) onLaunch(p); },
                  style:{padding:'6px 12px',borderRadius:8,border:'none',cursor:'pointer',fontFamily:'inherit',fontSize:12,fontWeight:800,color:'#fff',background:'linear-gradient(135deg,var(--accent),var(--accent2))'}},'▶ В рейс'),
                ce('button',{className:'fl-press',onClick:function(){ if(window.confirm('Удалить строку?')) onDelPlan(p.id); },title:'Удалить',style:{width:28,height:28,borderRadius:7,border:'1px solid color-mix(in srgb,var(--red) 40%,transparent)',background:'transparent',color:'var(--red)',cursor:'pointer',fontSize:12}},'🗑'))); }))); }));
}
