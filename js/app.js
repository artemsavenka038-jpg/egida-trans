...
function workedDays(d){
  if(!d||!d.workStart) return null;
  if(d.status==='vacation'||d.status==='off') return null;
  var t=new Date(d.workStart); if(isNaN(t.getTime())) return null;
  t.setHours(0,0,0,0); var n=new Date(); n.setHours(0,0,0,0);
  return Math.floor((n.getTime()-t.getTime())/86400000)+1;
}
...
function TruckCard({truck,clients,orders,settings,role,isBoss,onUpdate,onSwapRequest,onOpenDriver,onDelete}) {
...
function MechLegend(){
  ...
  return ce('div',{style:{background:'var(--bg2)',border:'1px solid var(--border)',borderRadius:13,marginBottom:20,overflow:'hidden'}},
    ...
    open?ce('div',null,
      ...
      row('🛞','Шины: рулевая ось','ресурс '+tk(TIRE_STEER_KM)+'. Жёлтый — меньше '+tk(TIRE_WARN_KM)+'.'),
      row('🛞','Шины: ведущая ось','ресурс '+tk(TIRE_DRIVE_KM)+'. Жёлтый — меньше '+tk(TIRE_WARN_KM)+'.'),
      row('🛞','Шины прицепа (1/2/3 ось)','ресурс '+tk(TIRE_TRAILER_KM)+' на каждую ось отдельно. Считается по пробегу тягача, который её возит.'),
...
function MechDash({tractors,trailers,acts,onJump}){
  ...
  tractors.forEach(function(t){
    ...
    items.push({plate:t.plate||'—',cat:'Шины руль',s:kmStatus(t.odometer,t.tireSteer,TIRE_STEER_KM,TIRE_WARN_KM),jump:'tractors'});
    items.push({plate:t.plate||'—',cat:'Шины ведущая',s:kmStatus(t.odometer,t.tireDrive,TIRE_DRIVE_KM,TIRE_WARN_KM),jump:'tractors'});
  ...
  trailers.forEach(function(tr){
    ...
    items.push({plate:tr.plate||'—',cat:'Шины прицепа 1 ось',s:kmStatus(odo,axleVal(tr,1),TIRE_TRAILER_KM,TIRE_WARN_KM),jump:'trailers'});
    items.push({plate:tr.plate||'—',cat:'Шины прицепа 2 ось',s:kmStatus(odo,axleVal(tr,2),TIRE_TRAILER_KM,TIRE_WARN_KM),jump:'trailers'});
    items.push({plate:tr.plate||'—',cat:'Шины прицепа 3 ось',s:kmStatus(odo,axleVal(tr,3),TIRE_TRAILER_KM,TIRE_WARN_KM),jump:'trailers'});
...
function TractorCard({t,onUpd,onDel}){
  ...
  const ts=kmStatus(t.odometer,t.tireSteer,TIRE_STEER_KM,TIRE_WARN_KM);
  const td=kmStatus(t.odometer,t.tireDrive,TIRE_DRIVE_KM,TIRE_WARN_KM);
...
function TrailerCard({tr,tractors,onUpd,onDel}){
  ...
  const ts1=kmStatus(odo,axleVal(tr,1),TIRE_TRAILER_KM,TIRE_WARN_KM);
  const ts2=kmStatus(odo,axleVal(tr,2),TIRE_TRAILER_KM,TIRE_WARN_KM);
  const ts3=kmStatus(odo,axleVal(tr,3),TIRE_TRAILER_KM,TIRE_WARN_KM);
...
function ActCard({a,onSave,onDel}){
  ...
  return ce('div',{className:'fl-card',style:{background:'var(--bg2)',border:'1px solid var(--border)',borderRadius:14,padding:14,marginBottom:12,boxShadow:'var(--shadow)',opacity:a.resolved?0.6:1}},
    ce('div',{style:{display:'flex',alignItems:'center',gap:10,marginBottom:8,flexWrap:'wrap'}},
      ...
      ce('label',{style:{marginLeft:'auto',display:'flex',alignItems:'center',gap:6,fontSize:12,color:a.resolved?'var(--green)':'var(--text3)',fontWeight:700,cursor:'pointer'}},
        ce('input',{type:'checkbox',checked:!!a.resolved,onChange:function(e){ onSave(Object.assign({},a,{resolved:e.target.checked})); }}),
        a.resolved?'Устранено':'Открыт'),
      ce('button',{onClick:function(){setEditing(true);},className:'fl-press',style:{width:30,height:30,borderRadius:8,border:'1px solid var(--border2)',background:'var(--bg3)',color:'var(--text2)',cursor:'pointer'}},'✎'),
      ce('button',{onClick:function(){ if(window.confirm('Удалить акт №'+(a.num||'')+'?')) onDel(a.id); },className:'fl-press',
    ...
    ce('div',{style:{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14}},
      ce('div',null, ce('div',{style:{fontSize:10.5,color:'var(--text3)',fontWeight:700,marginBottom:4}},'ТЯГАЧ'),
        ce('div',{style:{fontSize:12.5,whiteSpace:'pre-wrap',color:'var(--text2)'}},a.tractorDefects||'—')),
      ce('div',null, ce('div',{style:{fontSize:10.5,color:'var(--text3)',fontWeight:700,marginBottom:4}},'ПРИЦЕП'),
        ce('div',{style:{fontSize:12.5,whiteSpace:'pre-wrap',color:'var(--text2)'}},a.trailerDefects||'—'))));
}
...
function MechStock({tires,bags,tractors,trailers,onAddTire,onUpdTire,onDelTire,onAddBag,onUpdBag,onDelBag}){
...
function MechanicTab(props){
  ...
  const tabs=[['dash','📊 Сводка'],['tractors','🚚 Тягачи ('+tractors.length+')'],['trailers','🚛 Прицепы ('+trailers.length+')'],['stock','📦 Склад'],['acts','📋 Осмотры ('+acts.length+')']];
  return ce('div',{style:{padding:'16px',maxWidth:1700,margin:'0 auto'}},
    ...
    view==='stock'&&ce(MechStock,{tires:props.tires||[],bags:props.bags||[],tractors:tractors,trailers:trailers,
...
function HrDash({drivers,vacations,waybills,trucks,onJump}){
  const worked=drivers.map(function(d){ return {d:d,days:workedDays(d),s:workStatus(workedDays(d))}; });
  const red=worked.filter(function(x){ return x.s.st==='red'; });
  const amber=worked.filter(function(x){ return x.s.st==='amber'; });
  const wbBad=waybills.filter(function(w){ return waybillStatus(w.trips).st!=='green'; });
  const tnow=Date.now();
  ...
  const list=red.map(function(x){ return {who:x.d.name||'—',plate:x.d.plate||'',cat:'Без выходного',s:x.s,jump:'drivers'}; })
    ...
    .concat(wbBad.map(function(w){ return {who:w.driverName||'—',plate:w.plate||'',cat:'Путевой лист №'+(w.num||'—'),s:waybillStatus(w.trips),jump:'waybills'}; }));
  return ce('div',null,
    ce('div',{style:{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))',gap:12,marginBottom:20}},
      dashCard('🧑‍✈️ Водителей',drivers.length,'var(--accent)'),
      dashCard('🏖 В отпуске сейчас',onVac.length,'var(--violet)'),
      dashCard('🔴 Устали (30+ дн.)',red.length,'var(--red)'),
      dashCard('🟠 Скоро выходной',amber.length,'var(--amber)'),
...
function HrDrivers({drivers,vacations,onAdd,onUpd,onDel,onImport}){
  const [q,setQ]=useState('');
  const tnow=Date.now();
  const vacOf=function(name){ return vacations.find(function(v){ var a=dOnly(v.from),b=dOnly(v.to); return v.driverName===name&&a!=null&&b!=null&&a<=tnow&&tnow<=b; }); };
  const list=drivers.filter(function(d){ var s=(q||'').toLowerCase(); return !s||((d.name||'')+' '+(d.plate||'')).toLowerCase().indexOf(s)>=0; })
    .sort(function(a,b){ var x=workedDays(a),y=workedDays(b); return (y==null?-1:y)-(x==null?-1:x); });
  return ce('div',null,
    ...
    ce('div',{style:{display:'grid',gap:8}},
      list.map(function(d){ var days=workedDays(d), s=workStatus(days), v=vacOf(d.name);
        return ce('div',{key:d.id,style:{display:'flex',alignItems:'center',gap:10,flexWrap:'wrap',
          ...
          ce('div',{style:{display:'flex',flexDirection:'column',gap:2}},
            ...
            ce('input',{type:'date',defaultValue:d.workStart||'',onBlur:function(e){ onUpd(d.id,{workStart:e.target.value,status:'work'}); },style:Object.assign({},mecInp,{width:145})})),
          v?ce('span',{style:{padding:'5px 11px',borderRadius:8,fontSize:11.5,fontWeight:800,color:'#fff',background:VAC_COLOR[v.type]||'var(--violet)'}},
            (v.type||'отпуск')+' до '+fmtD(v.to)):ce(StatusPill,{s:s,label:'Без выходного'}),
          ce('button',{className:'fl-press',onClick:function(){ onUpd(d.id,{workStart:today(),status:'work'}); },title:'Отметить выход на работу сегодня',
            style:{marginLeft:'auto',padding:'7px 12px',borderRadius:8,cursor:'pointer',fontFamily:'inherit',fontSize:11.5,fontWeight:800,
            border:'1px solid color-mix(in srgb,var(--green) 45%,transparent)',background:'transparent',color:'var(--green)'}},'▶ Вышел сегодня'),
          ce('button',{className:'fl-press',onClick:function(){ onUpd(d.id,{status:'off'}); },title:'Ушёл на выходной — счётчик остановится',
            style:{padding:'7px 12px',borderRadius:8,cursor:'pointer',fontFamily:'inherit',fontSize:11.5,fontWeight:800,
            border:'1px solid var(--border2)',background:'var(--bg3)',color:'var(--text3)'}},'🏠 На выходной'),
...
function HrWaybills({waybills,drivers,onSave,onDel}){
  const add=function(){ var n=window.prompt('Номер путевого листа:'); if(!n||!n.trim()) return;
    onSave({num:n.trim(),driverName:'',plate:'',issued:today(),trips:0,note:''}); };
  const sorted=waybills.slice().sort(function(a,b){ return (Number(b.trips)||0)-(Number(a.trips)||0); });
  return ce('div',null,
    ce('div',{style:{display:'flex',justifyContent:'flex-end',marginBottom:12}},
      ce('button',{className:'fl-press',onClick:add,style:{padding:'9px 16px',borderRadius:10,cursor:'pointer',fontFamily:'inherit',
        border:'1px solid color-mix(in srgb,var(--green) 50%,transparent)',background:'color-mix(in srgb,var(--green) 14%,transparent)',color:'var(--green)',fontWeight:800,fontSize:12.5}},'➕ Новый путевой')),
    sorted.length===0?ce('div',{style:{textAlign:'center',padding:50,color:'var(--text3)'}},'Путевых листов пока нет.'):
    ce('div',{style:{display:'grid',gap:8}},
      sorted.map(function(w){ var s=waybillStatus(w.trips); var n=Number(w.trips)||0;
        return ce('div',{key:w.id,style:{display:'flex',alignItems:'center',gap:10,flexWrap:'wrap',
          background:'var(--bg2)',border:'1px solid '+(s.st==='red'?'color-mix(in srgb,var(--red) 45%,transparent)':'var(--border)'),borderRadius:12,padding:'10px 12px'}},
          ce('div',{style:{display:'flex',flexDirection:'column',gap:2}},
            ce('span',{style:{fontSize:9.5,color:'var(--text3)',fontWeight:700}},'№'),
            ce('input',{defaultValue:w.num||'',onBlur:function(e){ onSave(Object.assign({},w,{num:e.target.value.trim()})); },style:Object.assign({},mecInp,{width:90,fontWeight:800})})),
          ce('select',{value:w.driverName||'',onChange:function(e){ var d=drivers.find(function(x){return x.name===e.target.value;});
            onSave(Object.assign({},w,{driverName:e.target.value,plate:(d&&d.plate)||w.plate||''})); },style:Object.assign({},mecInp,{width:200})},
            ce('option',{value:''},'— водитель —'),
            drivers.map(function(d){ return ce('option',{key:d.id,value:d.name},d.name); })),
          ce('input',{defaultValue:w.plate||'',placeholder:'Машина',onBlur:function(e){ onSave(Object.assign({},w,{plate:e.target.value.trim()})); },style:Object.assign({},mecInp,{width:110})}),
          ce('div',{style:{display:'flex',flexDirection:'column',gap:2}},
            ce('span',{style:{fontSize:9.5,color:'var(--text3)',fontWeight:700}},'ВЫДАН'),
            ce('input',{type:'date',defaultValue:w.issued||'',onBlur:function(e){ onSave(Object.assign({},w,{issued:e.target.value})); },style:Object.assign({},mecInp,{width:140})})),
          ce('div',{style:{display:'flex',alignItems:'center',gap:6}},
            ce('button',{className:'fl-press',onClick:function(){ onSave(Object.assign({},w,{trips:Math.max(0,n-1)})); },
              style:{width:28,height:28,borderRadius:7,border:'1px solid var(--border2)',background:'var(--bg3)',color:'var(--text2)',cursor:'pointer',fontWeight:800}},'−'),
            ce('span',{style:{minWidth:120,textAlign:'center'}},ce(StatusPill,{s:s,label:'Рейсов'})),
            ce('button',{className:'fl-press',onClick:function(){ onSave(Object.assign({},w,{trips:n+1})); },
              style:{width:28,height:28,borderRadius:7,border:'1px solid color-mix(in srgb,var(--green) 45%,transparent)',background:'transparent',color:'var(--green)',cursor:'pointer',fontWeight:800}},'+')),
          ce('button',{className:'fl-press',onClick:function(){ onSave(Object.assign({},w,{trips:0,issued:today()})); },title:'Выдан новый лист — счётчик обнулится',
            style:{marginLeft:'auto',padding:'7px 12px',borderRadius:8,cursor:'pointer',fontFamily:'inherit',fontSize:11.5,fontWeight:800,
            border:'1px solid color-mix(in srgb,var(--accent) 45%,transparent)',background:'transparent',color:'var(--accent)'}},'🔄 Заменён'),
          ce('button',{onClick:function(){ if(window.confirm('Удалить путевой №'+(w.num||'')+'?')) onDel(w.id); },className:'fl-press',
            style:{width:30,height:30,borderRadius:8,border:'1px solid color-mix(in srgb,var(--red) 40%,transparent)',background:'transparent',color:'var(--red)',cursor:'pointer'}},'🗑')); })));
}
function HrBase({trucks,waybills}){
  const moving=(trucks||[]).filter(function(t){ return t.status&&t.status!=='FREE'&&t.status!=='SERVICE'; });
  const wbOf=function(plate){ return (waybills||[]).find(function(w){ return normPlate(w.plate)===normPlate(plate); }); };
  const rows=moving.map(function(t){ var w=wbOf(t.plate); return {t:t,w:w,s:w?waybillStatus(w.trips):null,ft:t.freeAt?new Date(t.freeAt).getTime():null}; })
    ...
    ce('div',{style:{border:'1px solid var(--border)',borderRadius:12,overflow:'hidden'}},
      rows.map(function(r,i){ var t=r.t;
        return ce('div',{key:t.id,style:{display:'flex',alignItems:'center',gap:12,padding:'11px 14px',flexWrap:'wrap',
          ...
          r.w?ce(StatusPill,{s:r.s,label:'Путевой №'+(r.w.num||'—')}):ce('span',{style:{fontSize:11.5,color:'var(--text3)'}},'путевой не заведён')); })));
}
function HrTab(props){
  const [view,setView]=useState('dash');
  const tabs=[['dash','📊 Сводка'],['drivers','🧑‍✈️ Водители ('+props.drivers.length+')'],['vac','📅 Отпуска'],['waybills','📋 Путевые ('+props.waybills.length+')'],['base','🚚 На базу']];
  return ce('div',{style:{padding:'16px',maxWidth:1700,margin:'0 auto'}},
    ce('div',{style:{display:'inline-flex',gap:3,marginBottom:16,background:'var(--bg2)',border:'1px solid var(--border)',borderRadius:11,padding:3,flexWrap:'wrap'}},
      tabs.map(function(v){ var on=view===v[0];
        return ce('button',{key:v[0],className:'fl-press',onClick:function(){setView(v[0]);},style:{padding:'8px 16px',borderRadius:9,cursor:'pointer',fontFamily:'inherit',fontSize:13,fontWeight:800,border:'none',
          background:on?'linear-gradient(135deg,var(--violet),var(--accent))':'transparent',color:on?'#fff':'var(--text3)'}},v[1]); })),
    view==='dash'&&ce(HrDash,{drivers:props.drivers,vacations:props.vacations,waybills:props.waybills,trucks:props.trucks,onJump:setView}),
    ...
    view==='waybills'&&ce(HrWaybills,{waybills:props.waybills,drivers:props.drivers,onSave:props.onSaveWaybill,onDel:props.onDelWaybill}),
    view==='base'&&ce(HrBase,{trucks:props.trucks,waybills:props.waybills}));
}
function App() {
  ...
  window._lang = lang;
  const [tab,setTab] = useState(()=>{ try{ var _r=localStorage.getItem('egida_role'); return _r==='Механик'?'mechanic':(_r==='Кадры'?'hr':'park'); }catch(e){ return 'park'; } });
  ...
  const importHrDrivers = ()=>{
    ...
    var add=[]; trucks.forEach(function(t){ var nm=(t.driver||'').trim(); if(!nm||have[nm.toLowerCase()]) return; have[nm.toLowerCase()]=true;
      add.push({id:'hd_'+Date.now()+'_'+add.length,name:nm,plate:t.plate||'',workStart:'',status:'work',note:''}); });
    if(!add.length){ tShow('Новых водителей не найдено'); return; }
    ...
    Promise.all(add.map(function(o){ return dbSave('/hr/drivers/'+o.id,o); })).then(function(rs){
      var bad=rs.filter(function(r){ return !r.ok; });
      if(bad.length) tErr('Добавлены на экране, но '+bad.length+' не сохранились: '+bad[0].err); else tShow('Добавлено водителей: '+add.length);
    });
  };
  ...
  return ce('div',{style:{minHeight:'100vh',color:'var(--text)'}},
    ...
    ce('div',{style:{background:'color-mix(in srgb,var(--bg2) 88%,transparent)',backdropFilter:'blur(10px)',
      ...
      ce('div',{style:{maxWidth:1700,margin:'0 auto',display:'flex',alignItems:'center',justifyContent:'space-between',height:60,gap:14}},
        ...
        ce('div',{style:{display:'flex',gap:11,alignItems:'center',flexShrink:0}},
          ...
          ce('button',{className:'fl-press',onClick:()=>setModal('logists'),style:Object.assign({},headBtn,{color:'var(--amber)',borderColor:'color-mix(in srgb,var(--amber) 40%,transparent)'})},'👥 Логисты'),
    ...
    tab==='park'&&ce('div',{style:{padding:'16px',maxWidth:1700,margin:'0 auto'}},
      ...
      (!isBoss)&&ce('div',{style:{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(330px,1fr))',gap:12}},
        filtered.map(t=>ce(TruckCard,{key:t.id,truck:t,clients,orders,settings,role,isBoss,onUpdate:updTruck,
          onSwapRequest:tk=>setModal({type:'swap',truck:tk}),onOpenDriver:id=>setDriverPageId(id),onDelete:delTruck}))),
    ...
    tab==='hr'&&ce(HrTab,{drivers:hrDrivers,vacations:hrVacations,waybills:hrWaybills,trucks:trucks,
    ...
    tab==='mechanic'&&ce(MechanicTab,{tractors:mecTractors,trailers:mecTrailers,acts:mecActs,
      ...
      tires:mecTires,bags:mecBags,onAddTire:addTire,onUpdTire:updTire,onDelTire:delTire,onAddBag:addBag,onUpdBag:updBag,onDelBag:delBag}),
