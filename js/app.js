...
function countTrips(name, sinceISO, history, plans){
  ...
  var n=0, seen={};
  // 1) события отправки в журнале
  (history||[]).forEach(function(e){
    if(!e||e.action!=='dispatch'||!sameDriver(e.driver,name)) return;
    var d=String(e.ts||'').slice(0,10);
    if(since && d && d<since) return;
    var key='h'+(e.ref||'')+d; if(seen[key]) return; seen[key]=1; n++;
  });
  // 2) ЗАПУЩЕННЫЕ планы этого водителя (надёжно — хранятся в базе)
  (plans||[]).forEach(function(p){
    if(!p||!p.launched||!sameDriver(p.truckDriver,name)) return;
    var d=String(p.loadDate||p.date||'').slice(0,10);
    if(since && d && d<since) return;
    var key='p'+(p.id||d); if(seen[key]) return; seen[key]=1; n++;
  });
  return n;
}
...
function PlanGrid({plans,clients,orders,trucks,geoIndex,onSavePlan,onDelPlan,onLaunch,onAddOrder,onSetBar,role,logists,isBoss,gridBacks,onSetGridBack,onDelOrder,gridTuda,onSetGridTuda}){
  ...
  return ce('div',{style:{padding:'14px',maxWidth:1500,margin:'0 auto'}},
    edit?ce(PlanRowEditor,{row:edit,clients:clients,orders:orders,trucks:trucks,dateKey:edit.loadDate,
      ...
      onDelete:((edit.loadDate||edit.date||'')>=todayK)?function(id){ if(onDelPlan) onDelPlan(id); setEdit(null); }:null}):null,
          ...
          return ce('div',{key:ri,className:'fl-grid-row',style:{display:'grid',gridTemplateColumns:gridCols,borderBottom:'1px solid var(--border)',position:'relative',minHeight:44}},
            ...
            ce('div',{style:{padding:'6px 10px',borderRight:'1px solid var(--border)',display:'flex',flexDirection:'column',justifyContent:'center'}},
              ...
              (function(){
                ...
                var okT=true;
                if(cur!=null && req){
                  var nums=String(req).replace(',','.').match(/-?\d+(?:\.\d+)?/g);
                ...
                return ce('div',{style:{fontSize:8.5,marginTop:2,display:'flex',alignItems:'center',gap:4,whiteSpace:'nowrap'}},
                  ce('span',{style:{color:'var(--text3)'}},'🌡'),
                  cur!=null?ce('span',{style:{fontWeight:800,color:okT?'var(--green)':'var(--red)'}},(cur>0?'+':'')+cur+'°'):null,
                  req?ce('span',{style:{color:'var(--text3)'}},'/ нужно '+req):null);
              })()),
            // ячейки дней
