...
function ManagerKPI({trucks,orders}){
  ...
  return ce('div',null,
    ...
    ce('div',{style:{fontSize:11,color:'var(--text3)',marginTop:4,lineHeight:1.6}},
      '«Обратка» — сколько машин в рейсе едут с обратным грузом (а не порожняком), напр. 3/5. «Опоздания» — расчётное время освобождения уже прошло, а машина ещё в рейсе. «Отметка» — когда водитель последний раз отмечал статус; ⚠ если не отмечался больше 6 часов.'));
}
...
function App() {
  ...
  useEffect(()=>{ var f=function(){ dbGet('/chat/last').then(function(x){ if(x) setChatLast(x); }).catch(function(){}); }; f(); var iv=setInterval(f,12000); return function(){ clearInterval(iv); }; },[]);
