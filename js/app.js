...
function AppMain() {
  ...
  const launchPlan = (p)=>{
    if(!p) return;
    var t=(trucks||[]).find(function(x){ return String(x.id)===String(p.truckId)||x.plate===p.truckPlate||(p.truckDriver&&(x.driver||'')===p.truckDriver); });
    if(!t){ tErr('Машина/водитель «'+(p.truckDriver||p.truckPlate||'')+'» не найдена в парке — заведите её или впишите точный номер.'); return; }
    // подставляем данные клиента: адрес загрузки/выгрузки, температура — чтобы логист не путал вручную
    ...
    var patch={status:'FORWARD',
      ...
      note:p.note||t.note||''};
    updTruck(t.id,patch);
    // помечаем строку запущенной (остаётся видна как «✓ в рейсе», не удаляем)
    var pl=Object.assign({},p,{launched:true,truckPlate:t.plate||p.truckPlate||''});
    dbPut('/plans/'+p.id,pl); setPlans(function(x){ return x.map(function(y){ return y.id===p.id?pl:y; }); });
    tShow('▶ '+(t.driver||t.plate)+' → '+(p.clientName||'')+' в рейсе');
  };
