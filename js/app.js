...
function PlanBoard({plans,clients,orders,trucks,onSavePlan,onDelPlan,onLaunch,dayNotes,onSaveDayNote,onCopyDriver}){
  ...
  // дни: сегодня + 11 вперёд, ПЛЮС любые прошедшие дни, где есть планы (на любую глубину назад)
  var allDays=[];
  var seen={};
  // прошедшие дни с планами (сколько бы назад ни были)
  (plans||[]).forEach(function(p){ var kk=p.loadDate||p.date; if(kk && kk<todayK && !seen[kk]){ seen[kk]=1; var parts=kk.split('-'); allDays.push(new Date(Number(parts[0]),Number(parts[1])-1,Number(parts[2]))); } });
  // сегодня и будущие
  for(var i=0;i<12;i++){ var dd=new Date(base.getTime()+i*864e5); var kk=dk(dd); if(!seen[kk]){ seen[kk]=1; allDays.push(dd); } }
  allDays.sort(function(a,b){ return a.getTime()-b.getTime(); });
  // прошедшие дни (до сегодня) прячем под кнопку, но не удаляем
  const pastDays=allDays.filter(function(d){ return dk(d)<todayK; });
  const futureDays=allDays.filter(function(d){ return dk(d)>=todayK; });
