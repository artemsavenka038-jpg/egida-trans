    ...
    ce('div',{style:{display:'flex',alignItems:'center',gap:12,marginBottom:14,flexWrap:'wrap'}},
      ...
      ce('div',{style:{display:'flex',alignItems:'center',gap:6,marginLeft:'auto'}},
        ...
        ce('button',{className:'fl-press',onClick:function(){
          ...
          var grid=document.querySelector('.fl-grid-wrap');
          if(grid){
            // A4 альбом ~ 1080×740px при полях; масштабируем по ширине И высоте
            var pageW=1080, pageH=740;
            var w=grid.scrollWidth||900, h=grid.scrollHeight||600;
            var sc=Math.min(1, pageW/w, pageH/h);
            grid.style.transformOrigin='top left'; grid.style.transform='scale('+sc+')';
            grid.style.width=w+'px'; grid.parentNode.style.height=(h*sc)+'px'; grid.parentNode.style.overflow='hidden';
          }
          setTimeout(function(){ window.print();
            setTimeout(function(){ document.body.classList.remove('fl-printing');
              if(grid){ grid.style.transform=''; grid.style.width=''; grid.parentNode.style.height=''; grid.parentNode.style.overflow=''; }
            },400);
          },100);
        },title:'Печать',style:{padding:'6px 12px',borderRadius:8,cursor:'pointer',fontFamily:'inherit',fontSize:13,border:'1px solid var(--border2)',background:'var(--bg2)',color:'var(--text2)'}},'🖨 Печать'))),
    // сетка
...
function PlanRowEditor({row,clients,orders,trucks,dateKey,onSave,onCancel,onDelete}){
  ...
  return ce(Modal,{title:(row.id?'✎ Строка плана':'➕ Новая строка')+' · '+fmtD(row.loadDate||dateKey),onClose:onCancel},
    ...
    ce('div',{style:{display:'flex',gap:9,marginTop:16}},
      ...
      ce('button',{onClick:function(){ if(!bad) onSave(Object.assign({},row,{loadDate:row.loadDate||dateKey,clientName:clientName.trim(),truckDriver:driver.trim(),truckPlate:plate.trim(),back:back.trim(),note:note.trim(),freeAt:freeAt?new Date(freeAt).toISOString():'',confirmed:confirmed})); },
        disabled:bad,className:'fl-press',style:{flex:1,padding:'12px',border:'none',borderRadius:11,color:'#fff',fontSize:14,fontWeight:800,cursor:bad?'default':'pointer',
        background:bad?'var(--bg3)':'linear-gradient(135deg,var(--accent),var(--accent2))',opacity:bad?0.5:1}},'Сохранить')));
}
  ...
  useEffect(function(){
    var load=function(){
      ...
      capFetch('temperature/current').then(function(r){
        ...
        // обновляем машины: currentTemp (факт), reqTemp (требуемая из клиента)
        setTrucks(function(prev){ return prev.map(function(t){
          var trailer=splitFleteraPlate(t.plate).trailer;
          var td=trailer&&tmap[trailer];
          if(!td) return t;
          var cl=(clients||[]).find(function(c){ return c.id===t.clientId; });
          var req=(cl&&cl.temp)||t.reqTemp||'';
          if(t.currentTemp===td.t && t.reqTemp===req) return t;
          var nt=Object.assign({},t,{currentTemp:td.t,currentTempAt:td.at,reqTemp:req});
          dbPatch('/trucks/'+t.id,{currentTemp:td.t,currentTempAt:td.at,reqTemp:req});
          return nt;
        }); });
      });
    };
