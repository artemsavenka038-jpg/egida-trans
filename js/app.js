...
// ── Status keys ───────────────────────────────────────────────
const MOVING = ['FORWARD','LOADED','BACK','BACK_MOVING','AT_LOAD','ARRIVED_LOAD','LOADING','AT_UNLOAD','ARRIVED_UNLOAD','UNLOADED','BACK_GO'];
// ── Roles / logists ───────────────────────────────────────────
...
const STR = {
  ru:{
    ...
    s_BACK_MOVING:'Везу обратный груз', s_BACK_MOVING_d:'Обратка в машине, еду назад',
    ...
    st_BACK_GO:'За обраткой', st_BACK_MOVING:'Везу обратку', st_SERVICE:'Сервис', st_WASH:'Мойка', st_DOCS:'Путевой',
  ...
  en:{
    ...
    st_BACK_GO:'For return', st_BACK_MOVING:'Return in transit', st_SERVICE:'Service', st_WASH:'Wash', st_DOCS:'Waybill',
...
function PlanGrid({plans,clients,orders,trucks,geoIndex,onSavePlan,onDelPlan,onLaunch,onAddOrder,onSetBar,role,logists,isBoss,gridBacks,onSetGridBack,onDelOrder,gridTuda,onSetGridTuda,driverMarks,onSetDriverMark}){
  ...
  return ce('div',{style:{padding:'14px',maxWidth:1500,margin:'0 auto'}},
    ...
    ce('div',{style:{display:'flex',alignItems:'center',gap:12,marginBottom:14,flexWrap:'wrap'}},
      ce('div',{style:{fontWeight:800,fontSize:19}},'📅 x2 Планирование'),
...
function PlanBoard({plans,clients,orders,trucks,onSavePlan,onDelPlan,onLaunch,dayNotes,onSaveDayNote,onCopyDriver}){
  ...
  return ce('div',{style:{padding:'14px',maxWidth:1100,margin:'0 auto'}},
    ...
    ce('div',{style:{display:'flex',alignItems:'center',gap:10,marginBottom:14,flexWrap:'wrap'}},
      ce('div',{style:{fontWeight:800,fontSize:19}},'🗒 План рейсов'),
      ce('div',{style:{fontSize:12,color:'var(--text3)'}},'как ваш лист — но строку можно отправить в рейс одним нажатием'),
    ...
    days.map(function(d){
      ...
      return ce('div',{key:k,style:{marginBottom:18}},
        ...
        (function(){
          ...
          var own=rows.filter(function(p){ return !isHiredRow(p); });
          var hired=rows.filter(function(p){ return isHiredRow(p); });
          // свои машины без обратки (привлечённые не считаем) — им надо искать обратный груз
          var noBack=own.filter(function(p){ return !p.back && (p.truckDriver||p.truckPlate); });
          if(rows.length===0) return ce('div',{style:{fontSize:12.5,color:'var(--text3)',padding:'4px 2px 8px',fontStyle:'italic'}},'—');
          return ce('div',null,
            noBack.length?ce('div',{style:{marginBottom:10,padding:'8px 11px',borderRadius:9,maxWidth:520,
              ...
              ce('div',{style:{fontSize:10.5,fontWeight:800,color:'var(--amber)',marginBottom:4,textTransform:'uppercase',letterSpacing:.4}},'⚠ Без обратки ('+noBack.length+') — найти обратный груз'),
            ...
            ce('div',{className:'fl-plan-cols',style:{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,alignItems:'start'}},
              ce('div',null,
                ce('div',{style:{fontSize:11,fontWeight:800,color:'var(--text3)',marginBottom:6,textTransform:'uppercase',letterSpacing:.5}},'🚛 Свои ('+own.length+')'),
                own.length?ce('div',{style:{display:'grid',gap:5}},own.map(renderRow)):ce('div',{style:{fontSize:12,color:'var(--text3)',fontStyle:'italic',padding:'4px 2px'}},'—')),
              ce('div',null,
                ce('div',{style:{fontSize:11,fontWeight:800,color:'var(--amber)',marginBottom:6,textTransform:'uppercase',letterSpacing:.5}},'🚚 Привлечённые ('+hired.length+')'),
                hired.length?ce('div',{style:{display:'grid',gap:5}},hired.map(renderRow)):ce('div',{style:{fontSize:12,color:'var(--text3)',fontStyle:'italic',padding:'4px 2px'}},'—'))));
        })()); }));
}
