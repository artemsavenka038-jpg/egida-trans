...
function PlanBoard({plans,clients,orders,trucks,onSavePlan,onDelPlan,onLaunch,dayNotes,onSaveDayNote,onCopyDriver}){
  ...
  var allDays=[]; for(var i=-3;i<12;i++){ var dd=new Date(base.getTime()+i*864e5); var kk=dk(dd); if(i>=0 || (plans||[]).some(function(p){ return (p.loadDate||p.date)===kk; })) allDays.push(dd); }
  ...
  const truckOf=function(p){ return (trucks||[]).find(function(x){
    var n=function(s){return String(s||'').toUpperCase().replace(/[\s\-\/]/g,'');};
    return (p.truckPlate&&n(x.plate)===n(p.truckPlate))||(p.truckDriver&&(x.driver||'').trim().toLowerCase()===String(p.truckDriver).trim().toLowerCase());
  }); };
    ...
    days.map(function(d){
      ...
      return ce('div',{key:k,style:{marginBottom:18}},
        ...
        (function(){
          ...
          return ce('div',{className:'fl-plan-cols',style:{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,alignItems:'start'}},
            ce('div',null,
