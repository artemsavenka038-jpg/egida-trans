...
function BossHr({drivers,vacations,waybills}){
  const tnow=Date.now();
...
function BossPanel(props){
  ...
  return ce('div',{style:{padding:'16px',maxWidth:1700,margin:'0 auto'}},
    ...
    sec==='hr'?ce(BossHr,{drivers:props.hrDrivers,vacations:props.hrVacations,waybills:props.hrWaybills}):null);
}
function MechanicTab(props){
  ...
  const tabs=[['dash','📊 Сводка'],['tractors','🚚 Тягачи ('+tractors.length+')'],['trailers','🚛 Прицепы ('+trailers.length+')'],['stock','📦 Склад'],['kasko','🛠 КАСКО'+(kaskoOpen?(' ('+kaskoOpen+')'):'')],['acts','📋 Осмотры ('+acts.length+')']];
  return ce('div',{style:{padding:'16px',maxWidth:1700,margin:'0 auto'}},
...
function App() {
  ...
  if(isBoss) TABS.unshift(['boss','👔 Пульт']);
  ...
  return ce('div',{style:{minHeight:'100vh',color:'var(--text)'}},
    ...
    tab==='boss'&&ce(BossPanel,{trucks:trucks,clients:clients,plans:plans,
    ...
    tab==='hr'&&ce(HrTab,{drivers:hrDrivers,vacations:hrVacations,waybills:hrWaybills,trucks:trucks.filter(function(t){return !t.isHired;}),
