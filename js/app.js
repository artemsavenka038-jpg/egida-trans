...
function PlanGrid({plans,clients,orders,trucks,geoIndex,onSavePlan,onDelPlan,onLaunch}){
  ...
  const barFor=function(row){
    var pl=row.plans.slice().sort(function(a,b){ return (a.loadDate||'').localeCompare(b.loadDate||''); })[0];
    if(!pl) return null;
    var loadK=pl.loadDate||pl.date;
    var startIdx=weekKeys.indexOf(loadK);
    if(startIdx<0) startIdx=0;
    var endIdx=startIdx;
    if(pl.freeAt){
      // конец по реальному освобождению
      var fk=String(pl.freeAt).slice(0,10); var fi=weekKeys.indexOf(fk);
      if(fi>=0) endIdx=fi; else { var fd=new Date(pl.freeAt); var diff=Math.round((fd-mkDate(loadK))/864e5); endIdx=startIdx+Math.max(0,diff); }
    } else {
      // нет освобождения — по времени рейса клиента (routeHours), иначе 1 день
      var cl=(clients||[]).find(function(c){ return c.name===pl.clientName; });
      var hrs=(cl&&cl.routeHours)||0;
      var addDays=hrs?Math.max(1,Math.round(hrs/24)):1;
      endIdx=startIdx+addDays;
    }
    if(endIdx>6) endIdx=6; if(endIdx<startIdx) endIdx=startIdx;
    return {startIdx:startIdx,endIdx:endIdx,plan:pl,col:clientColor(pl.clientName)};
  };
  ...
  return ce('div',{style:{padding:'14px',maxWidth:1500,margin:'0 auto'}},
    edit?ce(PlanRowEditor,{row:edit,clients:clients,orders:orders,trucks:trucks,dateKey:edit.loadDate,
      onSave:function(r){ onSavePlan(r); setEdit(null); },onCancel:function(){setEdit(null);}}):null,
      ...
      ce('div',{style:{minWidth:900}},
        ...
        (function(){
          ...
          return ce('div',{key:ri,style:{display:'grid',gridTemplateColumns:'160px repeat(7,1fr)',borderBottom:'1px solid var(--border)',position:'relative',minHeight:44}},
            ...
            // полоса рейса поверх (от загрузки до освобождения)
            bar?ce('div',{style:{position:'absolute',top:'50%',transform:'translateY(-50%)',height:4,borderRadius:3,pointerEvents:'none',
              ...
              background:'color-mix(in srgb,'+bar.col+' 55%,transparent)',opacity:0.5}}):null,
            ...
            (function(){
              ...
              // позиция фуры: доля пути внутри полосы (от startIdx до endIdx)
              var span=bar.endIdx-bar.startIdx+1;
              var f=rp.dir==='back'?(1-rp.frac):rp.frac;   // назад — фура едет обратно по полосе
              var posDays=bar.startIdx + f*span;
...
function PlanRowEditor({row,clients,orders,trucks,dateKey,onSave,onCancel}){
  row=row||{};
