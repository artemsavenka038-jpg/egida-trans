...
const Modal = ({title,onClose,children,wide}) => ce('div',{
  ...
  padding:'22px 20px',width:'100%',maxWidth:wide?640:420,
...
function MsgModal({truck,clients,orders,onClose}) {
  ...
  const backMsg = bc?[
    ...
    (bc.text||bc.note)?'📝 '+(bc.text||bc.note):null
  ].filter(Boolean).join('\n'):null;
...
function App() {
  ...
  return ce('div',{style:{minHeight:'100vh',color:'var(--text)'}},
    ...
    tab==='moving'&&(function(){
      ...
      var dayTrucks=trucks.filter(function(t){return onRoute(t,routeDay);});
      ...
      return ce('div',{style:{padding:'16px',maxWidth:1700,margin:'0 auto'}},
        ...
        ce('div',{style:{display:'flex',gap:7,overflowX:'auto',paddingBottom:8,marginBottom:14}},
          days.map(function(d){ var key=ds(d); var act=key===routeDay;
            var cnt=trucks.filter(function(t){return onRoute(t,key);}).length+(plans||[]).filter(function(p){return planOn(p,key);}).length;
        ...
        ce('div',{style:{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(340px,1fr))',gap:12}},
          dayTrucks.map(tr=>ce(MovingCard,{key:tr.id,tr,clients,orders,onUpdate:updTruck,modal:movingModal,setModal:setMovingModal})),
...
function EditTruckModal({truck,onSave,onClose}){
  ...
  var ok=plate&&driver;
  function save(){ if(!ok) return; onSave({plate:plate,driver:driver,phone:phone,pin:pin,isHired:hired,carrier:hired?carrier:''}); }
