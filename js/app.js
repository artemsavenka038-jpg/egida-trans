...
function AppMain() {
  ...
  const launchPlan = (p)=>{
    if(!p) return;
    var t=(trucks||[]).find(function(x){ return String(x.id)===String(p.truckId)||x.plate===p.truckPlate; });
    if(!t){ tErr('Машина '+(p.truckPlate||'')+' не найдена в парке — запустите рейс вручную.'); return; }
    updTruck(t.id,{status:'FORWARD',
      loadAddr:p.from||t.loadAddr||'',unloadAddr:p.to||t.unloadAddr||'',
      loadAt:(p.loadDate||p.date||'')+'T08:00',
      clientName:p.clientName||t.clientName||'',note:p.note||t.note||''});
