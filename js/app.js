...
function PlanGrid({plans,clients,orders,trucks,geoIndex,onSavePlan,onDelPlan,onLaunch,onAddOrder,onSetBar}){
  ...
  // статус-полоса рейса: от дня загрузки до дня освобождения
  const barFor=function(row){
    if(!row.plans.length) return null;
    var sorted=row.plans.slice().sort(function(a,b){ return (a.loadDate||'').localeCompare(b.loadDate||''); });
    var first=sorted[0], last=sorted[sorted.length-1];
    // ручные границы полосы (если логист задал), иначе — по дням планов
    var manualStart='', manualEnd='';
    row.plans.forEach(function(p){ if(p.barStart&&(!manualStart||p.barStart<manualStart)) manualStart=p.barStart;
                                    if(p.barEnd&&(!manualEnd||p.barEnd>manualEnd)) manualEnd=p.barEnd; });
    var loadK=manualStart||first.loadDate||first.date;
    var startIdx=weekKeys.indexOf(loadK);
    if(startIdx<0){ if(loadK<weekKeys[0]) startIdx=0; else return null; }
    var endK=manualEnd||last.loadDate||last.date;
    var endIdx=weekKeys.indexOf(endK);
    if(endIdx<0){ if(endK>weekKeys[6]) endIdx=6; else endIdx=startIdx; }
    // если ручных границ нет и один день — тянем на +1 (или +2 с обраткой)
    var anyBack=row.plans.some(function(p){ return !!p.back; });
    if(!manualEnd){
      if(anyBack) endIdx=Math.max(endIdx, weekKeys.indexOf(last.loadDate||last.date)+1);
      else if(endIdx===startIdx) endIdx=startIdx+1;
    }
    if(endIdx>6) endIdx=6; if(endIdx<startIdx) endIdx=startIdx;
    var backPlan=row.plans.filter(function(p){return !!p.back;}).sort(function(a,b){return (a.loadDate||'').localeCompare(b.loadDate||'');})[0];
    var backIdx=backPlan?weekKeys.indexOf(backPlan.loadDate||backPlan.date):-1;
    return {startIdx:startIdx,endIdx:endIdx,plan:first,col:clientColor(first.clientName),hasBack:anyBack,backIdx:backIdx,manual:!!(manualStart||manualEnd)};
  };
  ...
  return ce('div',{style:{padding:'14px',maxWidth:1500,margin:'0 auto'}},
    ...
    ce('div',{style:{display:'flex',alignItems:'center',gap:12,marginBottom:14,flexWrap:'wrap'}},
      ...
      ce('div',{style:{display:'flex',alignItems:'center',gap:6,marginLeft:'auto'}},
        ...
        ce('button',{className:'fl-press',onClick:function(){ window.print(); },title:'Печать / сохранить в PDF',style:{padding:'6px 12px',borderRadius:8,cursor:'pointer',fontFamily:'inherit',fontSize:13,border:'1px solid var(--border2)',background:'var(--bg2)',color:'var(--text2)'}},'🖨 Печать'))),
    // сетка
          ...
          return ce('div',{key:ri,className:'fl-grid-row',style:{display:'grid',gridTemplateColumns:'160px repeat(7,1fr)',borderBottom:'1px solid var(--border)',position:'relative',minHeight:44}},
            ...
            ce('div',{style:{padding:'6px 10px',borderRight:'1px solid var(--border)',display:'flex',flexDirection:'column',justifyContent:'center'}},
              ...
              (function(){
                ...
                return ce('div',{style:{fontSize:9,fontWeight:800,marginTop:2,color:c}},'● '+lbl);
              })()),
            // ячейки дней
            days.map(function(d,di){ var kk=dk(d);
              ...
              return ce('div',{key:di,onClick:function(){ if(cellPlan) setEdit(cellPlan); else setAddCell({driver:row.driver,dateKey:kk}); },
                ...
                cellPlan?ce('div',{style:{position:'absolute',inset:'4px',borderRadius:5,padding:'3px 5px',fontSize:9.5,lineHeight:1.15,overflow:'hidden',
                  ...
                  cellPlan.back?ce('div',{style:{color:'var(--text3)',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}},'↩ '+cellPlan.back):null,
            ...
            // полоса рейса: с обраткой — выделенная двухцветная, без обратки — красная (надо искать обратку)
            bar?ce('div',{style:{position:'absolute',top:'50%',transform:'translateY(-50%)',height:bar.hasBack?6:5,borderRadius:3,pointerEvents:'none',
              left:'calc(160px + '+(bar.startIdx)+' * ((100% - 160px)/7) + 4px)',
              width:'calc('+(bar.endIdx-bar.startIdx+1)+' * ((100% - 160px)/7) - 8px)',
              background:bar.hasBack
                ?('linear-gradient(90deg, '+bar.col+' 0%, '+bar.col+' 58%, var(--cyan) 58%, var(--cyan) 100%)')
              ...
              opacity:bar.hasBack?0.85:0.7,
              boxShadow:bar.hasBack?('0 0 6px color-mix(in srgb,var(--cyan) 40%,transparent)'):'none'}}):null,
            // ручки для ручного изменения длины полосы (стрелки на концах)
            bar?ce('div',{style:{position:'absolute',top:2,zIndex:3,display:'flex',gap:2,
              left:'calc(160px + '+(bar.startIdx)+' * ((100% - 160px)/7) + 4px)'}},
              ce('button',{className:'fl-press fl-bar-h',title:'Сдвинуть начало влево',onClick:function(e){e.stopPropagation(); if(onSetBar){ var s=weekKeys[Math.max(0,bar.startIdx-1)], en=weekKeys[bar.endIdx]; onSetBar(bar.plan,s,en); }},
              ...
              ce('button',{className:'fl-press fl-bar-h',title:'Сдвинуть начало вправо',onClick:function(e){e.stopPropagation(); if(onSetBar){ var s=weekKeys[Math.min(bar.endIdx,bar.startIdx+1)], en=weekKeys[bar.endIdx]; onSetBar(bar.plan,s,en); }},
                style:{width:16,height:14,borderRadius:3,border:'none',cursor:'pointer',fontSize:8,padding:0,background:'color-mix(in srgb,'+bar.col+' 30%,transparent)',color:'var(--text)'}},'▶')):null,
            bar?ce('div',{style:{position:'absolute',top:2,zIndex:3,display:'flex',gap:2,
              left:'calc(160px + '+(bar.endIdx+1)+' * ((100% - 160px)/7) - 40px)'}},
              ce('button',{className:'fl-press fl-bar-h',title:'Укоротить конец',onClick:function(e){e.stopPropagation(); if(onSetBar){ var s=weekKeys[bar.startIdx], en=weekKeys[Math.max(bar.startIdx,bar.endIdx-1)]; onSetBar(bar.plan,s,en); }},
                style:{width:16,height:14,borderRadius:3,border:'none',cursor:'pointer',fontSize:8,padding:0,background:'color-mix(in srgb,'+bar.col+' 30%,transparent)',color:'var(--text)'}},'◀'),
              ce('button',{className:'fl-press fl-bar-h',title:'Удлинить конец',onClick:function(e){e.stopPropagation(); if(onSetBar){ var s=weekKeys[bar.startIdx], en=weekKeys[Math.min(6,bar.endIdx+1)]; onSetBar(bar.plan,s,en); }},
                style:{width:16,height:14,borderRadius:3,border:'none',cursor:'pointer',fontSize:8,padding:0,background:'color-mix(in srgb,'+bar.col+' 30%,transparent)',color:'var(--text)'}},'▶')):null,
            // движущаяся фура по GPS — стоит там, где реально проехала
            (function(){
              if(!bar||!mt) return null;
              var _sp=splitFleteraPlate(mt.plate||''); var g=geoIndex&&geoIndex[_sp.tractor];
              if(!g) return null;
              // клиент для маршрута: из машины, а если нет — из плана строки
              var mtForRoute=mt;
              ...
              var posDays;
              if(rp.dir==='back' && bar.backIdx>=0){
                // везёт обратку: фура на обратном участке (от backIdx до конца), едет к концу по мере пути
                var backStart=bar.backIdx, backSpan=(bar.endIdx-backStart+1);
                posDays=backStart + rp.frac*backSpan;
              } else {
                // едет туда: от начала полосы
                posDays=bar.startIdx + rp.frac*span;
              }
              var goBack=(rp.dir==='back');
              return ce('div',{style:{position:'absolute',top:'50%',transform:'translate(-50%,-50%)',pointerEvents:'none',zIndex:2,transition:'left .5s ease',textAlign:'center',
                left:'calc(160px + '+posDays+' * ((100% - 160px)/7))'}},
                ce('div',{style:{fontSize:8,fontWeight:800,color:goBack?'var(--cyan)':bar.col,marginBottom:-2,whiteSpace:'nowrap'}},goBack?'◀ обратно':'туда ▶'),
