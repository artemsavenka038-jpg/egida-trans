...
function PlanGrid({plans,clients,orders,trucks,geoIndex,onSavePlan,onDelPlan,onLaunch}){
  ...
  const driverRows=useMemo(function(){
    var map={};
    // 1) все водители парка — всегда в сетке (даже без планов)
    (trucks||[]).forEach(function(t){
      if(!(t.driver||'').trim()) return;
      var key=(t.driver||t.plate||'—');
      if(!map[key]) map[key]={driver:t.driver||'',plate:t.plate||'',plans:[],isHired:!!t.isHired,carrier:t.carrier||''};
    });
  ...
  const barFor=function(row){
    ...
    var loadK=first.loadDate||first.date;
    var startIdx=weekKeys.indexOf(loadK);
    // загрузка раньше этой недели, но рейс заходит в неё (по freeAt) — полоса от начала недели
    if(startIdx<0){
      if(loadK<weekKeys[0]) startIdx=0;   // загрузка до недели — начинаем с понедельника
      else return null;                    // загрузка после недели — не показываем
    }
    var lastIdx=weekKeys.indexOf(last.loadDate||last.date); if(lastIdx<0) lastIdx=startIdx;
    var endIdx=lastIdx;
    var anyBack=row.plans.some(function(p){ return !!p.back; });
    var mtRow=(trucks||[]).find(function(x){ return (x.driver||'').trim().toLowerCase()===(row.driver||'').trim().toLowerCase(); });
    var freeK='';
    row.plans.forEach(function(p){ if(p.freeAt){ var k=String(p.freeAt).slice(0,10); if(k>freeK) freeK=k; } });
    if(!freeK && mtRow && mtRow.freeAt) freeK=String(mtRow.freeAt).slice(0,10);
    if(freeK){ var fi=weekKeys.indexOf(freeK); if(fi>=0 && fi>endIdx) endIdx=fi;
      else if(fi<0 && freeK>weekKeys[6]) endIdx=6; }
    if(anyBack){ endIdx=Math.max(endIdx, lastIdx+1); }
    else if(lastIdx===startIdx && endIdx===startIdx){ endIdx=startIdx+1; }
    ...
    return {startIdx:startIdx,endIdx:endIdx,plan:first,col:clientColor(first.clientName),hasBack:anyBack,backIdx:backIdx};
  };
    ...
    ce('div',{style:{overflowX:'auto',border:'1px solid var(--border)',borderRadius:12}},
      ce('div',{style:{minWidth:900}},
        ...
        (function(){
          ...
          var bar=barFor(row);
          var mt=(trucks||[]).find(function(x){ return (x.driver||'').trim().toLowerCase()===(row.driver||'').trim().toLowerCase(); });
          ...
          var ownR=driverRows.filter(function(r){ return !isHiredR(r); });
          var hiredR=driverRows.filter(function(r){ return isHiredR(r); });
          return ce('div',null,
            ownR.map(function(r,i){ return renderDriverRow(r,'o'+i); }),
            hiredR.length?ce('div',{style:{display:'grid',gridTemplateColumns:'160px repeat(7,1fr)',background:'color-mix(in srgb,var(--amber) 8%,transparent)',borderTop:'2px solid color-mix(in srgb,var(--amber) 35%,transparent)',borderBottom:'1px solid var(--border)'}},
              ce('div',{style:{gridColumn:'1 / -1',padding:'5px 10px',fontSize:10.5,fontWeight:800,color:'var(--amber)',textTransform:'uppercase',letterSpacing:.5}},'🚚 Привлечённый транспорт')):null,
            hiredR.map(function(r,i){ return renderDriverRow(r,'h'+i); }));
        })())),
              ...
              ce('div',{style:{padding:'6px 8px',display:'grid',gap:4}},
                dayOrders.length?dayOrders.map(function(o,oi){
                  return ce('div',{key:oi,title:(o.from||'')+' → '+(o.to||''),style:{fontSize:10.5,lineHeight:1.25,padding:'3px 6px',borderRadius:5,
                    ...
                    o.to?ce('div',{style:{color:'var(--text3)',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}},o.to):null); })
                :ce('div',{style:{fontSize:10,color:'var(--text3)',fontStyle:'italic',textAlign:'center',padding:'6px 0'}},'—'))); }))))
  );
}
