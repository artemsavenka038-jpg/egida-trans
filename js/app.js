...
function ManagerKPI({trucks,orders}){
  const inWork = t=>MOVING.includes(t.status);
  const truckRev = t=>{ if(!inWork(t)) return 0; var r=0; var o=(orders||[]).find(x=>String(x.assigned)===String(t.id)); if(o) r+=Number(o.rate)||0; if(t.backCargo&&t.backCargo.rate) r+=Number(t.backCargo.rate)||0; return r; };
  const ago = iso=>{ if(!iso) return '—'; var d=new Date(iso); if(isNaN(d.getTime())) return '—'; var m=Math.round((now()-d)/60000); if(m<0) m=0; if(m<1) return 'только что'; if(m<60) return m+' мин'; var h=Math.floor(m/60); if(h<24) return h+' ч'; return Math.floor(h/24)+' дн'; };
  ...
  const total=trucks.length, moving=trucks.filter(inWork).length, free=trucks.filter(t=>t.status==='FREE').length,
        ...
        rev=trucks.reduce((s,t)=>s+truckRev(t),0), util=total?Math.round(moving/total*100):0;
  ...
  function logistRow(g){ var m=g.ts.filter(inWork).length, f=g.ts.filter(t=>t.status==='FREE').length, p=g.ts.filter(t=>t.problem).length,
      r=g.ts.reduce((sm,t)=>sm+truckRev(t),0), u=g.ts.length?Math.round(m/g.ts.length*100):0;
    return ce('tr',{key:g.name},
      ...
      ce('td',{style:Object.assign({},td,{fontWeight:800,color:'var(--green)'})},money(r)+' ₽'));
  }
  const rank=t=>t.problem?0:(inWork(t)?1:2);
  const drivers=trucks.slice().sort((a,b)=>rank(a)-rank(b));
  function driverRow(t){ return ce('tr',{key:t.id,style:{background:t.problem?'color-mix(in srgb,var(--red) 7%,transparent)':'transparent'}},
      ...
      ce('td',{style:Object.assign({},td,{fontWeight:700,color:truckRev(t)?'var(--green)':'var(--text3)'})},truckRev(t)?(money(truckRev(t))+' ₽'):'—'),
      ce('td',{style:Object.assign({},td,{color:'var(--text2)'})},ago(t.lastTime)),
      ce('td',{style:td},t.problem?ce('span',{style:{color:'var(--red)',fontWeight:800}},'⚠'):ce('span',{style:{color:'var(--green)'}},'✓')));
  }
  ...
  return ce('div',null,
    ...
    ce('div',{style:{display:'flex',gap:10,flexWrap:'wrap',marginBottom:22}},
      ...
      stat('Выручка в работе',money(rev)+' ₽','var(--green)')),
    sect('👤 KPI логистов'),
    tableWrap(560, ce('div',null,
      ce('thead',null,ce('tr',null,['Логист','Машин','В рейсе','Свободно','Загрузка','Проблемы','Выручка в работе'].map(h=>ce('th',{key:h,style:th},h)))),
      ce('tbody',null,groups.map(logistRow)))),
    sect('🚛 KPI водителей'),
    tableWrap(620, ce('div',null,
      ce('thead',null,ce('tr',null,['Водитель','Машина','Логист','Статус','Выручка рейса','Отметка','OK'].map((h,i)=>ce('th',{key:i,style:th},h)))),
      ce('tbody',null,drivers.map(driverRow)))),
    ce('div',{style:{fontSize:11,color:'var(--text3)',marginTop:4,lineHeight:1.55}},
      '«Выручка в работе» — сумма ставок по текущим рейсам (заявка + обратный груз). «Отметка» — сколько прошло с последнего статуса от водителя. KPI за период (рейсов и выручка по неделям, % вовремя) появятся, когда включим журнал событий — скажите, добавлю.'));
}
function App() {
  ...
  return ce('div',{style:{minHeight:'100vh',color:'var(--text)'}},
    ...
    tab==='park'&&ce('div',{style:{padding:'16px',maxWidth:1700,margin:'0 auto'}},
      ...
      isBoss&&ce('div',null,
        ...
        managerView==='kpi'?ce(ManagerKPI,{trucks,orders}):ce(ManagerBoard,{trucks,onUpdate:updTruck,search,setSearch})),
