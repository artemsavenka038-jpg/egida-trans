...
function PlanGrid({plans,clients,orders,trucks,geoIndex,onSavePlan,onDelPlan,onLaunch,onAddOrder,onSetBar}){
  ...
  const driverRows=useMemo(function(){
    ...
    (trucks||[]).forEach(function(t){
      ...
      if(t.isHired) return;   // привлечённые пропускаем
      var key=(t.driver||t.plate||'—');
      if(!map[key]) map[key]={driver:t.driver||'',plate:t.plate||'',plans:[],isHired:false,carrier:''};
    });
    // 2) планы этой недели раскидываем по водителям (загрузка в неделе ИЛИ рейс заходит в неё по freeAt)
    (plans||[]).forEach(function(p){
      ...
      var key=(p.truckDriver||p.truckPlate||'—');
      if(!map[key]) map[key]={driver:p.truckDriver||'',plate:p.truckPlate||'',plans:[],isHired:false,carrier:''};
      map[key].plans.push(p);
    });
    // сортировка: сначала у кого есть планы, потом пустые — обе группы по алфавиту
    ...
    ce('div',{style:{overflowX:'auto',border:'1px solid var(--border)',borderRadius:12}},
      ce('div',{style:{minWidth:900}},
        ...
        (function(){
          ...
          return ce('div',{key:ri,style:{display:'grid',gridTemplateColumns:'160px repeat(7,1fr)',borderBottom:'1px solid var(--border)',position:'relative',minHeight:44}},
