...
function PlanGrid({plans,clients,orders,trucks,geoIndex,onSavePlan,onDelPlan,onLaunch,onAddOrder,onSetBar,role,logists,isBoss,gridBacks,onSetGridBack}){
  const [weekOff,setWeekOff]=useState(0);   // 0 = текущая неделя, -1 прошлая, +1 следующая
  ...
  const monday=new Date(base.getTime()-dow*864e5 + weekOff*7*864e5);
  const days=[]; for(var i=0;i<7;i++) days.push(new Date(monday.getTime()+i*864e5));
  ...
  const driverRows=useMemo(function(){
    ...
    (plans||[]).forEach(function(p){
      ...
      var spansIn=!inWeek && kk<weekKeys[0] && p.freeAt && String(p.freeAt).slice(0,10)>=weekKeys[0] && String(p.freeAt).slice(0,10)<=weekKeys[6];
  ...
  const barsFor=function(row){
    ...
    var bars=trips.map(function(tripPlans){
      ...
      if(eI<0){ if(eK>weekKeys[6]) eI=6; else eI=sI; }
  ...
  return ce('div',{style:{padding:'14px',maxWidth:1500,margin:'0 auto'}},
    ...
    ce('div',{style:{display:'flex',alignItems:'center',gap:12,marginBottom:14,flexWrap:'wrap'}},
      ...
      ce('div',{style:{display:'flex',alignItems:'center',gap:6,marginLeft:'auto'}},
        ce('button',{className:'fl-press',onClick:function(){setWeekOff(weekOff-1);},style:{padding:'6px 12px',borderRadius:8,cursor:'pointer',fontFamily:'inherit',fontSize:13,border:'1px solid var(--border2)',background:'var(--bg2)',color:'var(--text2)'}},'‹ неделя'),
        ce('span',{style:{fontSize:13,fontWeight:700,color:'var(--text2)',minWidth:150,textAlign:'center'}},
          days[0].toLocaleDateString('ru',{day:'2-digit',month:'2-digit'})+' — '+days[6].toLocaleDateString('ru',{day:'2-digit',month:'2-digit'})+(weekOff===0?' (эта)':'')),
        ce('button',{className:'fl-press',onClick:function(){setWeekOff(weekOff+1);},style:{padding:'6px 12px',borderRadius:8,cursor:'pointer',fontFamily:'inherit',fontSize:13,border:'1px solid var(--border2)',background:'var(--bg2)',color:'var(--text2)'}},'неделя ›'),
        weekOff!==0?ce('button',{className:'fl-press',onClick:function(){setWeekOff(0);},style:{padding:'6px 12px',borderRadius:8,cursor:'pointer',fontFamily:'inherit',fontSize:12,border:'1px solid var(--accent)',background:'color-mix(in srgb,var(--accent) 12%,transparent)',color:'var(--accent)'}},'сегодня'):null,
    ...
    ce('div',{className:'fl-grid-wrap',style:{overflowX:'auto',border:'1px solid var(--border)',borderRadius:12}},
      ce('div',{style:{minWidth:900}},
        // заголовок дней
        ce('div',{style:{display:'grid',gridTemplateColumns:'160px repeat(7,1fr)',borderBottom:'2px solid var(--border)',background:'var(--bg2)'}},
        ...
        (function(){
          ...
          var renderOneBar=function(bar,bi){
            return [
              ce('div',{key:'bar'+bi,style:{position:'absolute',top:'50%',transform:'translateY(-50%)',height:bar.hasBack?6:5,borderRadius:3,pointerEvents:'none',
                left:'calc(160px + '+(bar.startIdx)+' * ((100% - 160px)/7) + 4px)',
                width:'calc('+(bar.endIdx-bar.startIdx+1)+' * ((100% - 160px)/7) - 8px)',
              ...
              ce('div',{key:'h2'+bi,style:{position:'absolute',top:2,zIndex:3,display:'flex',gap:2,
                left:'calc(160px + '+(bar.endIdx+1)+' * ((100% - 160px)/7) - 40px)'}},
                ce('button',{className:'fl-press fl-bar-h',title:'Конец влево',onClick:function(e){e.stopPropagation(); if(onSetBar){ onSetBar(bar.plan,weekKeys[bar.startIdx],weekKeys[Math.max(bar.startIdx,bar.endIdx-1)]); }},
                  style:{width:16,height:14,borderRadius:3,border:'none',cursor:'pointer',fontSize:8,padding:0,background:'color-mix(in srgb,'+bar.col+' 30%,transparent)',color:'var(--text)'}},'◀'),
                ce('button',{className:'fl-press fl-bar-h',title:'Конец вправо (можно за неделю)',onClick:function(e){e.stopPropagation(); if(onSetBar){
                    var newEnd;
                    if(bar.endIdx>=6){ var d=mkDate(weekKeys[6]); d.setDate(d.getDate()+1); newEnd=dk(d); }  // за воскресенье — реальный следующий день
              ...
              (function(){
                ...
                return ce('div',{key:'truck'+bi,style:{position:'absolute',top:'50%',transform:'translate(-50%,-50%)',pointerEvents:'none',zIndex:2,transition:'left .5s ease',textAlign:'center',
                  left:'calc(160px + '+posDays+' * ((100% - 160px)/7))'}},
          ...
          return ce('div',{key:ri,className:'fl-grid-row',style:{display:'grid',gridTemplateColumns:'160px repeat(7,1fr)',borderBottom:'1px solid var(--border)',position:'relative',minHeight:44}},
    ...
    ce('div',{className:'fl-no-print',style:{marginTop:16}},
      ...
      ce('div',{style:{overflowX:'auto',border:'1px solid var(--border)',borderRadius:12}},
        ce('div',{style:{minWidth:900,display:'grid',gridTemplateColumns:'repeat(7,1fr)'}},
          days.map(function(d,di){ var kk=dk(d);
