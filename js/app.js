...
function PlanGrid({plans,clients,orders,trucks,geoIndex,onSavePlan,onDelPlan,onLaunch,onAddOrder,onSetBar,role,logists,isBoss,gridBacks,onSetGridBack,onDelOrder,gridTuda,onSetGridTuda}){
  ...
  const monday=new Date(base.getTime()-dow*864e5 - 7*864e5);   // понедельник ПРОШЛОЙ недели — прошлое не пропадает
  const NDAYS=28;   // 4 недели в одной ленте (1 прошлая + текущая + 2 вперёд), без переключений
        ...
        (function(){
          ...
          return ce('div',{key:ri,className:'fl-grid-row',style:{display:'grid',gridTemplateColumns:gridCols,borderBottom:'1px solid var(--border)',position:'relative',minHeight:44}},
            ...
            ce('div',{style:{padding:'6px 10px',borderRight:'1px solid var(--border)',display:'flex',flexDirection:'column',justifyContent:'center'}},
              ce('div',{style:{fontSize:12.5,fontWeight:700,color:'var(--text)',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}},row.driver||'—'),
