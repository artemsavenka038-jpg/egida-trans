...
function PlanGrid({plans,clients,orders,trucks,geoIndex,onSavePlan,onDelPlan,onLaunch,onAddOrder,onSetBar,role,logists,isBoss}){
  ...
  return ce('div',{style:{padding:'14px',maxWidth:1500,margin:'0 auto'}},
      ...
      ce('div',{style:{minWidth:900}},
        ...
        (function(){
          ...
          var renderOneBar=function(bar,bi){
            // линия из 3 коротких сегментов на каждый день (не сплошная)
            var segPerDay=3, gap=3;
            var days=(bar.endIdx-bar.startIdx+1);
            return [
              ce('div',{key:'bar'+bi,style:{position:'absolute',top:'50%',transform:'translateY(-50%)',height:bar.hasBack?6:5,borderRadius:2,pointerEvents:'none',
                left:'calc(160px + '+(bar.startIdx)+' * ((100% - 160px)/7) + 4px)',
                width:'calc('+days+' * ((100% - 160px)/7) - 8px)',
                background:bar.hasBack
                  ?('repeating-linear-gradient(90deg, '+bar.col+' 0, '+bar.col+' calc((100%/'+(days*segPerDay)+') - '+gap+'px), transparent calc((100%/'+(days*segPerDay)+') - '+gap+'px), transparent calc(100%/'+(days*segPerDay)+'))')
                  :('repeating-linear-gradient(90deg, color-mix(in srgb,var(--red) 60%,transparent) 0, color-mix(in srgb,var(--red) 60%,transparent) calc((100%/'+(days*segPerDay)+') - '+gap+'px), transparent calc((100%/'+(days*segPerDay)+') - '+gap+'px), transparent calc(100%/'+(days*segPerDay)+'))'),
              ...
              ce('div',{key:'h2'+bi,style:{position:'absolute',top:2,zIndex:3,display:'flex',gap:2,
                ...
                ce('button',{className:'fl-press fl-bar-h',title:'Конец влево',onClick:function(e){e.stopPropagation(); if(onSetBar){ onSetBar(bar.plan,weekKeys[bar.startIdx],weekKeys[Math.max(bar.startIdx,bar.endIdx-1)]); }},
                  style:{width:16,height:14,borderRadius:3,border:'none',cursor:'pointer',fontSize:8,padding:0,background:'color-mix(in srgb,'+bar.col+' 30%,transparent)',color:'var(--text)'}},'◀'),
                ce('button',{className:'fl-press fl-bar-h',title:'Конец вправо',onClick:function(e){e.stopPropagation(); if(onSetBar){ onSetBar(bar.plan,weekKeys[bar.startIdx],weekKeys[Math.min(6,bar.endIdx+1)]); }},
                  style:{width:16,height:14,borderRadius:3,border:'none',cursor:'pointer',fontSize:8,padding:0,background:'color-mix(in srgb,'+bar.col+' 30%,transparent)',color:'var(--text)'}},'▶')),
    ...
    ce('div',{className:'fl-no-print',style:{marginTop:16}},
      ce('div',{style:{fontSize:13,fontWeight:800,marginBottom:8,color:'var(--cyan)'}},'↩ Обратки по дням — что есть в наличии'),
      ce('div',{style:{overflowX:'auto',border:'1px solid var(--border)',borderRadius:12}},
        ce('div',{style:{minWidth:900,display:'grid',gridTemplateColumns:'repeat(7,1fr)'}},
          days.map(function(d,di){ var kk=dk(d);
            var dayOrders=(orders||[]).filter(function(o){ return !o.assigned && (o.date===kk || (!o.date && di===0)); });
            var isT=kk===todayK;
            return ce('div',{key:di,style:{borderRight:di<6?'1px solid var(--border)':'none',minHeight:80}},
              ...
              ce('div',{style:{padding:'6px 8px',display:'grid',gap:4}},
                dayOrders.length?dayOrders.map(function(o,oi){
                  return ce('div',{key:oi,title:(o.from||'')+' → '+(o.to||''),style:{fontSize:10.5,lineHeight:1.25,padding:'3px 6px',borderRadius:5,
                    ...
                    ce('div',{style:{fontWeight:700,color:'var(--text)'}},o.client||o.cargo||'—'),
                    o.to?ce('div',{style:{color:'var(--text3)',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}},o.to):null); }):null,
                ce('button',{className:'fl-press',onClick:function(){ var v=window.prompt('Обратка на '+wd[d.getDay()]+' '+d.getDate()+' (что / откуда → куда):',''); if(v&&v.trim()&&onAddOrder) onAddOrder({client:v.trim(),date:kk}); },
                  style:{fontSize:10,padding:'3px 6px',borderRadius:5,cursor:'pointer',fontFamily:'inherit',fontWeight:700,
                  border:'1px dashed color-mix(in srgb,var(--cyan) 40%,transparent)',background:'transparent',color:'var(--cyan)'}},'+ обратка'))); }))))
  );
}
...
function AppMain() {
  ...
  return ce('div',{style:{minHeight:'100vh',color:'var(--text)'}},
    ...
    tab==='grid'&&ce(PlanGrid,{plans:plans,clients:clients,orders:orders,trucks:trucks,geoIndex:geoIndex,role:role,logists:logists,isBoss:isBoss,
