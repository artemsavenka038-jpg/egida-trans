...
function PlanGrid({plans,clients,orders,trucks,geoIndex,onSavePlan,onDelPlan,onLaunch}){
  ...
  const driverRows=useMemo(function(){
    ...
    (plans||[]).forEach(function(p){
      var kk=p.loadDate||p.date; if(weekKeys.indexOf(kk)<0) return;
  ...
  const barFor=function(row){
    ...
    var startIdx=weekKeys.indexOf(first.loadDate||first.date); if(startIdx<0) startIdx=0;
    // конец: день последнего плана; если у последнего есть обратка — ещё +1 день
    var lastIdx=weekKeys.indexOf(last.loadDate||last.date); if(lastIdx<0) lastIdx=startIdx;
    var endIdx=lastIdx;
    // есть ли обратка хоть в одном плане строки
    var anyBack=row.plans.some(function(p){ return !!p.back; });
    // реальная дата освобождения — из плана freeAt или из машины
    ...
    if(freeK){ var fi=weekKeys.indexOf(freeK); if(fi>=0 && fi>endIdx) endIdx=fi;
      else if(fi<0 && freeK>weekKeys[6]) endIdx=6; }  // освобождение за пределами недели — тянем до конца
    ...
    // индекс дня, где начинается обратка (для двухцветной полосы и разворота фуры)
