...
function countTrips(name, sinceISO, history, plans){
  // Считаем ТОЛЬКО запущенные планы (нажали «в рейс») и ТОЛЬКО с даты замены путевого.
  // Удалили план — рейс перестаёт считаться. Журнал не используем: он не чистится при удалении.
  var since = sinceISO ? String(sinceISO).slice(0,10) : '';
  var n=0, seen={};
  (plans||[]).forEach(function(p){
    if(!p || !p.launched) return;
    if(!sameDriver(p.truckDriver,name)) return;
    if(!String(p.clientName||'').trim()) return;   // строка только с обраткой — это НЕ рейс, не считаем
    var d=String(p.loadDate||p.date||'').slice(0,10);
    if(since){ if(!d) return; if(d<since) return; }   // без даты или раньше замены — не считаем
    var key=String(p.id||('d'+d)); if(seen[key]) return; seen[key]=1; n++;
  });
  return n;
}
function sameDriver(a,b){ a=String(a||'').trim().toLowerCase(); b=String(b||'').trim().toLowerCase(); if(!a||!b) return false; return a===b || a.indexOf(b)>=0 || b.indexOf(a)>=0; }
...
/* Собственный пробег прицепа — непрерывный при пересадках между тягачами.
   accum   — накоплено до текущей сцепки
   attachBase — одометр тягача в момент прицепки
   Если полей нет (старые карточки) — работаем по-старому: пробег = одометр тягача. */
function trailerMileage(tr,tractors){
  if(!tr) return null;
  var pt=(tractors||[]).find(function(x){ return x.plate===tr.pairedTractor; });
  var odo=pt?Number(pt.odometer):NaN;
  var hasBase=(tr.attachBase!=null&&tr.attachBase!=='');
  var accum=Number(tr.accum);
  if(hasBase&&isFinite(Number(tr.attachBase))){
    if(!isFinite(odo)) return isFinite(accum)?accum:null;   // тягач без пробега — показываем накопленное
    var delta=odo-Number(tr.attachBase);
    if(delta<0) delta=0;
    return (isFinite(accum)?accum:0)+delta;
  }
  return isFinite(odo)?odo:null;                             // legacy
}
function TrailerSwapModal({tr,tractors,onSwap,onClose}){
  const cur=trailerMileage(tr,tractors);
  const [to,setTo]=useState('');
  const [mil,setMil]=useState(cur!=null?String(Math.round(cur)):'');
  const nt=(tractors||[]).find(function(x){ return x.plate===to; });
  const ntOdo=nt?Number(nt.odometer):NaN;
  const bad=!to||!isFinite(ntOdo);
  const line=(k,v,c)=>ce('div',{style:{display:'flex',gap:10,padding:'6px 0',fontSize:12.5}},
    ce('span',{style:{color:'var(--text3)',minWidth:190,flexShrink:0}},k),
    ce('span',{style:{fontWeight:700,color:c||'var(--text)'}},v));
  return ce(Modal,{title:'🔄 Пересадка прицепа '+(tr.plate||''),onClose,wide:true},
    ce('div',{style:{fontSize:12,color:'var(--text3)',marginBottom:12,lineHeight:1.6}},
      'Прицеп переходит под другой тягач. Собственный пробег прицепа сохранится и продолжит считаться от нового тягача — ТО и шины не собьются.'),
    ce('div',{style:{background:'var(--bg3)',border:'1px solid var(--border)',borderRadius:11,padding:'10px 13px',marginBottom:12}},
      line('Сейчас под тягачом', tr.pairedTractor||'— не привязан —'),
      ce('div',{style:{display:'flex',gap:10,alignItems:'center',padding:'6px 0',flexWrap:'wrap'}},
        ce('span',{style:{color:'var(--text3)',minWidth:190,fontSize:12.5,flexShrink:0}},'Пробег прицепа, км'),
        ce('input',{value:mil,onChange:function(e){setMil(e.target.value.replace(/[^0-9]/g,''));},inputMode:'numeric',
          style:{width:150,background:'var(--bg2)',border:'1px solid var(--border2)',borderRadius:9,padding:'8px 10px',color:'var(--text)',fontSize:14,fontFamily:'inherit',fontWeight:700}}),
        ce('span',{style:{fontSize:11,color:'var(--text3)'}},'можно поправить, если сбился'))),
    fld('Новый тягач',ce('select',{value:to,onChange:function(e){setTo(e.target.value);},style:mecInp},
      ce('option',{value:''},'— выберите тягач —'),
      (tractors||[]).filter(function(x){ return x.plate!==tr.pairedTractor; })
        .map(function(x){ return ce('option',{key:x.id,value:x.plate},(x.plate||'—')+(x.odometer?(' · '+Number(x.odometer).toLocaleString('ru')+' км'):' · без пробега')); }))),
    to&&!isFinite(ntOdo)?ce('div',{style:{marginTop:10,padding:'9px 12px',borderRadius:10,fontSize:12,lineHeight:1.5,
      background:'color-mix(in srgb,var(--amber) 12%,transparent)',border:'1px solid color-mix(in srgb,var(--amber) 40%,transparent)',color:'var(--amber)'}},
      '⚠ У этого тягача не заполнен пробег. Сначала внесите его — иначе счётчик прицепа встанет.'):null,
    to&&isFinite(ntOdo)?ce('div',{style:{marginTop:10,padding:'10px 13px',borderRadius:10,
      background:'color-mix(in srgb,var(--green) 9%,transparent)',border:'1px solid color-mix(in srgb,var(--green) 32%,transparent)'}},
      line('После пересадки пробег', (Number(mil)||0).toLocaleString('ru')+' км','var(--green)'),
      line('Считать от одометра', Number(ntOdo).toLocaleString('ru')+' км')):null,
    ce('div',{style:{display:'flex',gap:9,marginTop:16}},
      ce(Btn,{onClick:onClose,color:'var(--text3)',wide:true},'Отмена'),
      ce('button',{onClick:function(){ if(!bad) onSwap(tr,to,ntOdo,Number(mil)||0); },disabled:bad,className:'fl-press',
        style:{flex:1,padding:'12px',border:'none',borderRadius:11,color:'#fff',fontSize:14,fontWeight:800,cursor:bad?'default':'pointer',
        background:bad?'var(--bg3)':'linear-gradient(135deg,var(--accent),var(--accent2))',opacity:bad?0.5:1}},'Пересадить')));
}
/* Пересадка сцепки: переставляет прицеп между машинами и чинит все связанные данные разом */
function findMecTrailer(list,plateNorm){
  if(!plateNorm) return null;
  var byN=(list||[]).find(function(x){ return capNorm(x.plate)===plateNorm; });
  if(byN) return byN;
  var d=(plateNorm.match(/\d{4}/)||[])[0];
  if(!d) return null;
  return (list||[]).find(function(x){ return (String(x.plate||'').match(/\d{4}/)||[])[0]===d; })||null;
}
function findMecTractor(list,plateNorm){
  if(!plateNorm) return null;
  var byN=(list||[]).find(function(x){ return capNorm(x.plate)===plateNorm; });
  if(byN) return byN;
  var d=(plateNorm.match(/\d{4}/)||[])[0];
...
function RigSwapModal({trucks,mecTrailers,mecTractors,onApply,onClose}){
  const [srcId,setSrcId]=useState('');
  const [dstId,setDstId]=useState('');
  const [toService,setToService]=useState(false);
  const own=(trucks||[]).filter(function(t){ return !t.isHired&&t.plate&&!isSpareDriver(t); });
  const src=own.find(function(t){ return String(t.id)===String(srcId); });
  const dst=own.find(function(t){ return String(t.id)===String(dstId); });
  const part=function(t){ var p=String((t&&t.plate)||'').split('/'); return {tr:(p[0]||'').trim(),tl:(p.length>1?p[1]:'').trim()}; };
  const sp=part(src), dp=part(dst);
  const mk=function(a,b){ return b?(a+' / '+b):a; };
  const newSrc=src?mk(sp.tr,dp.tl):'';
  const newDst=dst?mk(dp.tr,sp.tl):'';
  const bad=!src||!dst||String(srcId)===String(dstId)||!sp.tl;
  const opt=function(t){ return ce('option',{key:t.id,value:t.id},(t.plate||'—')+(t.driver?(' · '+t.driver):'')); };
  const arrow=function(label,was,now){ return ce('div',{style:{padding:'8px 0',borderBottom:'1px solid var(--border)'}},
    ce('div',{style:{fontSize:10.5,color:'var(--text3)',fontWeight:700,marginBottom:3}},label),
    ce('div',{style:{fontSize:12.5,color:'var(--text3)'}},was),
    ce('div',{style:{fontSize:13,fontWeight:800,color:'var(--green)'}},'→ '+now)); };
  return ce(Modal,{title:'🔄 Пересадка сцепки',onClose,wide:true},
    ce('div',{style:{fontSize:12,color:'var(--text3)',marginBottom:12,lineHeight:1.6}},
      'Прицеп переставляется с одной машины на другую. Приложение само перепишет гос.номера в парке, перенесёт пробег прицепа у механика (ТО и шины не собьются) и запишет событие в журнал.'),
    ce('div',{style:{display:'grid',gap:10,marginBottom:12}},
      fld('Откуда снимаем прицеп',ce('select',{value:srcId,onChange:function(e){setSrcId(e.target.value);},style:mecInp},
        ce('option',{value:''},'— выберите машину —'), own.map(opt))),
      fld('На какую машину ставим',ce('select',{value:dstId,onChange:function(e){setDstId(e.target.value);},style:mecInp},
        ce('option',{value:''},'— выберите машину —'), own.filter(function(t){return String(t.id)!==String(srcId);}).map(opt)))),
    (src&&!sp.tl)?ce('div',{style:{padding:'9px 12px',borderRadius:10,marginBottom:10,fontSize:12,lineHeight:1.5,
      background:'color-mix(in srgb,var(--amber) 12%,transparent)',border:'1px solid color-mix(in srgb,var(--amber) 40%,transparent)',color:'var(--amber)'}},
      '⚠ У этой машины в гос.номере не указан прицеп (нужен формат «ВВ 2700-7 / А6904 I-7»). Впишите прицеп в карточке машины.'):null,
    (src&&dst&&sp.tl)?ce('div',{style:{background:'var(--bg3)',border:'1px solid var(--border)',borderRadius:11,padding:'6px 13px',marginBottom:12}},
      arrow('МАШИНА '+(src.driver||''),src.plate,newSrc),
      arrow('МАШИНА '+(dst.driver||''),dst.plate,newDst),
      (function(){ var mt=findMecTrailer(mecTrailers,capNorm(sp.tl));
        if(!mt) return ce('div',{style:{padding:'8px 0',fontSize:11.5,color:'var(--text3)'}},'У механика карточки этого прицепа нет — пробег переносить нечего.');
        var cur=trailerMileage(mt,mecTractors);
        var ntr=findMecTractor(mecTractors,capNorm(dp.tr));
        var nodo=ntr?Number(ntr.odometer):NaN;
        return ce('div',{style:{padding:'8px 0'}},
          ce('div',{style:{fontSize:10.5,color:'var(--text3)',fontWeight:700,marginBottom:3}},'ПРОБЕГ ПРИЦЕПА '+(mt.plate||'')),
          ce('div',{style:{fontSize:13,fontWeight:800,color:cur!=null?'var(--green)':'var(--amber)'}},
            cur!=null?(Math.round(cur).toLocaleString('ru')+' км — сохранится'):'нет данных'),
          !ntr?ce('div',{style:{fontSize:11.5,color:'var(--amber)',marginTop:3}},'⚠ У механика нет карточки тягача '+dp.tr+' — привязку сделайте вручную.'):
          (!isFinite(nodo)?ce('div',{style:{fontSize:11.5,color:'var(--amber)',marginTop:3}},'⚠ У тягача '+dp.tr+' не заполнен пробег — счётчик прицепа встанет.'):null)); })()):null,
    src?ce('label',{style:{display:'flex',alignItems:'center',gap:8,fontSize:12.5,color:'var(--text2)',cursor:'pointer',marginBottom:4}},
      ce('input',{type:'checkbox',checked:toService,onChange:function(e){setToService(e.target.checked);}}),
      'Отправить '+(sp.tr||'машину')+' на сервис'):null,
    ce('div',{style:{display:'flex',gap:9,marginTop:16}},
      ce(Btn,{onClick:onClose,color:'var(--text3)',wide:true},'Отмена'),
      ce('button',{onClick:function(){ if(!bad) onApply({src:src,dst:dst,newSrc:newSrc,newDst:newDst,movedTrailer:sp.tl,backTrailer:dp.tl,toService:toService}); },
        disabled:bad,className:'fl-press',
        style:{flex:1,padding:'12px',border:'none',borderRadius:11,color:'#fff',fontSize:14,fontWeight:800,cursor:bad?'default':'pointer',
        background:bad?'var(--bg3)':'linear-gradient(135deg,var(--accent),var(--accent2))',opacity:bad?0.5:1}},'Пересадить')));
}
...
// ── Status keys ───────────────────────────────────────────────
const MOVING = ['FORWARD','LOADED','BACK','BACK_MOVING','AT_LOAD','ARRIVED_LOAD','LOADING','AT_UNLOAD','ARRIVED_UNLOAD','UNLOADED','BACK_GO','BACK_AT','BACK_LOADING','BACK_AT_UNLOAD'];
// ── Roles / logists ───────────────────────────────────────────
...
function normLogists(x){ var a = Array.isArray(x)?x:(x&&typeof x==='object'?Object.keys(x).map(function(k){return x[k];}):null); if(!a) return null; a=a.filter(function(l){return l&&l.name;}).map(function(l,i){ return {name:String(l.name),color:l.color||LOGIST_PALETTE[i%LOGIST_PALETTE.length],seeAll:!!l.seeAll}; }); return a; }
...
const SC = { FREE:'var(--green)',FORWARD:'var(--accent)',LOADED:'var(--green)',BACK:'var(--cyan)',
  ...
  AT_UNLOAD:'var(--violet)',ARRIVED_UNLOAD:'var(--violet)',UNLOADED:'var(--green)',BACK_GO:'var(--cyan)',BACK_MOVING:'var(--cyan)',
  BACK_AT:'var(--cyan)',BACK_LOADING:'var(--cyan)',BACK_AT_UNLOAD:'var(--cyan)',
...
const SCHEX = { FREE:'#34d399',FORWARD:'#4f8cff',LOADED:'#34d399',BACK:'#22d3ee',
  ...
  AT_UNLOAD:'#a78bfa',ARRIVED_UNLOAD:'#a78bfa',UNLOADED:'#34d399',BACK_GO:'#22d3ee',BACK_MOVING:'#22d3ee',
...
const STR = {
  ru:{
    ...
    tab_park:'Парк', tab_moving:'Рейсы', tab_clients:'Клиенты', tab_orders:'Обратки', tab_plan:'Планирование', tab_log:'Журнал',
    ...
    orders_title:'Обратки', add_order:'+ Обратка', ord_need_truck:'Нужна машина', ord_assigned:'Назначена',
    ...
    mode_order:'Из обратки', mode_manual:'Вручную', no_avail_orders:'Нет свободных обраток. Добавьте их во вкладке «Обратки».',
    ...
    s_BACK_MOVING:'Выехал на выгрузку обратки', s_BACK_MOVING_d:'Обратка в машине, еду на выгрузку',
    s_BACK_AT:'Приехал на обратку', s_BACK_LOADING:'Начали грузить обратку', s_BACK_AT_UNLOAD:'Приехал на выгрузку обратки',
    ...
    t_order_upd:'✓ Обратка обновлена', t_order_add:'✓ Обратка добавлена', t_truck_add:'✓ Машина добавлена',
    ...
    st_BACK_GO:'За обраткой', st_BACK_MOVING:'Везу обратку', st_BACK_AT:'На обратке', st_BACK_LOADING:'Грузит обратку', st_BACK_AT_UNLOAD:'На выгрузке обратки', st_SERVICE:'Сервис', st_WASH:'Мойка', st_DOCS:'Путевой',
  ...
  en:{
    ...
    st_BACK_GO:'For return', st_BACK_MOVING:'Return in transit', st_BACK_AT:'At return point', st_BACK_LOADING:'Loading return', st_BACK_AT_UNLOAD:'At return unload', st_SERVICE:'Service', st_WASH:'Wash', st_DOCS:'Waybill',
...
function BackModal({truck,orders,clients,onSave,onClose}) {
  ...
  const [freeExact,setFreeExact] = useState('');
  const [oq,setOq] = useState('');
  const [btemp,setBtemp] = useState((truck.backCargo&&truck.backCargo.temp)||'');
  ...
  const save = ()=>{
    const freeAt = freeExact ? new Date(freeExact).toISOString() : addH(dep, parseInt(freeH)||20);
    onSave({status:'FORWARD', backCargo:{cargo,from,to,departAt:dep,freeAt,temp:(btemp||'').trim(),
      fromGeo:(selectedOrder&&selectedOrder.fromGeo)||null, toGeo:(selectedOrder&&selectedOrder.toGeo)||null,
  ...
  return ce(Modal,{title:T('m_back_title')+' — '+truck.plate,onClose,wide:true},
    ...
    ce('div',{style:{marginTop:mode==='order'?12:0}},
      ...
      ce(Field,{label:'🌡 Температура обратки (пусто = без режима)'},
        ce(Input,{value:btemp,onChange:setBtemp,placeholder:'напр. 0/+2 или -18'})),
      ce(Field,{label:T('travel_h')},
        ce('div',{style:{display:'flex',gap:6}},
          ['10','15','20','24','30'].map(h=>ce('button',{key:h,className:'fl-press',onClick:()=>{ setFreeH(h); setFreeExact(''); },
            style:{flex:1,textAlign:'center',padding:'8px',borderRadius:9,fontSize:12.5,fontWeight:700,cursor:'pointer',
              background:(freeH===h&&!freeExact)?'color-mix(in srgb,var(--accent) 18%,transparent)':'var(--bg3)',
              border:`1px solid ${(freeH===h&&!freeExact)?'var(--accent)':'var(--border)'}`,
              color:(freeH===h&&!freeExact)?'var(--accent)':'var(--text3)'}},h+' ч')))),
      ce(Field,{label:'🕒 Или точное время освобождения'},
        ce('input',{type:'datetime-local',value:freeExact,onChange:e=>setFreeExact(e.target.value),
          style:{width:'100%',background:'var(--bg3)',border:'1px solid '+(freeExact?'var(--cyan)':'var(--border2)'),borderRadius:10,
            padding:'10px 12px',color:'var(--text)',fontSize:14,fontFamily:'inherit'}})),
      freeExact?ce('div',{style:{fontSize:11.5,color:'var(--cyan)',marginTop:-4,marginBottom:6,fontWeight:700}},'Освободится '+fmtDT(freeExact)+' — карточка станет свободной автоматически.'):null,
...
function OrderModal({clients,onSave,onClose,maxId,editOrder}) {
  ...
  const geoToStr=function(g){ return (g&&typeof g==='object'&&g.lat!=null)?(g.lat+', '+g.lon):(g||''); };
  const [fromGeo,setFromGeo] = useState(geoToStr(e0&&e0.fromGeo));
  const [toGeo,setToGeo] = useState(geoToStr(e0&&e0.toGeo));
  const cl = clients.find(c=>c.id===cid)||clients[0];

  useEffect(()=>{
    if(!e0 && mode==='existing' && cl){
      setFromAddr(cl.loadAddr); setToAddr((cl.unloadOptions&&cl.unloadOptions[0]&&cl.unloadOptions[0].addr)||'');
      if(cl.loadGeo) setFromGeo(typeof cl.loadGeo==='object'?(cl.loadGeo.lat+', '+cl.loadGeo.lon):cl.loadGeo);
      if(cl.unloadGeo) setToGeo(typeof cl.unloadGeo==='object'?(cl.unloadGeo.lat+', '+cl.unloadGeo.lon):cl.unloadGeo);
  ...
  const save = ()=>{
    ...
    const base={client:clientName,cid:finalCid,cargo,cnt:parseInt(cnt)||1,from:fromAddr,to:toAddr,
      fromGeo:normGeo(fromGeo),toGeo:normGeo(toGeo),
  ...
  return ce(Modal,{title:e0?T('m_order_edit'):T('m_order_new'),onClose,wide:true},
    ...
    ce('div',{style:{background:'color-mix(in srgb,var(--cyan) 6%,transparent)',border:'1px solid color-mix(in srgb,var(--cyan) 25%,transparent)',borderRadius:11,padding:'10px 12px'}},
      ce('div',{style:{fontSize:11,color:'var(--cyan)',fontWeight:800,marginBottom:7}},'📍 Координаты (для автостатусов) — из Google Maps'),
      ce('div',{style:{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}},
        ce(Field,{label:'Загрузка'},ce(Input,{value:fromGeo,onChange:setFromGeo,placeholder:'54.05, 28.20'})),
        ce(Field,{label:'Выгрузка'},ce(Input,{value:toGeo,onChange:setToGeo,placeholder:'55.68, 37.72'})))),
...
function EditClientModal({client,onSave,onDelete,onClose}) {
  ...
  const geoToStr=function(g){ return (g&&typeof g==='object'&&g.lat!=null)?(g.lat+', '+g.lon):(g||''); };
  const [loadGeo,setLoadGeo] = useState(geoToStr(client.loadGeo));
  const [unloadGeo,setUnloadGeo] = useState(geoToStr(client.unloadGeo));
  ...
  const save = ()=> onSave(Object.assign({},client,{name,color,loadCity,loadAddr,
    ...
    loadGeo:normGeo(loadGeo),unloadGeo:normGeo(unloadGeo),
  ...
  return ce(Modal,{title:T('m_client_edit')+' — '+client.name,onClose,wide:true},
    ...
    ce('div',{style:{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}},
      ...
      ce(Field,{label:T('temp_mode')},ce(Input,{value:temp,onChange:setTemp,placeholder:'+4'}))),
    ce('div',{style:{background:'color-mix(in srgb,var(--cyan) 6%,transparent)',border:'1px solid color-mix(in srgb,var(--cyan) 25%,transparent)',borderRadius:11,padding:'11px 13px',marginBottom:2}},
      ce('div',{style:{fontSize:11.5,color:'var(--cyan)',fontWeight:800,marginBottom:3}},'📍 Координаты складов (для автостатусов)'),
      ce('div',{style:{fontSize:11,color:'var(--text3)',lineHeight:1.5,marginBottom:9}},'В Google Maps правый клик по складу → скопируйте числа (напр. 54.0482, 28.2055) и вставьте сюда. Тогда система сама определит, что машина «на загрузке» / «на выгрузке».'),
      ce('div',{style:{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}},
        ce(Field,{label:'Координаты загрузки'},ce(Input,{value:loadGeo,onChange:setLoadGeo,placeholder:'54.0482, 28.2055'})),
        ce(Field,{label:'Координаты выгрузки'},ce(Input,{value:unloadGeo,onChange:setUnloadGeo,placeholder:'55.68, 37.72'}))),
      (loadGeo&&!normGeo(loadGeo))?ce('div',{style:{fontSize:11,color:'var(--amber)',marginTop:5,fontWeight:700}},'⚠ Загрузка: не разобрал координаты. Формат: 54.0482, 28.2055'):null,
      (unloadGeo&&!normGeo(unloadGeo))?ce('div',{style:{fontSize:11,color:'var(--amber)',marginTop:3,fontWeight:700}},'⚠ Выгрузка: не разобрал координаты. Формат: 55.68, 37.72'):null),
    ce(Field,{label:'📝 '+T('note_for_drivers')},ce(Textarea,{value:note,onChange:setNote,placeholder:T('note_ph'),rows:4})),
    onDelete?ce('button',{onClick:function(){ if(window.confirm('Удалить клиента «'+client.name+'»? Это не затронет уже созданные рейсы, но клиент исчезнет из списка выбора.')) onDelete(client.id); },className:'fl-press',
      style:{width:'100%',padding:'11px',marginTop:6,marginBottom:2,borderRadius:11,cursor:'pointer',fontFamily:'inherit',fontSize:13,fontWeight:800,
      border:'1px solid color-mix(in srgb,var(--red) 45%,transparent)',background:'color-mix(in srgb,var(--red) 8%,transparent)',color:'var(--red)'}},'🗑 Удалить клиента'):null,
    ce('div',{style:{display:'flex',gap:9,marginTop:4}},
      ce(Btn,{onClick:onClose,color:'var(--text3)',wide:true},T('btn_cancel')),
      ce('button',{onClick:save,className:'fl-press',style:{flex:1,padding:'11px',
        background:'linear-gradient(135deg,var(--accent),var(--accent2))',border:'none',
...
function DriverPage({truck,clients,orders,onUpdate,onClose}) {
  ...
  const sendGeo = ()=>{
    setGeoMsg('');
    if(typeof navigator==='undefined'||!navigator.geolocation){ setGeoMsg('Телефон не поддерживает геолокацию.'); return; }
    setGeoBusy(true);
    navigator.geolocation.getCurrentPosition(function(pos){
      var lat=pos.coords.latitude, lon=pos.coords.longitude;
      var save=function(addr){
        var loc={lat:lat,lon:lon,addr:addr||'',at:nowLocalISO()};
        onUpdate({driverLoc:loc});
        var _id=String(Date.now())+Math.random().toString(36).slice(2,6); var _o={};
        _o[_id]={ts:nowLocalISO(),plate:truck.plate||'',driver:truck.driver||'',logist:truck.logist||'',action:'geo',label:'📍 '+(addr||(lat.toFixed(4)+', '+lon.toFixed(4)))};
        dbPatch('/history',_o);
        setGeoBusy(false); setGeoMsg('Отправлено ✓'+(addr?(' — '+addr):''));
      };
      // обратное геокодирование: координаты -> адрес (работает на живом сайте)
      try{
        fetch('https://nominatim.openstreetmap.org/reverse?format=json&lat='+lat+'&lon='+lon+'&accept-language=ru&zoom=18')
          .then(function(r){ return r.json(); })
          .then(function(d){ save((d&&d.display_name)?d.display_name:''); })
          .catch(function(){ save(''); });
      }catch(e){ save(''); }
    },function(err){
      setGeoBusy(false);
      setGeoMsg(err&&err.code===1?'Доступ к геолокации запрещён. Разрешите в настройках браузера.':'Не удалось определить местоположение.');
  ...
  return ce('div',{style:{position:'fixed',inset:0,background:'var(--bg)',zIndex:600,display:'flex',flexDirection:'column',overflowY:'auto'}},
    ...
    ce('div',{style:{padding:'14px 16px',maxWidth:560,margin:'0 auto',width:'100%'}},
      ce('div',{style:{marginBottom:18,padding:'14px',borderRadius:14,background:'color-mix(in srgb,var(--cyan) 8%,transparent)',border:'1px solid color-mix(in srgb,var(--cyan) 30%,transparent)'}},
        ce('div',{style:{fontSize:10,color:'var(--cyan)',fontWeight:800,letterSpacing:'1.4px',textTransform:'uppercase',marginBottom:8}},'📍 Где я на 08:00'),
        ce('div',{style:{fontSize:12.5,color:'var(--text3)',lineHeight:1.5,marginBottom:11}},'Утром отметьте, где находитесь. Нажмите кнопку — телефон отправит ваше местоположение диспетчеру.'),
        truck.driverLoc&&truck.driverLoc.addr?ce('div',{style:{fontSize:13,color:'var(--text)',fontWeight:600,marginBottom:10,padding:'9px 11px',background:'var(--bg2)',borderRadius:10,lineHeight:1.4}},'📍 '+truck.driverLoc.addr):null,
        ce('button',{className:'fl-press',onClick:sendGeo,disabled:geoBusy,style:{width:'100%',padding:'14px',borderRadius:12,border:'none',cursor:geoBusy?'default':'pointer',fontFamily:'inherit',fontSize:15,fontWeight:800,color:'#fff',
          background:'linear-gradient(135deg,var(--cyan),var(--accent))',opacity:geoBusy?0.6:1}},geoBusy?'Определяю…':'📍 Отправить моё местоположение'),
        geoMsg?ce('div',{style:{marginTop:9,fontSize:12.5,fontWeight:700,color:geoMsg.indexOf('✓')>=0?'var(--green)':'var(--amber)',lineHeight:1.4}},geoMsg):null),
...
// ═══════════ СВОДКА ВЫГРУЗОК ═══════════
function unloadPoints(trucks){
  // финальная выгрузка машины: если назначена обратка — это выгрузка обратки,
  // иначе выгрузка рейса «туда». Ориентир по времени — freeAt (освобождение).
  var pts=[];
  (trucks||[]).forEach(function(t){
    if(!MOVING.includes(t.status)) return;
    var bc=t.backCargo;
    if(bc&&(bc.to||bc.freeAt)){
      // едет с обраткой — конечная выгрузка там, освободится по freeAt обратки
      pts.push({truck:t,plate:t.plate,driver:t.driver,when:bc.freeAt||t.freeAt,
        addr:bc.to||t.unloadAddr||'',client:bc.cargo||'обратный груз',freeAt:t.freeAt,back:true});
      return;
    }
    var trips=(t.trips||[]).filter(function(x){ return x&&x.dir==='forward'&&x.arriveUnload; });
    if(trips.length){
      var last=trips[trips.length-1];
      // если у рейса есть freeAt позже прибытия — значит машина ещё занята (везёт/ждёт), берём freeAt
      var when=(t.freeAt && new Date(t.freeAt).getTime()>new Date(last.arriveUnload).getTime())?t.freeAt:last.arriveUnload;
      pts.push({truck:t,plate:t.plate,driver:t.driver,when:when,
        addr:last.to||t.unloadAddr||'',client:last.clientName||t.clientName||'',freeAt:t.freeAt});
    } else if(t.freeAt){
      pts.push({truck:t,plate:t.plate,driver:t.driver,when:t.freeAt,
        addr:t.unloadAddr||'',client:t.clientName||'',freeAt:t.freeAt});
    }
  });
  return pts.filter(function(p){ var d=new Date(p.when); return !isNaN(d.getTime()); })
    .sort(function(a,b){ return new Date(a.when)-new Date(b.when); });
}
function UnloadSummary({trucks,onEngage}){
  const [open,setOpen]=useState(true);
  const [day,setDay]=useState(0); // 0=сегодня, 1=завтра...
  const [engaged,setEngaged]=useState({}); // задействованные — скрыты из списка
  const pts=unloadPoints(trucks).filter(function(p){ return !engaged[p.truck.id]; });
  const base=new Date(); base.setHours(0,0,0,0);
  const dayStart=new Date(base.getTime()+day*864e5).getTime();
  const dayEnd=dayStart+864e5;
  const wd=['Вс','Пн','Вт','Ср','Чт','Пт','Сб'];
  const dstr=function(off){ var d=new Date(base.getTime()+off*864e5); return (off===0?'Сегодня':(off===1?'Завтра':wd[d.getDay()]))+' '+String(d.getDate()).padStart(2,'0')+'.'+String(d.getMonth()+1).padStart(2,'0'); };
  const forDay=pts.filter(function(p){ var t=new Date(p.when).getTime(); return t>=dayStart&&t<dayEnd; });
  const counts=[0,1,2,3,4,5,6].map(function(o){ var s=base.getTime()+o*864e5,e=s+864e5; return pts.filter(function(p){ var t=new Date(p.when).getTime(); return t>=s&&t<e; }).length; });
  return ce('div',{style:{background:'var(--bg2)',border:'1px solid var(--border)',borderRadius:14,marginBottom:14,overflow:'hidden'}},
    ce('button',{className:'fl-press',onClick:function(){setOpen(!open);},style:{width:'100%',display:'flex',alignItems:'center',gap:9,padding:'12px 15px',
      background:'transparent',border:'none',cursor:'pointer',fontFamily:'inherit',color:'var(--text)',fontSize:14,fontWeight:800,textAlign:'left'}},
      ce('span',null,'📦'),'Выгрузки по дням',
      ce('span',{style:{padding:'1px 9px',borderRadius:999,background:'color-mix(in srgb,var(--violet) 20%,transparent)',color:'var(--violet)',fontSize:12,fontWeight:800}},String(counts[0])+' сегодня'),
      ce('span',{style:{marginLeft:'auto',color:'var(--text3)',fontSize:12,fontWeight:600}},open?'скрыть ▲':'показать ▼')),
    open?ce('div',{style:{padding:'0 12px 12px'}},
      ce('div',{style:{display:'flex',gap:6,overflowX:'auto',paddingBottom:9,marginBottom:4,WebkitOverflowScrolling:'touch'}},
        [0,1,2,3,4,5,6].map(function(o){ var act=o===day;
          return ce('button',{key:o,className:'fl-press',onClick:function(){setDay(o);},style:{flexShrink:0,minWidth:74,padding:'7px 9px',borderRadius:10,cursor:'pointer',fontFamily:'inherit',textAlign:'center',
            border:'1px solid '+(act?'var(--violet)':'var(--border)'),background:act?'color-mix(in srgb,var(--violet) 15%,transparent)':'var(--bg3)',color:act?'var(--violet)':'var(--text3)'}},
            ce('div',{style:{fontSize:10.5,fontWeight:700}},dstr(o)),
            ce('div',{style:{fontSize:13,fontWeight:800,marginTop:2}},counts[o]?counts[o]+' авто':'—')); })),
      forDay.length===0?ce('div',{style:{textAlign:'center',padding:'20px',color:'var(--text3)',fontSize:13}},'В этот день выгрузок нет.'):
      ce('div',{style:{border:'1px solid var(--border)',borderRadius:11,overflow:'hidden'}},
        forDay.map(function(p,i){ var overdue=new Date(p.when).getTime()<Date.now();
          var stCol=SC[p.truck.status]||'var(--text3)';
          var _sl=T('s_'+p.truck.status); if(_sl==='s_'+p.truck.status) _sl=T('st_'+p.truck.status); if(_sl==='st_'+p.truck.status) _sl=p.truck.status;
          var stLabel=_sl;
          return ce('div',{key:i,style:{display:'flex',alignItems:'center',gap:10,padding:'10px 12px',flexWrap:'wrap',
            borderBottom:i<forDay.length-1?'1px solid var(--border)':'none',fontSize:12.5}},
            ce('div',{style:{minWidth:96,flexShrink:0}},
              ce('span',{style:{display:'inline-block',padding:'2px 8px',borderRadius:7,fontSize:10.5,fontWeight:800,whiteSpace:'nowrap',
                background:'color-mix(in srgb,'+stCol+' 16%,transparent)',color:stCol}},stLabel)),
            ce('div',{style:{minWidth:56,fontWeight:800,fontSize:14,color:overdue?'var(--green)':'var(--violet)'}},
              new Date(p.when).toLocaleTimeString('ru',{hour:'2-digit',minute:'2-digit'})),
            ce('div',{style:{minWidth:120}},
              ce('div',{style:{fontWeight:800}},p.plate||'—'),
              ce('div',{style:{fontSize:11,color:'var(--text3)',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis',maxWidth:150}},p.driver||'—')),
            ce('div',{style:{flex:1,minWidth:140,color:'var(--text3)'}},
              (p.client?('🏢 '+p.client):'')+(p.addr?('  📍 '+p.addr):'')),
            overdue?ce('span',{style:{fontSize:10.5,color:'var(--green)',fontWeight:700}},'уже выгрузился'):null,
            onEngage?ce('button',{className:'fl-press',onClick:function(){ setEngaged(function(prev){ var n=Object.assign({},prev); n[p.truck.id]=true; return n; }); onEngage(p.truck); },
              style:{padding:'6px 12px',borderRadius:8,cursor:'pointer',fontFamily:'inherit',fontSize:11.5,fontWeight:800,
              border:'1px solid color-mix(in srgb,var(--accent) 45%,transparent)',background:'color-mix(in srgb,var(--accent) 10%,transparent)',color:'var(--accent)'}},'Задействовать →'):null); }))):null);
}
// ═══════════ СВОДКА ВЫГРУЗОК КОНЕЦ ═══════════
function TruckCard({truck,clients,orders,settings,role,isBoss,onUpdate,onSwapRequest,onOpenDriver,onDelete,onUndoDispatch,geoIndex}) {
  const [modal,setModal] = useState(null);
  const [expanded,setExpanded] = useState(false);
  const sc = SC[truck.status]||'var(--text3)';
  const isMoving = MOVING.includes(truck.status);
  const autoFreed = isMoving && truck.freeAt && (new Date(truck.freeAt).getTime()<=Date.now());  // время вышло
  ...
  const bc = truck.backCargo;
  // автостатус по GPS (подсвечивает КАРТОЧКУ; статус водителя остаётся отдельной плашкой)
  var tgeo=null; if(geoIndex){ var _sp=splitFleteraPlate(truck.plate); tgeo=geoIndex[_sp.tractor]||null; }
  const autoSt = isMoving ? autoGeoStatus(truck,tgeo,clients,orders) : null;
  const whereNow = isMoving ? geoWhereNow(truck,tgeo,clients,orders) : null;
  // авто-определение направления по прохождению точки (когда водитель молчит про обратку)
  const autoDir = isMoving ? autoDirection(truck,tgeo,clients,orders) : null;
  const driverSaidBack = ['BACK','BACK_GO','BACK_MOVING'].indexOf(truck.status)>=0;
  useEffect(function(){
    if(!autoDir) return;
    if(autoDir.reached && !truck.reachedUnload){ onUpdate(truck.id,{reachedUnload:true}); }
  },[autoDir&&autoDir.reached]);
  const bord = (truck.problem||truck.status==='PROBLEM')?'var(--red)':truck.onService?'var(--orange)':truck.needService?'var(--amber)':
    (autoSt&&(autoSt.st==='at_load'||autoSt.st==='at_unload'||autoSt.st==='moving')?autoSt.color:'var(--border)');
  ...
  const setFree = ()=> upd({status:'FREE',clientId:'',clientName:'',loadAddr:'',unloadAddr:'',loadAt:'',freeAt:nowLocalISO(),backCargo:null,trips:[],reachedUnload:false});

  return ce('div',{className:'fl-card fl-row-hover',style:{
    background:'var(--bg2)',border:`1.5px solid ${bord}`,borderRadius:14,padding:'14px 15px',
    position:'relative',overflow:'hidden',boxShadow:(autoSt&&autoSt.st!=='stopped'&&autoSt.st!=='moving')?('0 0 0 1px '+bord+', var(--shadow)'):'var(--shadow)'}},
    ce('div',{style:{position:'absolute',top:0,left:0,right:0,height:3,background:sc}}),
    autoSt&&(autoSt.st==='at_load'||autoSt.st==='at_unload'||autoSt.st==='moving')?ce('div',{style:{display:'flex',alignItems:'center',gap:6,marginBottom:8,padding:'5px 10px',borderRadius:8,
      background:'color-mix(in srgb,'+autoSt.color+' 12%,transparent)',border:'1px solid color-mix(in srgb,'+autoSt.color+' 35%,transparent)'}},
      ce('span',{style:{fontSize:11.5,fontWeight:800,color:autoSt.color}},autoSt.label+(autoSt.distKm!=null&&autoSt.distKm>2?(' · '+autoSt.distKm+' км до цели'):'')),
      ce('span',{style:{fontSize:10,color:'var(--text3)',marginLeft:'auto'}},'по GPS'+(tgeo&&geoAgeMin(tgeo.at)>30?(' · '+(geoAgeMin(tgeo.at)<60?geoAgeMin(tgeo.at)+'м':Math.floor(geoAgeMin(tgeo.at)/60)+'ч')+' назад'):''))):null,
    (autoDir&&autoDir.goingBack&&!driverSaidBack)?ce('div',{style:{display:'flex',alignItems:'center',gap:7,marginBottom:8,padding:'7px 11px',borderRadius:9,fontSize:11.5,
      background:'color-mix(in srgb,var(--cyan) 10%,transparent)',border:'1px dashed color-mix(in srgb,var(--cyan) 45%,transparent)'}},
      ce('span',null,'↩'),
      ce('span',{style:{fontWeight:700,color:'var(--cyan)'}},'Похоже, едет назад'),
      ce('span',{style:{color:'var(--text3)'}},'— по GPS ушёл с выгрузки, водитель ещё не отметил')):null,
    whereNow?ce('div',{style:{display:'flex',alignItems:'center',gap:7,marginBottom:8,padding:'7px 11px',borderRadius:9,fontSize:12,
      background:'var(--bg3)',border:'1px solid var(--border)'}},
      ce('span',{style:{fontSize:13}},whereNow.moving?'🚗':'⏸'),
      ce('span',{style:{fontWeight:700,color:whereNow.moving?'var(--cyan)':'var(--text2)'}},whereNow.moving?('едет · '+whereNow.speed+' км/ч'):'стоит'),
      whereNow.distKm!=null?ce('span',{style:{color:'var(--text3)'}},'· '+(whereNow.goingBack?'обратно, до '+whereNow.targetName+' ':'до '+whereNow.targetName+' ')+whereNow.distKm+' км'+(whereNow.moving&&whereNow.etaH?(' (~'+(whereNow.etaH<1?Math.round(whereNow.etaH*60)+' мин':whereNow.etaH.toFixed(1)+' ч')+')'):'')):
        (whereNow.goingBack?ce('span',{style:{color:'var(--cyan)'}},'· ↩ везёт обратно'+(whereNow.targetName&&whereNow.targetName!=='обратно домой'?(' → '+whereNow.targetName):'')):null),
      whereNow.ageMin!=null&&whereNow.ageMin>30?ce('span',{style:{marginLeft:'auto',fontSize:10,color:'var(--amber)'}},(whereNow.ageMin<60?whereNow.ageMin+'м':Math.floor(whereNow.ageMin/60)+'ч')+' назад'):
        ce('a',{href:'https://www.google.com/maps?q='+whereNow.lat+','+whereNow.lon,target:'_blank',rel:'noopener',style:{marginLeft:'auto',fontSize:10,fontWeight:700,color:'var(--accent)',textDecoration:'none'}},'карта')):null,
    ...
    truck.driverLoc?ce('div',{style:{display:'flex',alignItems:'flex-start',gap:7,marginBottom:8,padding:'8px 11px',borderRadius:10,
      background:'color-mix(in srgb,var(--cyan) 9%,transparent)',border:'1px solid color-mix(in srgb,var(--cyan) 28%,transparent)'}},
      ce('span',{style:{fontSize:13,flexShrink:0}},'📍'),
      ce('div',{style:{minWidth:0,flex:1}},
        ce('div',{style:{fontSize:10,color:'var(--cyan)',fontWeight:800,textTransform:'uppercase',letterSpacing:'.4px'}},'На 08:00'+(truck.driverLoc.at?(' · '+fmtDT(truck.driverLoc.at)):'')),
        ce('div',{style:{fontSize:12.5,color:'var(--text)',fontWeight:600,lineHeight:1.4,marginTop:2}},truck.driverLoc.addr||'координаты получены')),
      (truck.driverLoc.lat!=null)?ce('a',{href:'https://www.google.com/maps?q='+truck.driverLoc.lat+','+truck.driverLoc.lon,target:'_blank',rel:'noopener',
        style:{flexShrink:0,fontSize:11,fontWeight:700,color:'var(--accent)',textDecoration:'none',padding:'4px 8px',borderRadius:7,border:'1px solid color-mix(in srgb,var(--accent) 35%,transparent)'}},'карта'):null):null,
    ...
    expanded&&ce('div',{style:{display:'flex',alignItems:'center',gap:7,marginBottom:8}},
    ...
    expanded&&ce('div',{onClick:()=>setModal('note'),style:{cursor:'pointer',marginBottom:8,padding:'9px 11px',borderRadius:10,
    ...
    (!expanded&&truck.note)?ce('div',{style:{fontSize:11.5,color:'var(--amber)',marginBottom:8,padding:'6px 10px',borderRadius:8,background:'color-mix(in srgb,var(--amber) 10%,transparent)',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}},'📝 '+truck.note):null,

    expanded&&ce('div',{style:{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'4px 10px',fontSize:12,marginBottom:8}},
    ...
    isMoving&&!autoFreed&&ce('div',{style:{display:'flex',alignItems:'center',gap:8,marginBottom:8,padding:'8px 11px',borderRadius:10,
    ...
    autoFreed&&ce('div',{style:{display:'flex',alignItems:'center',gap:8,marginBottom:8,padding:'8px 11px',borderRadius:10,flexWrap:'wrap',
      background:'color-mix(in srgb,var(--green) 12%,transparent)',border:'1px solid color-mix(in srgb,var(--green) 45%,transparent)'}},
      ce('span',{style:{fontSize:10.5,color:'var(--green)',fontWeight:800,textTransform:'uppercase',letterSpacing:'.4px'}},'✅ Уже свободна'),
      ce('span',{style:{fontSize:11.5,color:'var(--text3)'}},'время вышло '+(truck.freeAt?fmtDT(truck.freeAt):'')),
      ce('button',{className:'fl-press',onClick:function(){ upd({status:'FREE',clientId:'',clientName:'',loadAddr:'',unloadAddr:'',loadAt:'',freeAt:nowLocalISO(),backCargo:null,trips:[]}); },
        style:{marginLeft:'auto',padding:'6px 12px',borderRadius:8,border:'none',cursor:'pointer',fontFamily:'inherit',fontSize:11.5,fontWeight:800,color:'#fff',background:'var(--green)'}},'Подтвердить')),
    ...
    expanded&&ce('div',{style:{display:'flex',gap:14,marginBottom:8,padding:'7px 11px',background:'var(--bg3)',borderRadius:9,border:'1px solid var(--border)'}},
    ...
    expanded&&bc&&truck.backCargo&&ce('div',{style:{background:'color-mix(in srgb,var(--cyan) 8%,transparent)',
    ...
    ce('div',{style:{display:'flex',gap:6,flexWrap:'wrap'}},
      ...
      ce(Btn,{onClick:setFree,color:'var(--violet)',sm:true},T('btn_free')),
      expanded&&ce(Btn,{onClick:()=>setModal('back'),color:'var(--cyan)'},T('btn_return')),
      expanded&&ce(Btn,{onClick:()=>{if(onSwapRequest)onSwapRequest(truck);},color:'var(--amber)'},T('btn_swap')),
      expanded&&ce(Btn,{onClick:()=>setModal('edittruck'),color:'var(--violet)',sm:true},'✏️ '+T('cl_edit')),
      expanded&&ce(Btn,{onClick:()=>window.open(DRIVER_URL+'?pin='+(truck.pin||''),'_blank'),color:'var(--text3)',sm:true},'📱'),
      expanded&&isMoving&&onUndoDispatch&&ce(Btn,{onClick:()=>{ if(window.confirm('Отменить ошибочную отправку? У водителя '+(truck.driver||'')+' спишется 1 рейс в путевом. На саму машину это не влияет.')) onUndoDispatch(truck.id); },color:'var(--red)',sm:true},'↩ −1 путевой'),
      ce('button',{className:'fl-press',onClick:()=>setExpanded(!expanded),style:{marginLeft:'auto',padding:'6px 12px',borderRadius:8,cursor:'pointer',fontFamily:'inherit',fontSize:11.5,fontWeight:800,
        border:'1px solid var(--border2)',background:'var(--bg3)',color:'var(--text3)'}},expanded?'Свернуть ▲':'Подробнее ▾')),
...
function MovingCard({tr,clients,orders,settings,onUpdate,onModal,modal,setModal}) {
  ...
  return ce('div',{className:'fl-card fl-row-hover',style:{background:'var(--bg2)',
    ...
    ce('div',{style:{display:'flex',alignItems:'center',gap:8,marginBottom:8,padding:'8px 11px',borderRadius:10,
      background:'color-mix(in srgb,var(--accent) 10%,transparent)',border:'1px solid color-mix(in srgb,var(--accent) 30%,transparent)'}},
      ce('span',{style:{fontSize:10.5,color:'var(--accent)',fontWeight:800,textTransform:'uppercase',letterSpacing:'.4px'}},'🕓 Освободится'),
      ce('span',{style:{fontSize:15,color:'var(--text)',fontWeight:800}},!tr.backCargo?'не определено':(tr.freeAt?fmtDT(tr.freeAt):'—')),
      (tr.backCargo&&freeH!==null)?ce('span',{style:{marginLeft:'auto',fontSize:11.5,color:freeH<=4?'var(--green)':'var(--text3)',fontWeight:700}},freeH<=0?'сейчас':('~'+freeH+' ч')):null),
    ...
    ce('div',{style:{background:'color-mix(in srgb,var(--green) 8%,transparent)',border:'1px solid color-mix(in srgb,var(--green) 22%,transparent)',
      ...
      !tr.backCargo ? ce('span',{style:{color:'var(--amber)',fontWeight:700}},'Обратки нет — время освобождения определить нельзя.')
...
function LogTab({trucks,logDate,setLogDate}) {
  const entries = useMemo(()=>{
    ...
    // текущие рейсы (машина сейчас в рейсе, но ещё не в trips) — чтобы в журнале были и те, кто едет сейчас
    trucks.forEach(t=>{
      if(!MOVING.includes(t.status)) return;
      if(!t.clientName && !t.clientId) return;
      // не дублируем, если такой рейс уже есть в trips
      var already=(t.trips||[]).some(function(tr){ return tr.clientName===t.clientName && tr.loadAt===t.loadAt; });
      if(already) return;
      all.push({driver:t.driver,plate:t.plate,client:t.clientName||'',dir:'forward',
        from:t.loadAddr||'',to:t.unloadAddr||'',date:(t.loadAt||'').slice(0,10)||today(),loadAt:t.loadAt,freeAt:t.freeAt,wb:false,active:true});
    });
  ...
  const byClient = useMemo(()=>{ const m={}; entries.forEach(e=>{ if(e.dir==='back')return; var c=e.client||'—'; (m[c]=m[c]||[]).push(e);}); return m; },[entries]);
  const [mode,setMode]=useState('client');   // client | driver
  const [openClient,setOpenClient]=useState(null);

  const modeBtn=function(k,label){ var on=mode===k; return ce('button',{className:'fl-press',onClick:function(){setMode(k);setOpenClient(null);},
    style:{padding:'7px 15px',borderRadius:9,cursor:'pointer',fontFamily:'inherit',fontSize:12.5,fontWeight:800,
    border:'1px solid '+(on?'var(--accent)':'var(--border2)'),background:on?'color-mix(in srgb,var(--accent) 14%,transparent)':'var(--bg2)',color:on?'var(--accent)':'var(--text3)'}},label); };

  return ce('div',{style:{padding:'16px',maxWidth:1100,margin:'0 auto'}},
    ce('div',{style:{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:14,gap:10,flexWrap:'wrap'}},
      ce('h2',{style:{fontWeight:800,color:'var(--text)',margin:0,fontSize:20}},T('log_title')),
      ce('div',{style:{display:'flex',gap:7}},modeBtn('client','🏢 По клиентам'),modeBtn('driver','👤 По водителям'))),

    mode==='client'?(
      openClient?
      // рейсы выбранного клиента
      ce('div',null,
        ce('button',{className:'fl-press',onClick:function(){setOpenClient(null);},style:{marginBottom:12,padding:'6px 13px',borderRadius:8,cursor:'pointer',fontFamily:'inherit',fontSize:12.5,fontWeight:700,
          border:'1px solid var(--border2)',background:'var(--bg2)',color:'var(--text2)'}},'← Все клиенты'),
        ce('div',{className:'fl-card',style:{background:'var(--bg2)',border:'1px solid var(--border)',borderRadius:13,padding:'14px',boxShadow:'var(--shadow)'}},
          ce('div',{style:{fontWeight:800,fontSize:15,marginBottom:10,color:'var(--accent)'}},'🏢 '+openClient+' · '+(byClient[openClient]||[]).length+' рейс.'),
          (byClient[openClient]||[]).map(function(tr,i){ return ce('div',{key:i,style:{borderTop:'1px solid var(--border)',paddingTop:8,marginTop:8,display:'grid',gridTemplateColumns:'auto 1fr',gap:'2px 12px',fontSize:12.5}},
            ce('span',{style:{color:'var(--text3)',whiteSpace:'nowrap'}},fmtDT(tr.loadAt)),
            ce('div',null,
              ce('b',{style:{color:'var(--text)'}},'👤 '+(tr.driver||'—')),
              tr.active?ce('span',{style:{fontSize:10,fontWeight:800,marginLeft:6,padding:'1px 7px',borderRadius:5,background:'color-mix(in srgb,var(--accent) 16%,transparent)',color:'var(--accent)'}},'в рейсе'):null,
              ce('span',{style:{color:'var(--text3)'}},'  '+(tr.plate||'')),
              (tr.from||tr.to)?ce('div',{style:{color:'var(--text3)',fontSize:11.5,marginTop:1}},(tr.from||'')+' → '+(tr.to||'')):null)); }))):
      // список клиентов
      ce('div',null,
        ce('div',{style:{fontSize:12.5,color:'var(--text3)',marginBottom:12}},'Всего клиентов: '+Object.keys(byClient).length+'. Нажмите клиента — увидите все его рейсы.'),
        ce('div',{style:{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(240px,1fr))',gap:10}},
          Object.keys(byClient).sort(function(a,b){return byClient[b].length-byClient[a].length;}).map(function(c){
            return ce('button',{key:c,className:'fl-press',onClick:function(){setOpenClient(c);},style:{textAlign:'left',padding:'14px 15px',borderRadius:13,cursor:'pointer',fontFamily:'inherit',
              background:'var(--bg2)',border:'1px solid var(--border)',boxShadow:'var(--shadow)'}},
              ce('div',{style:{fontWeight:800,fontSize:14,color:'var(--text)',marginBottom:4}},'🏢 '+c),
              ce('div',{style:{fontSize:12.5,color:'var(--accent)',fontWeight:700}},byClient[c].length+' рейс.'),
              ce('div',{style:{fontSize:11,color:'var(--text3)',marginTop:3}},'нажмите — водители, авто, даты →')); }))) 
    ):(
      // по водителям (как было)
      ce('div',null,
...
function LogistsModal({logists,trucks,onSave,onClose}){
  const [list,setList]=useState((logists||[]).map(function(l){return {name:l.name,color:l.color,seeAll:!!l.seeAll};}));
  ...
  return ce(Modal,{title:'👥 Логисты',onClose,wide:true},
    ...
    ce('div',{style:{display:'flex',flexDirection:'column',gap:8,marginBottom:16}},
      ...
      list.map(function(l){ var c=cnt(l.name); return ce('div',{key:l.name,style:{display:'flex',alignItems:'center',gap:11,padding:'10px 12px',borderRadius:11,background:'var(--bg2)',border:'1px solid var(--border)',flexWrap:'wrap'}},
        ...
        ce('label',{style:{display:'flex',alignItems:'center',gap:5,fontSize:11.5,color:l.seeAll?'var(--accent)':'var(--text3)',cursor:'pointer',marginLeft:8},title:'Видит весь транспорт в x2 Планировании (а не только свой)'},
          ce('input',{type:'checkbox',checked:!!l.seeAll,onChange:function(e){ setList(list.map(function(x){ return x.name===l.name?Object.assign({},x,{seeAll:e.target.checked}):x; })); },style:{width:14,height:14,cursor:'pointer'}}),
          '👁 видит весь транспорт'),
...
function ManagerKPI({trucks,orders,history}){
  ...
  const total=trucks.length, moving=trucks.filter(effMoving).length, free=trucks.filter(isFreeTruck).length,
...
function MechLegend(){
  ...
  return ce('div',{style:{background:'var(--bg2)',border:'1px solid var(--border)',borderRadius:13,marginBottom:20,overflow:'hidden'}},
    ...
    open?ce('div',null,
      ...
      row('🛞','Шины прицепа (1/2/3 ось)','ресурс '+tk(TIRE_TRAILER_KM)+' на каждую ось отдельно.'),
      row('🔄','Пересадка прицепа','у прицепа свой счётчик пробега. При переходе под другой тягач (кнопка 🔄 на карточке) пробег сохраняется и продолжает расти от нового тягача — ТО и шины не сбиваются.'),
...
function MechDash({tractors,trailers,acts,trucks,onJump}){
  const items=[];
  ...
  trailers.forEach(function(tr){
    var pt=tractors.find(function(x){return x.plate===tr.pairedTractor;});
    var odo=trailerMileage(tr,tractors);
    ...
    items.push({plate:tr.plate||'—',cat:'Санпаспорт',s:dateStatus(tr.sanitary),jump:'trailers'});
  ...
  var inRoute=(trucks||[]).filter(function(t){ return MOVING.includes(t.status); });
  return ce('div',null,
...
function TrailerCard({tr,tractors,onUpd,onDel,onSwap}){
  const pt=tractors.find(function(x){return x.plate===tr.pairedTractor;});
  const odo=trailerMileage(tr,tractors);
  ...
  const sanitary=dateStatus(tr.sanitary);
  ...
  return ce('div',{className:'fl-card',style:{background:'var(--bg2)',border:'1px solid var(--border)',borderRadius:14,padding:14,boxShadow:'var(--shadow)'}},
    ce('div',{style:{display:'flex',alignItems:'center',gap:8,marginBottom:10}},
      ...
      onSwap?ce('button',{onClick:function(){ onSwap(tr); },className:'fl-press',title:'Пересадить под другой тягач',
        style:{width:32,height:32,borderRadius:8,border:'1px solid color-mix(in srgb,var(--cyan) 50%,transparent)',background:'transparent',color:'var(--cyan)',cursor:'pointer',fontSize:14,flexShrink:0}},'🔄'):null,
      ce('button',{onClick:function(){ if(window.confirm('Удалить прицеп '+(tr.plate||'')+' из списка механика?')) onDel(tr.id); },className:'fl-press',
        style:{width:32,height:32,borderRadius:8,border:'1px solid color-mix(in srgb,var(--red) 45%,transparent)',background:'transparent',color:'var(--red)',cursor:'pointer',fontSize:15,flexShrink:0}},'🗑')),
    ce('div',{style:{display:'flex',flexWrap:'wrap',gap:8,marginBottom:12}},
      ce(StatusPill,{s:to,label:'ТО прицепа'}),ce(StatusPill,{s:ts1,label:'Шины 1 ось'}),ce(StatusPill,{s:ts2,label:'Шины 2 ось'}),ce(StatusPill,{s:ts3,label:'Шины 3 ось'}),ce(StatusPill,{s:tinsp,label:'Техосмотр'}),ce(StatusPill,{s:frc,label:'FRC'}),ce(StatusPill,{s:sanitary,label:'Санпаспорт'}),ce(StatusPill,{s:unit,label:'ТО установки'}),ce(StatusPill,{s:unitFull,label:'ТО полное'}),
      ce(StatusPill,{s:ins,label:'Страховка'}),ce(StatusPill,{s:kas,label:'КАСКО'}),ce(StatusPill,{s:akb,label:'АКБ'})),
    ...
    ce('div',{style:{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:8}},
      ...
      fld('Тягач (гос.номер)',ce('select',{value:tr.pairedTractor||'',onChange:function(e){
        var np=e.target.value, had=tr.pairedTractor;
        if(had&&np&&np!==had){                                   // это пересадка — сохраняем пробег прицепа
          var cur=trailerMileage(tr,tractors);
          var nt=(tractors||[]).find(function(x){ return x.plate===np; });
          var nodo=nt?Number(nt.odometer):NaN;
          if(isFinite(nodo)&&cur!=null){ onUpd(tr.id,{pairedTractor:np,accum:String(Math.round(cur)),attachBase:String(nodo)}); return; }
        }
        onUpd(tr.id,{pairedTractor:np});
      },style:mecInp},
      ...
      fld('Санитарно-гигиенич. паспорт до',ce('input',dat('sanitary'))),
...
function MechTrailers({trailers,tractors,onAdd,onUpd,onDel}){
  ...
  return ce('div',null,
    swapFor?ce(TrailerSwapModal,{tr:swapFor,tractors:tractors,onClose:function(){setSwapFor(null);},
      onSwap:function(tr2,newPlate,newOdo,curMil){ onUpd(tr2.id,{pairedTractor:newPlate,attachBase:String(newOdo),accum:String(Math.round(curMil||0))}); setSwapFor(null); }}):null,
    ...
    ce('div',{style:{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(360px,1fr))',gap:14}},
      trailers.map(function(tr){ return ce(TrailerCard,{key:tr.id,tr:tr,tractors:tractors,onUpd:onUpd,onDel:onDel,onSwap:function(x){setSwapFor(x);}}); })));
}
...
// ═══════════ ЗАЯВКИ КЛИЕНТОВ (планирование) ═══════════
const DEM_KIND={load:{t:'Загрузка',c:'var(--violet)'},back:{t:'Обратка',c:'var(--cyan)'}};
function demDayKey(d){ return d.getFullYear()+'-'+String(d.getMonth()+1).padStart(2,'0')+'-'+String(d.getDate()).padStart(2,'0'); }
function freeAtTs(t){ if(!t) return null; var d=new Date(t); return isNaN(d.getTime())?null:d.getTime(); }
/* Свободна ли машина к этому дню: свободна сейчас или освободится до конца дня */
function availableBy(t,dayKey){
  if(MOVING.includes(t.status)){
    var ft=freeAtTs(t.freeAt); if(ft==null) return false;
    return ft<=new Date(dayKey+'T23:59:59').getTime();
  }
  return t.status!=='SERVICE'&&!t.onService;
}
function DemandEditor({d,clients,orders,dayKey,onSave,onCancel}){
  d=d||{};
  const [clientName,setClientName]=useState(d.clientName||'');
  const [count,setCount]=useState(d.count!=null?String(d.count):'1');
  const [kind,setKind]=useState(d.kind||'load');
  const [time,setTime]=useState(d.time||'');
  const [note,setNote]=useState(d.note||'');
  const [orderId,setOrderId]=useState(d.orderId||'');
  const pickOrder=function(oid){
    setOrderId(oid);
    var o=(orders||[]).find(function(x){ return String(x.id)===String(oid); });
    if(o){ setClientName(o.client||''); if(o.cargo) setNote(o.cargo+(o.from||o.to?(' · '+(o.from||'')+' → '+(o.to||'')):'')); }
  };
  const bad=!clientName.trim()||!(Number(count)>0);
  return ce(Modal,{title:(d.id?'✎ Заявка':'➕ Новая заявка')+' · '+fmtD(d.date||dayKey),onClose:onCancel},
    ce('div',{style:{display:'grid',gap:10}},
      ce('div',{style:{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}},
        fld('Сколько машин',ce('input',{value:count,onChange:e=>setCount(e.target.value.replace(/[^0-9]/g,'')),inputMode:'numeric',style:mecInp})),
        fld('Тип',ce('select',{value:kind,onChange:e=>{ setKind(e.target.value); if(e.target.value==='load'){ setOrderId(''); } },style:mecInp},
          ce('option',{value:'load'},'Загрузка'),ce('option',{value:'back'},'Обратка')))),
      kind==='back'?fld('Выбрать обратку (из раздела «Обратки»)',ce('select',{value:orderId,onChange:e=>pickOrder(e.target.value),style:mecInp},
        ce('option',{value:''},'— вписать вручную ниже —'),
        (orders||[]).filter(function(o){ return !o.assigned; }).map(function(o){
          return ce('option',{key:o.id,value:o.id},(o.client||'—')+' · '+(o.cargo||'')+(o.to?(' → '+o.to):'')); }))):null,
      fld('Клиент',ce('input',{value:clientName,onChange:e=>{ setClientName(e.target.value); setOrderId(''); },list:'fl-dem-clients',placeholder:'Санта, Серволюкс…',style:mecInp})),
      ce('datalist',{id:'fl-dem-clients'},(clients||[]).map(function(c,i){ return ce('option',{key:i,value:c.name}); })),
      fld('Время / смена',ce('input',{value:time,onChange:e=>setTime(e.target.value),placeholder:'например: 3 в ночь, 5 днём',style:mecInp})),
      fld('Заметка',ce('input',{value:note,onChange:e=>setNote(e.target.value),style:mecInp}))),
    ce('div',{style:{display:'flex',gap:9,marginTop:16}},
      ce(Btn,{onClick:onCancel,color:'var(--text3)',wide:true},'Отмена'),
      ce('button',{onClick:function(){ if(!bad) onSave(Object.assign({},d,{date:d.date||dayKey,clientName:clientName.trim(),count:Number(count),kind:kind,time:time.trim(),note:note.trim(),orderId:orderId||''})); },
        disabled:bad,className:'fl-press',style:{flex:1,padding:'12px',border:'none',borderRadius:11,color:'#fff',fontSize:14,fontWeight:800,cursor:bad?'default':'pointer',
        background:bad?'var(--bg3)':'linear-gradient(135deg,var(--accent),var(--accent2))',opacity:bad?0.5:1}},'Сохранить')));
}
function AssignModal({dem,trucks,onSave,onClose}){
  const [sel,setSel]=useState((dem.assigned||[]).slice());
  const [showHired,setShowHired]=useState(false);
  const [q,setQ]=useState('');
  const s=(q||'').toLowerCase();
  const pool=(trucks||[]).filter(function(t){
    if(isSpareDriver(t)) return false;              // подменные — не показываем
    if(!showHired&&t.isHired) return false;
    if(s&&((t.plate||'')+' '+(t.driver||'')).toLowerCase().indexOf(s)<0) return false;
    return true;
  }).map(function(t){ return {t:t,free:availableBy(t,dem.date),ft:freeAtTs(t.freeAt)}; })
   .sort(function(a,b){
     if(a.free!==b.free) return a.free?-1:1;
     if(a.ft==null) return -1; if(b.ft==null) return 1; return a.ft-b.ft; });
  const toggle=function(id){ setSel(function(p){ return p.indexOf(id)>=0?p.filter(function(x){return x!==id;}):p.concat([id]); }); };
  const need=Number(dem.count)||0;
  return ce(Modal,{title:'🚚 '+dem.clientName+' · '+need+' авто · '+fmtD(dem.date),onClose,wide:true},
    ce('div',{style:{fontSize:12,color:'var(--text3)',marginBottom:10,lineHeight:1.6}},
      (dem.time?('Время: '+dem.time+'. '):'')+'Сверху — кто свободен к этому дню. Привлечённые скрыты, включите переключателем.'),
    ce('div',{style:{display:'flex',gap:9,marginBottom:10,flexWrap:'wrap',alignItems:'center'}},
      ce('input',{value:q,onChange:e=>setQ(e.target.value),placeholder:'Поиск',
        style:{flex:1,minWidth:150,background:'var(--bg3)',border:'1px solid var(--border2)',borderRadius:10,padding:'9px 11px',color:'var(--text)',fontSize:13.5,fontFamily:'inherit'}}),
      ce('label',{style:{display:'flex',alignItems:'center',gap:6,fontSize:12,color:'var(--text3)',cursor:'pointer'}},
        ce('input',{type:'checkbox',checked:showHired,onChange:e=>setShowHired(e.target.checked)}),'показать привлечённых'),
      ce('span',{style:{fontWeight:800,fontSize:13,color:sel.length>=need?'var(--green)':'var(--amber)'}},'выбрано '+sel.length+' из '+need)),
    ce('div',{style:{maxHeight:330,overflowY:'auto',border:'1px solid var(--border)',borderRadius:11}},
      pool.length===0?ce('div',{style:{padding:'18px 12px',color:'var(--text3)',fontSize:13}},'Машин не найдено.'):
      pool.map(function(x,i){ var t=x.t, on=sel.indexOf(t.id)>=0;
        return ce('div',{key:t.id,className:'fl-press',onClick:function(){toggle(t.id);},style:{display:'flex',alignItems:'center',gap:10,padding:'10px 12px',cursor:'pointer',flexWrap:'wrap',
          borderBottom:i<pool.length-1?'1px solid var(--border)':'none',fontSize:12.5,
          background:on?'color-mix(in srgb,var(--accent) 12%,transparent)':'transparent',
          borderLeft:'3px solid '+(on?'var(--accent)':(x.free?'var(--green)':'transparent'))}},
          ce('span',{style:{width:16,flexShrink:0,color:on?'var(--accent)':'var(--text3)',fontWeight:800}},on?'✓':''),
          ce('span',{style:{fontWeight:800,minWidth:120}},t.plate||'—'),
          ce('span',{style:{color:'var(--text2)',minWidth:130,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}},t.driver||'—'),
          t.isHired?ce('span',{style:{fontSize:10.5,color:'var(--amber)',fontWeight:700}},'привл.'):null,
          ce('span',{style:{marginLeft:'auto',fontSize:11.5,fontWeight:700,color:x.free?'var(--green)':'var(--text3)'}},
            x.free?(MOVING.includes(t.status)?('освободится '+fmtDT(t.freeAt)):'свободна'):(t.freeAt?('занята до '+fmtDT(t.freeAt)):'занята'))); })),
    ce('div',{style:{display:'flex',gap:9,marginTop:14}},
      ce(Btn,{onClick:onClose,color:'var(--text3)',wide:true},'Отмена'),
      ce('button',{onClick:function(){ onSave(Object.assign({},dem,{assigned:sel})); },className:'fl-press',
        style:{flex:1,padding:'12px',border:'none',borderRadius:11,color:'#fff',fontSize:14,fontWeight:800,cursor:'pointer',
        background:'linear-gradient(135deg,var(--accent),var(--accent2))'}},'Сохранить ('+sel.length+')')));
}
function DemandsTab({demands,trucks,clients,orders,onSave,onDel,carrierOffers,onDelOffer,onArchiveOffer,onLaunch}){
  const [showOffers,setShowOffers]=useState(true);
  const offers=(carrierOffers||[]).filter(function(o){ return o.status!=='done'; })
    .sort(function(a,b){ return (b.ts||'')<(a.ts||'')?-1:1; });
  const base=new Date(); base.setHours(0,0,0,0);
  const [day,setDay]=useState(demDayKey(base));
  const [view,setView]=useState('day');
  const [edit,setEdit]=useState(null);
  const [adding,setAdding]=useState(false);
  const [assign,setAssign]=useState(null);
  const wd=['Вс','Пн','Вт','Ср','Чт','Пт','Сб'];
  const days=[]; for(var i=0;i<14;i++) days.push(new Date(base.getTime()+i*864e5));
  const todayK=demDayKey(new Date());
  const byDay=function(k){ return (demands||[]).filter(function(d){ return d.date===k; }); };
  const list=byDay(day).sort(function(a,b){ return (a.kind===b.kind)?0:(a.kind==='load'?-1:1); });
  const own=(trucks||[]).filter(function(t){ return !t.isHired&&!isSpareDriver(t); });
  const freeBy=own.filter(function(t){ return availableBy(t,day); });
  const truckById=function(id){ return (trucks||[]).find(function(t){ return String(t.id)===String(id); }); };
  const weekRows=days.map(function(d){
    var k=demDayKey(d), ds=byDay(k);
    return {d:d,k:k,ds:ds,
      need:ds.reduce(function(a,x){return a+(Number(x.count)||0);},0),
      got:ds.reduce(function(a,x){return a+((x.assigned||[]).length);},0),
      free:own.filter(function(t){ return availableBy(t,k); }).length};
  });
  return ce('div',{style:{padding:'14px',maxWidth:1700,margin:'0 auto'}},
    adding?ce(DemandEditor,{dayKey:day,clients:clients,orders:orders,onSave:function(d){ onSave(d); setAdding(false); },onCancel:function(){setAdding(false);}}):null,
    edit?ce(DemandEditor,{d:edit,clients:clients,orders:orders,dayKey:day,onSave:function(d){ onSave(d); setEdit(null); },onCancel:function(){setEdit(null);}}):null,
    assign?ce(AssignModal,{dem:assign,trucks:trucks,onSave:function(d){ onSave(d); setAssign(null); },onClose:function(){setAssign(null);}}):null,
    ce('div',{style:{display:'flex',alignItems:'center',gap:10,marginBottom:12,flexWrap:'wrap'}},
      ce('div',{style:{fontWeight:800,fontSize:18}},'📋 Заявки клиентов'),
      ce('div',{className:'fl-subtabs',style:{display:'inline-flex',gap:3,background:'var(--bg2)',border:'1px solid var(--border)',borderRadius:9,padding:2}},
        [['day','День'],['week','Неделя']].map(function(v){ var on=view===v[0];
          return ce('button',{key:v[0],className:'fl-press',onClick:function(){setView(v[0]);},style:{padding:'6px 13px',borderRadius:7,cursor:'pointer',fontFamily:'inherit',fontSize:12,fontWeight:800,border:'none',
            background:on?'linear-gradient(135deg,var(--accent),var(--accent2))':'transparent',color:on?'#fff':'var(--text3)'}},v[1]); })),
      ce('button',{className:'fl-press',onClick:function(){setAdding(true);},style:{marginLeft:'auto',padding:'9px 15px',borderRadius:10,cursor:'pointer',fontFamily:'inherit',fontSize:12.5,fontWeight:800,
        border:'1px solid color-mix(in srgb,var(--green) 50%,transparent)',background:'color-mix(in srgb,var(--green) 14%,transparent)',color:'var(--green)'}},'➕ Заявка')),
    offers.length?ce('div',{style:{marginBottom:14,border:'1px solid color-mix(in srgb,var(--green) 40%,transparent)',borderRadius:13,overflow:'hidden'}},
      ce('button',{className:'fl-press',onClick:function(){setShowOffers(!showOffers);},style:{width:'100%',display:'flex',alignItems:'center',gap:9,padding:'11px 14px',
        background:'color-mix(in srgb,var(--green) 10%,transparent)',border:'none',cursor:'pointer',fontFamily:'inherit',color:'var(--text)',fontSize:13.5,fontWeight:800,textAlign:'left'}},
        ce('span',null,'🚛'),'Предложения перевозчиков',
        ce('span',{style:{padding:'1px 9px',borderRadius:999,background:'var(--green)',color:'#04120a',fontSize:12,fontWeight:800}},String(offers.length)),
        ce('span',{style:{marginLeft:'auto',color:'var(--text3)',fontSize:12,fontWeight:600}},showOffers?'скрыть ▲':'показать ▼')),
      showOffers?ce('div',null,offers.map(function(o,i){
        return ce('div',{key:o.id,style:{padding:'11px 14px',borderTop:'1px solid var(--border)',fontSize:12.5}},
          ce('div',{style:{display:'flex',alignItems:'center',gap:9,flexWrap:'wrap',marginBottom:5}},
            ce('span',{style:{fontWeight:800,fontSize:13.5}},o.name||'—'),
            ce('span',{style:{padding:'2px 9px',borderRadius:7,fontSize:11,fontWeight:800,background:'color-mix(in srgb,var(--accent) 15%,transparent)',color:'var(--accent)'}},(o.count||0)+' авто'),
            o.freeNow?ce('span',{style:{padding:'2px 9px',borderRadius:7,fontSize:11,fontWeight:800,background:'color-mix(in srgb,var(--green) 15%,transparent)',color:'var(--green)'}},'свободна сейчас'):
              ce('span',{style:{padding:'2px 9px',borderRadius:7,fontSize:11,fontWeight:800,background:'color-mix(in srgb,var(--amber) 15%,transparent)',color:'var(--amber)'}},'освободится'+(o.freeWhen?': '+o.freeWhen:'')),
            ce('span',{style:{marginLeft:'auto',fontSize:10.5,color:'var(--text3)'}},o.ts?fmtDT(o.ts):'')),
          ce('div',{style:{display:'flex',gap:14,flexWrap:'wrap',color:'var(--text3)',fontSize:12,marginBottom:o.note?5:0}},
            o.phone?ce('a',{href:'tel:'+o.phone,style:{color:'var(--accent)',textDecoration:'none',fontWeight:700}},'📞 '+o.phone):null,
            o.where?ce('span',null,'📍 '+o.where):null,
            o.date?ce('span',null,'📅 грузиться '+fmtD(o.date)):null),
          o.note?ce('div',{style:{fontSize:12,color:'var(--text2)',marginBottom:5}},'📝 '+o.note):null,
          ce('div',{style:{display:'flex',gap:7,marginTop:4}},
            ce('button',{className:'fl-press',onClick:function(){ onArchiveOffer&&onArchiveOffer(o.id); },style:{padding:'6px 12px',borderRadius:8,cursor:'pointer',fontFamily:'inherit',fontSize:11.5,fontWeight:800,
              border:'1px solid var(--border2)',background:'var(--bg3)',color:'var(--text2)'}},'✓ Обработано'),
            ce('button',{className:'fl-press',onClick:function(){ if(window.confirm('Удалить предложение от '+(o.name||'')+'?')) onDelOffer&&onDelOffer(o.id); },style:{padding:'6px 12px',borderRadius:8,cursor:'pointer',fontFamily:'inherit',fontSize:11.5,fontWeight:800,
              border:'1px solid color-mix(in srgb,var(--red) 40%,transparent)',background:'transparent',color:'var(--red)'}},'🗑 Удалить'))); })):null):null,
    view==='week'?ce('div',{style:{display:'grid',gap:8}},
      weekRows.filter(function(r){ return r.need>0||r.free>0; }).map(function(r){
        var hit=r.got>=r.need&&r.need>0, gap=r.need-r.got;
        return ce('div',{key:r.k,className:'fl-press',onClick:function(){ setDay(r.k); setView('day'); },
          style:{background:'var(--bg2)',border:'1px solid var(--border)',borderRadius:12,padding:'10px 12px',cursor:'pointer',
          borderLeft:'3px solid '+(r.need===0?'transparent':(hit?'var(--green)':(r.free<gap?'var(--red)':'var(--amber)')))}},
          ce('div',{style:{display:'flex',alignItems:'center',gap:9,flexWrap:'wrap',marginBottom:r.ds.length?6:0}},
            ce('span',{style:{fontWeight:800,fontSize:13.5,minWidth:96}},wd[r.d.getDay()]+' '+String(r.d.getDate()).padStart(2,'0')+'.'+String(r.d.getMonth()+1).padStart(2,'0')),
            r.need?ce('span',{style:{fontWeight:800,fontSize:13,color:hit?'var(--green)':'var(--amber)'}},r.got+' / '+r.need+' авто'):
              ce('span',{style:{fontSize:12,color:'var(--text3)'}},'заявок нет'),
            ce('span',{style:{marginLeft:'auto',fontSize:11.5,fontWeight:700,color:r.free?'var(--text2)':'var(--red)'}},'свободно '+r.free),
            (r.need>r.got&&r.free<gap)?ce('span',{style:{fontSize:11,fontWeight:800,color:'var(--red)'}},'не хватает '+(gap-r.free)):null),
          r.ds.length?ce('div',{style:{display:'flex',gap:6,flexWrap:'wrap'}},
            r.ds.map(function(x){ var K=DEM_KIND[x.kind]||DEM_KIND.load;
              return ce('span',{key:x.id,style:{padding:'3px 9px',borderRadius:7,fontSize:11,fontWeight:700,
                background:'color-mix(in srgb,'+K.c+' 13%,transparent)',color:K.c}},
                (x.clientName||'—')+' · '+((x.assigned||[]).length)+'/'+(x.count||0)+(x.time?(' · '+x.time):'')); })):null); })):
    ce('div',{style:{display:'flex',gap:6,overflowX:'auto',paddingBottom:8,marginBottom:12,WebkitOverflowScrolling:'touch'}},
      days.map(function(d){ var k=demDayKey(d), act=k===day;
        var ds=byDay(k), need=ds.reduce(function(a,x){ return a+(Number(x.count)||0); },0);
        var got=ds.reduce(function(a,x){ return a+((x.assigned||[]).length); },0);
        return ce('button',{key:k,className:'fl-press',onClick:function(){setDay(k);},style:{flexShrink:0,minWidth:62,padding:'7px 9px',borderRadius:11,cursor:'pointer',fontFamily:'inherit',textAlign:'center',
          ...
          ce('div',{style:{fontSize:10.5,fontWeight:700}},wd[d.getDay()]+(k===todayK?' •':'')),
          ce('div',{style:{fontSize:13,fontWeight:800,marginTop:1}},String(d.getDate()).padStart(2,'0')+'.'+String(d.getMonth()+1).padStart(2,'0')),
          need?ce('div',{style:{fontSize:9.5,marginTop:2,fontWeight:800,color:got>=need?'var(--green)':'var(--amber)'}},got+'/'+need):
            ce('div',{style:{fontSize:9.5,marginTop:2}},'—')); })),
    view==='week'?null:ce('div',{style:{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(150px,1fr))',gap:10,marginBottom:14}},
      dashCard('Нужно машин',list.reduce(function(a,x){return a+(Number(x.count)||0);},0),'var(--violet)'),
      dashCard('Расставлено',list.reduce(function(a,x){return a+((x.assigned||[]).length);},0),'var(--accent)'),
      dashCard('Свободны к дню',freeBy.length,freeBy.length?'var(--green)':'var(--red)','из своих')),
    view==='week'?null:
    list.length===0?ce('div',{style:{textAlign:'center',padding:36,color:'var(--text3)',fontSize:13}},'На этот день заявок нет.'):
    ce('div',{style:{display:'grid',gap:10}},
      list.map(function(d){ var K=DEM_KIND[d.kind]||DEM_KIND.load; var got=(d.assigned||[]).length, need=Number(d.count)||0;
        return ce('div',{key:d.id,style:{background:'var(--bg2)',border:'1px solid var(--border)',borderRadius:13,padding:'11px 13px',
          borderLeft:'3px solid '+K.c}},
          ce('div',{style:{display:'flex',alignItems:'center',gap:9,flexWrap:'wrap',marginBottom:7}},
            ce('span',{style:{fontWeight:800,fontSize:14}},d.clientName||'—'),
            ce('span',{style:{padding:'2px 8px',borderRadius:7,fontSize:10.5,fontWeight:800,background:'color-mix(in srgb,'+K.c+' 15%,transparent)',color:K.c}},K.t),
            d.time?ce('span',{style:{fontSize:11.5,color:'var(--text3)'}},'⏰ '+d.time):null,
            ce('span',{style:{marginLeft:'auto',fontWeight:800,fontSize:13,color:got>=need?'var(--green)':'var(--amber)'}},got+' / '+need),
            ce('button',{onClick:function(){setEdit(d);},className:'fl-press',style:{width:28,height:28,borderRadius:7,border:'1px solid var(--border2)',background:'var(--bg3)',color:'var(--text2)',cursor:'pointer'}},'✎'),
            ce('button',{onClick:function(){ if(window.confirm('Удалить заявку '+(d.clientName||'')+'?')) onDel(d.id); },className:'fl-press',
              style:{width:28,height:28,borderRadius:7,border:'1px solid color-mix(in srgb,var(--red) 40%,transparent)',background:'transparent',color:'var(--red)',cursor:'pointer'}},'🗑')),
          d.note?ce('div',{style:{fontSize:11.5,color:'var(--text3)',marginBottom:6}},d.note):null,
          (d.assigned||[]).length?ce('div',{style:{display:'flex',gap:6,flexWrap:'wrap',marginBottom:7}},
            (d.assigned||[]).map(function(id){ var t=truckById(id);
              return ce('span',{key:id,style:{padding:'3px 9px',borderRadius:7,fontSize:11,fontWeight:700,
                background:'var(--bg3)',border:'1px solid var(--border2)',color:'var(--text2)'}},
                (t&&t.plate)||'—'); })):null,
          ce('button',{className:'fl-press',onClick:function(){setAssign(d);},style:{width:'100%',padding:'8px',borderRadius:9,cursor:'pointer',fontFamily:'inherit',fontSize:12,fontWeight:800,
            border:'1px solid color-mix(in srgb,var(--accent) 45%,transparent)',background:'color-mix(in srgb,var(--accent) 10%,transparent)',color:'var(--accent)'}},
            got?'🚚 Изменить машины':'🚚 Подобрать машины'),
          (got&&onLaunch)?ce('button',{className:'fl-press',onClick:function(){ if(window.confirm('Отправить '+got+' машин в рейс по заявке «'+(d.clientName||'')+'»? Они станут активными рейсами.')) onLaunch(d); },
            style:{width:'100%',marginTop:6,padding:'9px',borderRadius:9,cursor:'pointer',fontFamily:'inherit',fontSize:12.5,fontWeight:800,border:'none',color:'#fff',
            background:'linear-gradient(135deg,var(--green),var(--accent))'}},'▶ Отправить в рейс ('+got+')'):null,
          ce('label',{style:{display:'flex',alignItems:'center',gap:7,marginTop:8,fontSize:11.5,color:d.customerSent?'var(--green)':'var(--text3)',cursor:'pointer'}},
            ce('input',{type:'checkbox',checked:!!d.customerSent,onChange:function(e){ onSave(Object.assign({},d,{customerSent:e.target.checked})); },style:{width:14,height:14,cursor:'pointer'}}),
            '✅ данные заказчику отправлены')); })));
}
...
function BindTripModal({plate,driver,trucks,clients,onPick,onClose,onRememberTemp}){
  const [q,setQ]=useState('');
  ...
  const s=(q||'').toLowerCase();
  const list=moving.filter(function(t){ return !s||((t.plate||'')+' '+(t.driver||'')+' '+(t.clientName||'')).toLowerCase().indexOf(s)>=0; });
  const clientTemp=function(t){ var cl=(clients||[]).find(function(c){ return c.id===t.clientId; }); return (cl&&cl.temp)||''; };
  const clientTempByName=function(nm){ var cl=(clients||[]).find(function(c){ return c.name===nm; }); return (cl&&cl.temp)||''; };
  const choose=function(t){ setSel(t); if(!touched) setTemp(clientTemp(t)); };
  const chooseManual=function(nm){ setManualClient(nm); if(!touched){ var tp=clientTempByName(nm); if(tp) setTemp(tp); } };
  const [remember,setRemember]=useState(true);
  const range=parseTempRange(temp);
  const ready=!!(sel||temp.trim()||manualClient);
  const save=function(){
    if(!ready) return;
    var hrs=Number(hours)||14;
    var cname=sel?(sel.clientName||''):manualClient;
    // запоминаем температуру за клиентом — в следующий раз подставится сама
    if(remember&&cname&&temp.trim()&&onRememberTemp) onRememberTemp(cname,temp.trim());
    // отложенный старт: если дата задана и она в будущем — отсчёт с неё, иначе с текущего момента
    var startTs=Date.now();
    if(startDate){ var sd=new Date(startDate+'T00:00:00'); if(!isNaN(sd.getTime())&&sd.getTime()>Date.now()) startTs=sd.getTime(); }
    onPick({truckId:sel?sel.id:'',truckPlate:sel?(sel.plate||''):'',driver:sel?(sel.driver||''):'',
      clientName:cname,
      temp:temp.trim(),at:nowLocalISO(),since:new Date(startTs).toISOString(),
      starts:startTs>Date.now()?new Date(startTs).toISOString():'',
      expires:new Date(startTs+hrs*3600*1000).toISOString(),hours:hrs});
  };
  return ce(Modal,{title:'🔗 Привязать '+plate,onClose,wide:true},
    ce('div',{style:{fontSize:12,color:'var(--text3)',marginBottom:10,lineHeight:1.6}},
      'Выберите рейс — температура подставится из карточки клиента. Её можно изменить или задать вручную, если рейса в списке нет.'+(driver?(' Датчик подписан: '+driver+'.'):'')),
    ce('input',{value:q,onChange:function(e){setQ(e.target.value);},placeholder:'Поиск по машине, водителю, клиенту',
      style:{width:'100%',background:'var(--bg3)',border:'1px solid var(--border2)',borderRadius:10,padding:'10px 12px',color:'var(--text)',fontSize:13.5,fontFamily:'inherit',marginBottom:10}}),
    list.length===0?ce('div',{style:{color:'var(--text3)',fontSize:13,padding:'14px 2px'}},
      moving.length?'Ничего не найдено.':'Сейчас нет машин в рейсе — можно задать только температуру ниже.'):
    ce('div',{style:{maxHeight:230,overflowY:'auto',border:'1px solid var(--border)',borderRadius:11,marginBottom:12}},
      list.map(function(t,i){ var tp=clientTemp(t); var on=sel&&sel.id===t.id;
        return ce('div',{key:t.id,className:'fl-press',onClick:function(){ choose(t); },style:{display:'flex',alignItems:'center',gap:10,padding:'10px 12px',cursor:'pointer',
          borderBottom:i<list.length-1?'1px solid var(--border)':'none',fontSize:12.5,
          background:on?'color-mix(in srgb,var(--accent) 12%,transparent)':'transparent',
          borderLeft:'3px solid '+(on?'var(--accent)':'transparent')}},
          ce('span',{style:{fontWeight:800,minWidth:130}},t.plate||'—'),
          ce('span',{style:{color:'var(--text2)',minWidth:140,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}},t.driver||'—'),
          ce('span',{style:{color:'var(--text3)',flex:1,minWidth:0,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}},t.clientName||'—'),
          ce('span',{style:{fontWeight:800,color:tp?'var(--cyan)':'var(--text3)',minWidth:70,textAlign:'right'}},tp?(tp+'°'):'без t°'),
          on?ce('span',{style:{color:'var(--accent)',fontWeight:800,fontSize:14}},'✓'):null); })),
    ce('div',{style:{background:'var(--bg3)',border:'1px solid var(--border)',borderRadius:11,padding:'11px 13px',marginBottom:10}},
      ce('div',{style:{fontSize:11,color:'var(--text3)',fontWeight:700,marginBottom:6}},'КЛИЕНТ (ЧЕЙ ГРУЗ ВЕЗЁТ)'),
      sel?ce('div',{style:{fontSize:13,fontWeight:700,color:'var(--cyan)'}},'🚚 '+(sel.clientName||'—')+' (из рейса)'):
      ce('select',{value:manualClient,onChange:function(e){chooseManual(e.target.value);},style:mecInp},
        ce('option',{value:''},'— выберите клиента —'),
        (clients||[]).map(function(c){ return ce('option',{key:c.id,value:c.name},c.name); })),
      !sel?ce('div',{style:{fontSize:10.5,color:'var(--text3)',marginTop:6}},'Если рейс не выбран — отметьте вручную, чей груз, чтобы на карточке было видно «везёт Серволюкс».'):null),
    ce('div',{style:{background:'var(--bg3)',border:'1px solid var(--border)',borderRadius:11,padding:'11px 13px'}},
      ce('div',{style:{fontSize:11,color:'var(--text3)',fontWeight:700,marginBottom:6}},'ТРЕБУЕМАЯ ТЕМПЕРАТУРА'),
      ce('div',{style:{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}},
        ce('input',{value:temp,onChange:function(e){ setTemp(e.target.value); setTouched(true); },placeholder:'например -18 или +1/-3',
          style:{flex:1,minWidth:150,background:'var(--bg2)',border:'1px solid var(--border2)',borderRadius:9,padding:'9px 11px',color:'var(--text)',fontSize:14,fontFamily:'inherit',fontWeight:700}}),
        ['-18','-20','+1/-3','+2/+6','0/+4'].map(function(p){
          return ce('button',{key:p,className:'fl-press',onClick:function(){ setTemp(p); setTouched(true); },
            style:{padding:'6px 11px',borderRadius:8,cursor:'pointer',fontFamily:'inherit',fontSize:11.5,fontWeight:800,
            border:'1px solid var(--border2)',background:'var(--bg2)',color:'var(--text3)'}},p+'°'); })),
      temp.trim()&&!range?ce('div',{style:{marginTop:7,fontSize:11.5,color:'var(--amber)',fontWeight:700}},
        '⚠ Не разобрал температуру — напишите числом: -18, +4, или диапазоном +1/-3'):null,
      range?ce('div',{style:{marginTop:7,fontSize:11.5,color:'var(--green)',fontWeight:700}},
        range.min===range.max?('Норма: '+fmtTemp(range.max)+(range.max<=0?' — холоднее допустимо':'')):
        ('Норма: от '+fmtTemp(range.min)+' до '+fmtTemp(range.max))):null,
      !temp.trim()?ce('div',{style:{marginTop:7,fontSize:11.5,color:'var(--text3)'}},
        'Без температуры индикатор не появится — будет видна только текущая цифра.'):null),
      ((sel&&sel.clientName)||manualClient)&&temp.trim()?ce('label',{style:{display:'flex',alignItems:'center',gap:8,marginTop:10,fontSize:12,color:'var(--text2)',cursor:'pointer'}},
        ce('input',{type:'checkbox',checked:remember,onChange:function(e){setRemember(e.target.checked);}}),
        'Запомнить '+temp.trim()+'° для клиента «'+((sel&&sel.clientName)||manualClient)+'» — подставится в следующий раз'):null,
    ce('div',{style:{background:'var(--bg3)',border:'1px solid var(--border)',borderRadius:11,padding:'11px 13px',marginTop:10}},
      ce('div',{style:{fontSize:11,color:'var(--text3)',fontWeight:700,marginBottom:6}},'СРОК ДЕЙСТВИЯ КАРТОЧКИ'),
      ce('div',{style:{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}},
        ce('input',{value:hours,onChange:function(e){ setHours(e.target.value.replace(/[^0-9]/g,'')); },inputMode:'numeric',
          style:{width:70,background:'var(--bg2)',border:'1px solid var(--border2)',borderRadius:9,padding:'8px 10px',color:'var(--text)',fontSize:14,fontFamily:'inherit',fontWeight:700,textAlign:'center'}}),
        ce('span',{style:{fontSize:12.5,color:'var(--text2)'}},'часов'),
        [['8','8ч'],['14','14ч'],['24','сутки'],['48','2 суток']].map(function(p){
          return ce('button',{key:p[0],className:'fl-press',onClick:function(){setHours(p[0]);},
            style:{padding:'6px 11px',borderRadius:8,cursor:'pointer',fontFamily:'inherit',fontSize:11.5,fontWeight:800,
            border:'1px solid '+(hours===p[0]?'color-mix(in srgb,var(--accent) 50%,transparent)':'var(--border2)'),
            background:hours===p[0]?'color-mix(in srgb,var(--accent) 12%,transparent)':'var(--bg2)',color:hours===p[0]?'var(--accent)':'var(--text3)'}},p[1]); })),
      ce('div',{style:{fontSize:11,color:'var(--text3)',marginTop:7,lineHeight:1.5}},'Через это время карточка сама уйдёт из рейса и индикатор погаснет — не нужно отвязывать вручную.')),
    ce('div',{style:{background:'var(--bg3)',border:'1px solid var(--border)',borderRadius:11,padding:'11px 13px',marginTop:10}},
      ce('div',{style:{fontSize:11,color:'var(--text3)',fontWeight:700,marginBottom:6}},'НАЧАТЬ СЛЕЖЕНИЕ С ДАТЫ (необязательно)'),
      ce('div',{style:{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}},
        ce('input',{type:'date',value:startDate,onChange:function(e){ setStartDate(e.target.value); },
          style:{background:'var(--bg2)',border:'1px solid var(--border2)',borderRadius:9,padding:'8px 10px',color:'var(--text)',fontSize:14,fontFamily:'inherit',fontWeight:700}}),
        startDate?ce('button',{className:'fl-press',onClick:function(){setStartDate('');},
          style:{padding:'7px 12px',borderRadius:8,cursor:'pointer',fontFamily:'inherit',fontSize:11.5,fontWeight:800,border:'1px solid var(--border2)',background:'var(--bg2)',color:'var(--text3)'}},'сейчас'):null),
      (function(){ if(!startDate) return ce('div',{style:{fontSize:11,color:'var(--text3)',marginTop:7}},'Пусто — слежение начнётся сразу.');
        var sd=new Date(startDate+'T00:00:00');
        if(!isNaN(sd.getTime())&&sd.getTime()>Date.now())
          return ce('div',{style:{fontSize:11,color:'var(--accent)',marginTop:7,fontWeight:700,lineHeight:1.5}},'📅 Карточка будет серой до '+fmtD(startDate)+', с этой даты включится контроль на '+(Number(hours)||14)+' ч.');
        return ce('div',{style:{fontSize:11,color:'var(--text3)',marginTop:7}},'Дата уже прошла — слежение начнётся сразу.'); })()),
    ce('div',{style:{display:'flex',gap:9,marginTop:14}},
      ce(Btn,{onClick:onClose,color:'var(--text3)',wide:true},'Отмена'),
      ce('button',{onClick:save,disabled:!ready,className:'fl-press',
        style:{flex:1,padding:'12px',border:'none',borderRadius:11,color:'#fff',fontSize:14,fontWeight:800,cursor:ready?'pointer':'default',
        background:ready?'linear-gradient(135deg,var(--accent),var(--accent2))':'var(--bg3)',opacity:ready?1:0.5}},
        sel?'Привязать к рейсу':(manualClient?'Привязать · '+manualClient:'Привязать по температуре'))));
}
function bindExpired(bind){ if(!bind||!bind.expires) return false; var t=new Date(bind.expires); return !isNaN(t.getTime())&&t.getTime()<=Date.now(); }
function bindPending(bind){ if(!bind||!bind.starts) return false; var t=new Date(bind.starts); return !isNaN(t.getTime())&&t.getTime()>Date.now(); }
function durLeft(iso){ if(!iso) return '—'; var t=new Date(iso); if(isNaN(t.getTime())) return '—';
  var min=Math.floor((t.getTime()-Date.now())/60000); if(min<=0) return 'истекла';
  if(min<60) return min+' мин'; var h=Math.floor(min/60); if(h<24) return h+' ч '+(min%60)+' мин'; return Math.floor(h/24)+' дн '+(h%24)+' ч'; }
function TempCard({r,onBind,onUnbind,onHistory,readOnly}){
  const has=r.val!=null;
  const bound=!!r.bind;
  const col=bound?r.s.color:(has?'var(--text)':'var(--text3)');
  return ce('div',{style:{background:'var(--bg2)',borderRadius:14,padding:'12px 13px',
    border:'1px solid '+((bound&&(r.s.st==='high'||r.s.st==='low'))?'color-mix(in srgb,var(--red) 55%,transparent)':'var(--border)'),
    boxShadow:'var(--shadow)',display:'flex',flexDirection:'column',gap:7,minWidth:0}},
    ce('div',{style:{display:'flex',alignItems:'baseline',gap:8}},
      ce('div',{style:{fontWeight:800,fontSize:14,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}},r.plate),
      r.vid?ce('button',{className:'fl-press',onClick:function(){onHistory(r);},title:'История температуры',
        style:{marginLeft:'auto',width:26,height:26,borderRadius:7,border:'1px solid var(--border2)',background:'var(--bg3)',color:'var(--text3)',cursor:'pointer',fontSize:12,flexShrink:0}},'📈'):null),
    r.driver?ce('div',{style:{fontSize:11,color:'var(--text3)',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis',marginTop:-4}},r.driver):null,
    (r.capDriver&&r.driver&&r.capDriver!==r.driver)?ce('div',{style:{fontSize:9.5,color:'var(--amber)',marginTop:-3,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}},'в CapNavi: '+r.capDriver):null,
    ce('div',{style:{display:'flex',alignItems:'baseline',gap:8}},
      ce('div',{style:{fontSize:27,fontWeight:800,color:col,lineHeight:1.1}},fmtTemp(r.val)),
      ce('div',{style:{fontSize:10.5,color:'var(--text3)'}},
        r.age==null?'':(r.age<1?'только что':(r.age<60?(r.age+' мин'):(Math.floor(r.age/60)+' ч'))))),
    r.alt!=null?ce('div',{style:{fontSize:10,color:'var(--text3)',marginTop:-3}},'датчик тягача: '+fmtTemp(r.alt)):null,
    r.geo?ce('div',{style:{display:'flex',alignItems:'center',gap:6,fontSize:11,marginTop:-1}},
      ce('span',{style:{fontWeight:700,color:r.geo.state==='moving'?'var(--cyan)':'var(--text3)'}},
        r.geo.state==='moving'?('🚗 в пути · '+Math.round(r.geo.speed)+' км/ч'):(r.geo.state==='stopped'?'⏸ стоит':'📍')),
      (r.geo.at&&geoAgeMin(r.geo.at)!=null&&geoAgeMin(r.geo.at)>30)?ce('span',{style:{fontSize:9.5,color:'var(--amber)'}},'('+(geoAgeMin(r.geo.at)<60?geoAgeMin(r.geo.at)+' мин':Math.floor(geoAgeMin(r.geo.at)/60)+' ч')+' назад)'):null,
      ce('a',{href:'https://www.google.com/maps?q='+r.geo.lat+','+r.geo.lon,target:'_blank',rel:'noopener',
        style:{marginLeft:'auto',fontSize:10,fontWeight:700,color:'var(--accent)',textDecoration:'none',padding:'2px 7px',borderRadius:6,border:'1px solid color-mix(in srgb,var(--accent) 30%,transparent)'}},'карта')):null,
    bound?ce('div',{style:{display:'flex',flexDirection:'column',gap:5,paddingTop:6,borderTop:'1px solid var(--border)'}},
      r.bind.clientName?ce('div',{style:{fontSize:12,fontWeight:800,color:'var(--cyan)',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}},'🚚 везёт: '+r.bind.clientName):
        ce('div',{style:{fontSize:11,color:'var(--text3)'}},r.bind.truckPlate?('рейс '+r.bind.truckPlate):'задано вручную'),
      (r.bind.clientName&&r.bind.truckPlate)?ce('div',{style:{fontSize:10,color:'var(--text3)',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis',marginTop:-2}},r.bind.truckPlate):null,
      r.bind.since?ce('div',{style:{fontSize:10,color:'var(--text3)',marginTop:-2}},'⏳ карточка ещё '+durLeft(r.bind.expires)):null,
      ce('div',{style:{display:'flex',alignItems:'center',gap:7,flexWrap:'wrap'}},
        ce('span',{style:{padding:'3px 9px',borderRadius:7,fontSize:11,fontWeight:800,
          background:'color-mix(in srgb,'+r.s.color+' 14%,transparent)',color:r.s.color}},r.s.label),
        r.bind.temp?ce('span',{style:{fontSize:10.5,color:'var(--text3)'}},'норма '+r.bind.temp+'°'):
          ce('span',{style:{fontSize:10.5,color:'var(--amber)'}},'у клиента нет t°'),
        readOnly?null:ce('button',{className:'fl-press',onClick:function(){onUnbind(r);},title:'Отвязать',
          style:{marginLeft:'auto',padding:'3px 8px',borderRadius:7,border:'1px solid var(--border2)',background:'transparent',color:'var(--text3)',cursor:'pointer',fontSize:10.5}},'отвязать'))):
      (readOnly?ce('div',{style:{fontSize:11,color:'var(--text3)',paddingTop:6,borderTop:'1px solid var(--border)'}},'не привязан к рейсу'):
      ce('div',{style:{paddingTop:6,borderTop:'1px solid var(--border)'}},
        ce('div',{style:{fontSize:10,color:'var(--text3)',marginBottom:5,lineHeight:1.4}},'Привяжите к рейсу — появится контроль режима'),
        ce('button',{className:'fl-press',onClick:function(){onBind(r);},style:{width:'100%',padding:'7px 10px',borderRadius:9,cursor:'pointer',fontFamily:'inherit',fontSize:11.5,fontWeight:800,
          border:'1px solid color-mix(in srgb,var(--accent) 45%,transparent)',background:'color-mix(in srgb,var(--accent) 10%,transparent)',color:'var(--accent)'}},'🔗 Привязать к рейсу'))));
}
/* Уведомления о выходе за режим. Работают, пока приложение открыто (в т.ч. в фоновой вкладке).
   На iPhone — только если сайт добавлен на главный экран и разрешение выдано. */
var TEMP_NOTIFIED={};
function tempNotifyReady(){ try{ return typeof Notification!=='undefined'&&Notification.permission==='granted'; }catch(e){ return false; } }
function tempNotifyAsk(cb){ try{ if(typeof Notification==='undefined'){ cb&&cb('no'); return; }
    if(Notification.permission==='granted'){ cb&&cb('granted'); return; }
    Notification.requestPermission().then(function(p){ cb&&cb(p); }); }catch(e){ cb&&cb('no'); } }
function tempNotify(list){
  if(!tempNotifyReady()) return;
  list.forEach(function(x){
    var key=x.plate+'|'+x.s.st;
    if(TEMP_NOTIFIED[key]) return;                 // не дублируем, пока состояние не менялось
    TEMP_NOTIFIED[key]=Date.now();
    try{ new Notification('🌡 '+x.plate+' — вне режима',{
      body:(x.driver?(x.driver+' · '):'')+fmtTemp(x.val)+' при норме '+(x.bind&&x.bind.temp?x.bind.temp+'°':'—')+'\n'+x.s.label,
      tag:'temp-'+x.plate, renotify:false }); }catch(e){}
  });
  // сбрасываем отметку для тех, кто вернулся в норму
  Object.keys(TEMP_NOTIFIED).forEach(function(k){
    var pl=k.split('|')[0];
    if(!list.some(function(z){ return z.plate===pl; })) delete TEMP_NOTIFIED[k];
  });
}
function TempLegend(){
  const [open,setOpen]=useState(false);
  const row=(icon,name,rule)=>ce('div',{key:name,style:{display:'flex',gap:10,padding:'8px 12px',borderBottom:'1px solid var(--border)',fontSize:12.5,alignItems:'baseline'}},
    ...
    ce('span',{style:{fontWeight:800,minWidth:170,flexShrink:0}},name),
    ce('span',{style:{color:'var(--text2)',lineHeight:1.5}},rule));
  return ce('div',{style:{background:'var(--bg2)',border:'1px solid var(--border)',borderRadius:13,marginBottom:14,overflow:'hidden'}},
    ce('button',{className:'fl-press',onClick:()=>setOpen(!open),style:{width:'100%',display:'flex',alignItems:'center',gap:9,padding:'11px 15px',
      background:'transparent',border:'none',cursor:'pointer',fontFamily:'inherit',color:'var(--text)',fontSize:13.5,fontWeight:800,textAlign:'left'}},
      ce('span',null,'ℹ️'),'Как это работает',
      ce('span',{style:{marginLeft:'auto',color:'var(--text3)',fontSize:12,fontWeight:600}},open?'скрыть ▲':'показать ▼')),
    open?ce('div',null,
      row('🔗','Привяжите авто к рейсу','Чтобы контролировать температуру, нажмите на карточке «Привязать к рейсу» и выберите машину. Из рейса подтянется требуемая температура клиента.'),
      row('🚦','Индикатор','Появляется ТОЛЬКО после привязки — до этого видно просто цифру, без оценки. Именно индикатор показывает, соблюдается режим или нет.'),
      row('⟳','Обновление','Показания обновляются автоматически раз в 10 минут, а также по нажатию кнопки «Обновить» — если нужно посмотреть прямо сейчас.'),
      row('📡','Запрос к датчику','Сами датчики на машинах отправляют данные примерно раз в 10 минут. Поэтому запрашивать чаще смысла нет — придут те же цифры.'),
      row('🕒','Возраст замера','Под температурой написано, когда её сняли («5 мин», «2 ч»). Если данным больше часа — вместо оценки покажем «данные устарели»: скорее всего датчик молчит.'),
      row('❄️','Заморозка','Если требование задано одним числом ниже нуля (например −18), то холоднее — не нарушение. Ругаемся только когда теплее нормы.'),
      row('🌡','Охлаждёнка','Если требование в плюсе или диапазоном (+1/−3), нарушением считается выход в обе стороны — подморозить груз тоже брак. Допуск ±0.5°.'),
      ce('div',{style:{display:'flex',gap:16,padding:'10px 15px',fontSize:12,flexWrap:'wrap'}},
        ce('span',{style:{color:'var(--green)',fontWeight:700}},'🟢 в норме'),
        ce('span',{style:{color:'var(--red)',fontWeight:700}},'🔴 вне режима'),
        ce('span',{style:{color:'var(--amber)',fontWeight:700}},'🟡 данные устарели'),
        ce('span',{style:{color:'var(--text3)',fontWeight:700}},'⚪ не привязан к рейсу'))):null);
}
function TempTab({trucks,clients,binds,onBind,readOnly,onRememberTemp,geoIndex}){
  ...
  const [notif,setNotif]=useState(function(){ return tempNotifyReady(); });
  const load=function(){
  ...
  useEffect(function(){ load(); var iv=setInterval(load,10*60*1000); var tk=setInterval(function(){ setTick(function(n){return n+1;}); },60*1000); return function(){ clearInterval(iv); clearInterval(tk); }; },[]);
  const B=binds||{};
  // строим карточки ПО ПРИЦЕПАМ из CapNavi (без привязки к диспетчерской)
  const trailers=[];
  const tractorTemp={};
  (rows||[]).forEach(function(r){
    var p=capParseName(r.name||'');
    ...
    if(!p.plate) return;
    if(p.kind==='tractor'){ if(v!=null) tractorTemp[p.plate]=v; return; }
    var nm=String(r.name||'').replace(/^\s*\d{3,4}\s*/,'').replace(/[A-ZА-ЯЁa-zа-яё]?\s*\d{4}\s*[A-ZА-ЯЁ]?\s*-?\s*\d?/,'').trim();
    // водитель и машина — из НАШЕЙ базы (что назначено в Fletera), CapNavi только как запасной вариант
    var own=(trucks||[]).find(function(t){ return splitFleteraPlate(t.plate).trailer===p.plate; });
    // геолокация — по тягачу этой сцепки (координаты приходят на тягач)
    var geo=null;
    if(own){ var sp2=splitFleteraPlate(own.plate); if(geoIndex&&geoIndex[sp2.tractor]) geo=geoIndex[sp2.tractor]; }
    trailers.push({plate:p.plate,driver:(own&&own.driver)||nm,capDriver:nm,ownPlate:own?own.plate:'',
      val:v,at:s.measured_at,age:tempAgeMin(s.measured_at),vid:r.vehicle_id,raw:r.name,geo:geo});
  });
  // привязка тягача той же сцепки (если есть)
  const list=trailers.map(function(x){
    var bind=B[x.plate]||null;
    if(bind&&bindExpired(bind)) bind=null;   // срок вышел — карточка уходит из рейса
    var pending=bind?bindPending(bind):false;  // старт ещё не наступил
    var range=bind?parseTempRange(bind.temp):null;
    var s=(bind&&!pending)?tempStatus(x.val,range,x.age):
          (pending?{st:'pending',label:'слежение с '+fmtD(bind.starts),color:'var(--violet)'}:{st:'idle',label:'не привязан',color:'var(--text3)'});
    var alt=null;
    if(bind&&bind.truckPlate){ var sp=splitFleteraPlate(bind.truckPlate); if(tractorTemp[sp.tractor]!=null) alt=tractorTemp[sp.tractor]; }
    return Object.assign({},x,{bind:bind,range:range,s:s,alt:alt,pending:pending});
  }).sort(function(a,b){
    var rank=function(z){ if(z.bind&&(z.s.st==='high'||z.s.st==='low')) return 0; if(z.bind) return 1; if(z.val!=null) return 2; return 3; };
    var d=rank(a)-rank(b); if(d) return d;
    return String(a.plate).localeCompare(String(b.plate));
  });
  const bad=list.filter(function(x){ return x.bind&&(x.s.st==='high'||x.s.st==='low'); });
  useEffect(function(){ if(rows) tempNotify(bad); },[rows]);
  const boundCnt=list.filter(function(x){ return x.bind; }).length;
  var clientFilter=filter&&filter.indexOf('cl:')===0?filter.slice(3):null;
  const shown=filter==='bad'?bad:(filter==='bound'?list.filter(function(x){return x.bind;}):
    (clientFilter?list.filter(function(x){return x.bind&&x.bind.clientName===clientFilter;}):list));
  // список клиентов, которые сейчас в рейсах
  var clientsInTrip={}; list.forEach(function(x){ if(x.bind&&x.bind.clientName) clientsInTrip[x.bind.clientName]=(clientsInTrip[x.bind.clientName]||0)+1; });
  var clientNames=Object.keys(clientsInTrip).sort();
  const chip=function(k,label,n){ var on=filter===k;
    return ce('button',{key:k,className:'fl-press',onClick:function(){setFilter(k);},style:{padding:'6px 13px',borderRadius:9,cursor:'pointer',fontFamily:'inherit',fontSize:12,fontWeight:800,
      border:'1px solid '+(on?'var(--accent)':'var(--border2)'),background:on?'color-mix(in srgb,var(--accent) 14%,transparent)':'var(--bg2)',color:on?'var(--accent)':'var(--text3)'}},label+' ('+n+')'); };
  return ce('div',{style:{padding:'16px',maxWidth:1700,margin:'0 auto'}},
    hist?ce(TempHistoryModal,{vid:hist.vid,title:hist.plate+(hist.driver?(' · '+hist.driver):''),range:hist.range,onClose:function(){setHist(null);}}):null,
    bindFor?ce(BindTripModal,{plate:bindFor.plate,driver:bindFor.driver,trucks:trucks,clients:clients,
      onPick:function(b){ onBind(bindFor.plate,b); setBindFor(null); },onClose:function(){setBindFor(null);},
      onRememberTemp:onRememberTemp}):null,
    ce('div',{style:{display:'flex',alignItems:'center',gap:12,marginBottom:12,flexWrap:'wrap'}},
      ...
      ce('button',{className:'fl-press',onClick:function(){ tempNotifyAsk(function(p){ setNotif(p==='granted'); if(p!=='granted') window.alert('Уведомления не разрешены. На iPhone добавьте сайт на главный экран, откройте оттуда и разрешите уведомления.'); }); },
        style:{marginLeft:'auto',padding:'8px 13px',borderRadius:9,cursor:'pointer',fontFamily:'inherit',fontSize:12.5,fontWeight:800,
        border:'1px solid '+(notif?'color-mix(in srgb,var(--green) 45%,transparent)':'var(--border2)'),
        background:notif?'color-mix(in srgb,var(--green) 12%,transparent)':'var(--bg2)',color:notif?'var(--green)':'var(--text2)'}},
        notif?'🔔 Уведомления вкл':'🔔 Включить уведомления'),
      ce('button',{className:'fl-press',onClick:load,disabled:loading,style:{padding:'8px 15px',borderRadius:9,cursor:loading?'default':'pointer',fontFamily:'inherit',
    ...
    err?ce('div',{style:{background:'color-mix(in srgb,var(--red) 12%,transparent)',border:'1px solid color-mix(in srgb,var(--red) 40%,transparent)',
      ...
      '⚠ Не удалось получить данные CapNavi: '+err):null,
    ce(TempLegend,null),
    ce('div',{style:{display:'flex',alignItems:'center',gap:8,marginBottom:12,flexWrap:'wrap'}},
      chip('all','Все рефы',list.length),
      clientNames.map(function(cn){ return chip('cl:'+cn,'🚚 '+cn,clientsInTrip[cn]); }),
      chip('bound','В рейсе',boundCnt),
      chip('bad','Вне режима',bad.length),
      ce('span',{style:{fontSize:11.5,color:'var(--text3)',marginLeft:'auto'}},
        'Обновление: авто раз в 10 мин и по кнопке. Индикатор — после привязки к рейсу.')),
    rows===null&&!err?ce('div',{style:{textAlign:'center',padding:40,color:'var(--text3)'}},'Загружаю данные CapNavi…'):
    shown.length===0?ce('div',{style:{textAlign:'center',padding:40,color:'var(--text3)'}},
      filter==='bad'?'Нарушений нет.':(filter==='bound'?'Ничего не привязано к рейсам.':'Рефов с датчиками не найдено.')):
    ce('div',{style:{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(215px,1fr))',gap:11}},
      shown.map(function(r){ return ce(TempCard,{key:r.plate,r:r,readOnly:readOnly,
        onBind:function(x){ setBindFor(x); },
        onUnbind:function(x){ if(window.confirm('Отвязать '+x.plate+' от рейса?')) onBind(x.plate,null); },
        onHistory:function(x){ setHist(x); }}); })));
}
// ═══════════ ПУЛЬТ РУКОВОДИТЕЛЯ ═══════════
...
// ═══ Статус водителя в x2: выходной / пересадка на другую машину ═══
// ═══ Окно водителя в «Своём транспорте»: отправить / выходной / пересадка ═══
function DriverMarkModal({driver,mark,trucks,truck,geoIndex,nextLoad,unloadPoint,onSave,onClear,onSend,onCancelGo,onClose}){
  const [type,setType]=useState((mark&&mark.type)||(truck&&truck.goTo&&!truck.goTo.done?'go':'go'));
  const [from,setFrom]=useState((mark&&mark.from)||new Date().toISOString().slice(0,10));
  const [to,setTo]=useState((mark&&mark.to)||'');
  const [plate,setPlate]=useState((mark&&mark.plate)||'');
  const [note,setNote]=useState((mark&&mark.note)||'');
  const [place,setPlace]=useState((truck&&truck.goTo&&truck.goTo.place)||'base');
  const [when,setWhen]=useState((truck&&truck.goTo&&truck.goTo.when)||'after_unload');
  const [calc,setCalc]=useState(null);
  const inp={width:'100%',background:'var(--bg3)',border:'1px solid var(--border2)',borderRadius:10,padding:'10px 12px',color:'var(--text)',fontSize:14,fontFamily:'inherit'};
  const chip=function(active,col){ return {flex:1,textAlign:'center',padding:'9px 6px',borderRadius:9,fontSize:12.5,fontWeight:700,cursor:'pointer',
    background:active?('color-mix(in srgb,'+col+' 18%,transparent)'):'var(--bg2)',border:'1px solid '+(active?col:'var(--border)'),color:active?col:'var(--text3)'}; };

  // текущая точка машины по GPS
  var gps=null;
  if(truck){ var sp=splitFleteraPlate(truck.plate||''); var g=geoIndex&&geoIndex[sp.tractor]; if(g&&g.lat!=null) gps={lat:g.lat,lon:g.lon,name:'текущее место'}; }
  // откуда считаем: после выгрузки — от точки выгрузки, сейчас — от GPS
  var origin=(when==='after_unload'&&unloadPoint)?unloadPoint:gps;

  // расчёт крюка: origin → место → следующая загрузка  против  origin → загрузка
  useEffect(function(){
    if(type!=='go'){ return; }
    var pl=placeById(place);
    if(!origin||!pl){ setCalc({err:!origin?'Нет координат машины/выгрузки':'Нет места'}); return; }
    setCalc({loading:true});
    var alive=true;
    var o=[origin.lat,origin.lon], p=[pl.lat,pl.lon];
    var jobs=[roadDistances([o],p)];
    if(nextLoad){ var n=[nextLoad.lat,nextLoad.lon]; jobs.push(roadDistances([p],n)); jobs.push(roadDistances([o],n)); }
    Promise.all(jobs).then(function(r){
      if(!alive) return;
      var toPlace=r[0].km[0], road=r.every(function(x){return x.road;});
      if(!nextLoad){ setCalc({toPlace:toPlace,road:road}); return; }
      var placeToNext=r[1].km[0], direct=r[2].km[0];
      var via=(toPlace!=null&&placeToNext!=null)?toPlace+placeToNext:null;
      setCalc({toPlace:toPlace,placeToNext:placeToNext,direct:direct,via:via,extra:(via!=null&&direct!=null)?via-direct:null,road:road});
    });
    return function(){ alive=false; };
  },[type,place,when,origin&&origin.lat,origin&&origin.lon,nextLoad&&nextLoad.lat]);

  var pl=placeById(place);
  var goActive=truck&&truck.goTo&&!truck.goTo.done;
  return ce(Modal,{onClose:onClose,title:'👤 '+driver},
    ce('div',{style:{display:'grid',gap:12}},
      ce('div',{style:{display:'flex',gap:6}},
        ce('div',{onClick:function(){setType('go');},style:chip(type==='go','var(--accent)')},'📍 Отправить'),
        ce('div',{onClick:function(){setType('off');},style:chip(type==='off','var(--amber)')},'🌴 Выходной'),
        ce('div',{onClick:function(){setType('swap');},style:chip(type==='swap','var(--violet)')},'🔄 Пересадка')),

      // ── ОТПРАВИТЬ ──
      type==='go'?ce('div',{style:{display:'grid',gap:12}},
        goActive?ce('div',{style:{padding:'9px 11px',borderRadius:9,fontSize:12.5,background:'color-mix(in srgb,var(--accent) 10%,transparent)',border:'1px solid color-mix(in srgb,var(--accent) 35%,transparent)'}},
          'Уже отправлен: '+((placeById(truck.goTo.place)||{}).name||truck.goTo.name||'')+(truck.goTo.when==='after_unload'?' (после выгрузки)':' (сейчас)')+' · '+(truck.goTo.ack?'✅ водитель принял':'⏳ водитель ещё не открыл')):null,
        ce('div',null,
          ce('div',{style:{fontSize:12,color:'var(--text3)',marginBottom:5}},'Куда'),
          ce('div',{style:{display:'flex',gap:6}},
            FLEET_PLACES.map(function(p){ return ce('div',{key:p.id,onClick:function(){setPlace(p.id);},style:chip(place===p.id,'var(--accent)')},p.icon+' '+p.name); }))),
        ce('div',null,
          ce('div',{style:{fontSize:12,color:'var(--text3)',marginBottom:5}},'Когда'),
          ce('div',{style:{display:'flex',gap:6}},
            ce('div',{onClick:function(){setWhen('after_unload');},style:chip(when==='after_unload','var(--green)')},'После выгрузки'),
            ce('div',{onClick:function(){setWhen('now');},style:chip(when==='now','var(--green)')},'Сейчас'))),
        // расчёт крюка
        ce('div',{style:{padding:'10px 12px',borderRadius:10,background:'var(--bg3)',border:'1px solid var(--border2)',fontSize:13,lineHeight:1.55}},
          !calc?null:
          calc.err?ce('div',{style:{color:'var(--text3)'}},calc.err):
          calc.loading?ce('div',{style:{color:'var(--text3)'}},'Считаю маршрут…'):
          ce('div',null,
            ce('div',{style:{color:'var(--text3)',fontSize:11.5,marginBottom:4}},'Откуда: '+(origin&&origin.name||'')+(calc.road?' · по дорогам':' · ≈ по прямой')),
            ce('div',null,'До «'+(pl&&pl.name)+'»: ',ce('b',null,(calc.toPlace!=null?calc.toPlace+' км':'—'))),
            nextLoad?ce('div',null,'Оттуда на загрузку «'+nextLoad.name+'»: ',ce('b',null,(calc.placeToNext!=null?calc.placeToNext+' км':'—'))):null,
            nextLoad&&calc.direct!=null?ce('div',{style:{color:'var(--text3)'}},'Если ехать сразу на загрузку: '+calc.direct+' км'):null,
            (nextLoad&&calc.extra!=null)?ce('div',{style:{marginTop:6,padding:'7px 10px',borderRadius:8,fontWeight:800,
              background:calc.extra>30?'color-mix(in srgb,var(--red) 14%,transparent)':'color-mix(in srgb,var(--green) 12%,transparent)',
              color:calc.extra>30?'var(--red)':'var(--green)'}},
              calc.extra>30?('⚠ Крюк +'+calc.extra+' км порожняком'):('✓ Почти по пути: +'+Math.max(0,calc.extra)+' км')):null,
            !nextLoad?ce('div',{style:{color:'var(--text3)',fontSize:11.5,marginTop:4}},'Следующая загрузка не назначена — крюк не посчитать'):null)),
        ce('div',null,
          ce('div',{style:{fontSize:12,color:'var(--text3)',marginBottom:5}},'Сообщение водителю (необязательно)'),
          ce('input',{value:note,onChange:function(e){setNote(e.target.value);},placeholder:'напр. сдать документы, заменить колесо',style:inp})),
        ce('div',{style:{display:'flex',gap:9}},
          ce(Btn,{onClick:onClose,color:'var(--text3)',wide:true},'Отмена'),
          goActive?ce('button',{onClick:function(){ onCancelGo(); },className:'fl-press',
            style:{padding:'12px 14px',border:'1px solid color-mix(in srgb,var(--red) 45%,transparent)',borderRadius:11,background:'transparent',color:'var(--red)',fontSize:13,fontWeight:800,cursor:'pointer',fontFamily:'inherit'}},'Отменить'):null,
          ce('button',{onClick:function(){ onSend({place:place,name:pl.name,lat:pl.lat,lon:pl.lon,when:when,note:note.trim(),
              extraKm:(calc&&calc.extra!=null)?calc.extra:null,toKm:(calc&&calc.toPlace!=null)?calc.toPlace:null,
              nextLoad:nextLoad?nextLoad.name:''}); },className:'fl-press',
            style:{flex:1,padding:'12px',border:'none',borderRadius:11,color:'#fff',fontSize:14,fontWeight:800,cursor:'pointer',
            background:'linear-gradient(135deg,var(--accent),var(--accent2))'}},'📤 Отправить водителю'))):null,

      // ── ВЫХОДНОЙ / ПЕРЕСАДКА ──
      type!=='go'?ce('div',{style:{display:'grid',gap:12}},
        ce('div',null,
          ce('div',{style:{fontSize:12,color:'var(--text3)',marginBottom:5}},type==='off'?'С какого числа':'Пересаживается с числа'),
          ce('input',{type:'date',value:from,onChange:function(e){setFrom(e.target.value);},style:inp})),
        type==='off'?ce('div',null,
          ce('div',{style:{fontSize:12,color:'var(--text3)',marginBottom:5}},'По какое (пусто = один день)'),
          ce('input',{type:'date',value:to,onChange:function(e){setTo(e.target.value);},style:inp})):null,
        type==='swap'?ce('div',null,
          ce('div',{style:{fontSize:12,color:'var(--text3)',marginBottom:5}},'На какую машину'),
          ce('input',{value:plate,onChange:function(e){setPlate(e.target.value);},list:'fl-mark-trucks',placeholder:'номер машины',style:inp}),
          ce('datalist',{id:'fl-mark-trucks'},(trucks||[]).map(function(t,i){ return ce('option',{key:i,value:t.plate}); }))):null,
        ce('div',null,
          ce('div',{style:{fontSize:12,color:'var(--text3)',marginBottom:5}},'Заметка'),
          ce('input',{value:note,onChange:function(e){setNote(e.target.value);},placeholder:'необязательно',style:inp})),
        ce('div',{style:{display:'flex',gap:9,marginTop:4}},
          ce(Btn,{onClick:onClose,color:'var(--text3)',wide:true},'Отмена'),
          mark?ce('button',{onClick:function(){ onClear(); },className:'fl-press',
            style:{padding:'12px 16px',border:'1px solid color-mix(in srgb,var(--red) 45%,transparent)',borderRadius:11,background:'transparent',color:'var(--red)',fontSize:13,fontWeight:800,cursor:'pointer',fontFamily:'inherit'}},'Убрать'):null,
          ce('button',{onClick:function(){ onSave({type:type,from:from,to:to,plate:plate.trim(),note:note.trim()}); },className:'fl-press',
            style:{flex:1,padding:'12px',border:'none',borderRadius:11,color:'#fff',fontSize:14,fontWeight:800,cursor:'pointer',
            background:'linear-gradient(135deg,var(--accent),var(--accent2))'}},'Сохранить'))):null));
}
// ═══════════ x2 ПЛАНИРОВАНИЕ — недельная сетка водитель × день с полосой рейса ═══════════
function PlanGrid({plans,clients,orders,trucks,geoIndex,onSavePlan,onDelPlan,onLaunch,onAddOrder,onSetBar,role,logists,isBoss,gridBacks,onSetGridBack,onDelOrder,gridTuda,onSetGridTuda,driverMarks,onSetDriverMark,onUpdateTruck}){
  const [drvQ,setDrvQ]=useState('');
  const [markDrv,setMarkDrv]=useState(null);
  const [edit,setEdit]=useState(null);
  const [addCell,setAddCell]=useState(null); // {driver, dateKey}
  const dk=function(d){ return d.getFullYear()+'-'+String(d.getMonth()+1).padStart(2,'0')+'-'+String(d.getDate()).padStart(2,'0'); };
  const wd=['Вс','Пн','Вт','Ср','Чт','Пт','Сб'];
  const base=new Date(); base.setHours(0,0,0,0);
  const dow=(base.getDay()+6)%7; // 0=Пн
  const [showPastWeek,setShowPastWeek]=useState(function(){ try{ return localStorage.getItem('egida_x2_past')==='1'; }catch(e){ return false; } });
  const thisMonday=new Date(base.getTime()-dow*864e5);
  const monday=showPastWeek?new Date(thisMonday.getTime()-7*864e5):thisMonday;   // прошлая неделя — по кнопке
  const NDAYS=showPastWeek?28:21;
  const days=[]; for(var i=0;i<NDAYS;i++) days.push(new Date(monday.getTime()+i*864e5));
  const weekKeys=days.map(dk);
  const todayK=dk(new Date());
  const COLW=110;   // ширина колонки дня (px)
  const gridCols='170px repeat('+NDAYS+','+COLW+'px)';   // сетка: имя + N дней

  // строки-водители: берём из планов этой недели + машины в рейсе
  const driverRows=useMemo(function(){
    var map={};
    var norm=function(s){ return String(s||'').trim().toLowerCase(); };
    var normPlate=function(s){ return String(s||'').toUpperCase().replace(/[\s\-\/]/g,''); };
    // 1) свои водители парка — всегда в сетке (привлечённые в x2 не показываем)
    (trucks||[]).forEach(function(t){
      if(!(t.driver||'').trim()) return;
      if(t.isHired) return;
      var key=norm(t.driver)||normPlate(t.plate)||'—';
      if(!map[key]) map[key]={driver:t.driver||'',plate:t.plate||'',plans:[],_np:normPlate(t.plate)};
    });
    // находим ключ строки, к которой относится план (по фамилии или номеру)
    var findKey=function(p){
      var dn=norm(p.truckDriver), pl=normPlate(p.truckPlate);
      // точное по фамилии
      if(dn && map[dn]) return dn;
      // частичное по фамилии (план «Ткаченко» → парк «Ткаченко Сергей...»)
      var byName=Object.keys(map).find(function(k){ return dn && (k.indexOf(dn)>=0 || dn.indexOf(k)>=0); });
      if(byName) return byName;
      // по номеру машины
      var byPlate=Object.keys(map).find(function(k){ return pl && map[k]._np && map[k]._np===pl; });
      if(byPlate) return byPlate;
      return null;
    };
    // 2) планы этой недели раскидываем по водителям
    (plans||[]).forEach(function(p){
      var kk=p.loadDate||p.date;
      var inWeek=weekKeys.indexOf(kk)>=0;
      var spansIn=!inWeek && kk<weekKeys[0] && p.freeAt && String(p.freeAt).slice(0,10)>=weekKeys[0] && String(p.freeAt).slice(0,10)<=weekKeys[NDAYS-1];
      if(!inWeek && !spansIn) return;
      var key=findKey(p);
      if(!key){ key=norm(p.truckDriver)||normPlate(p.truckPlate)||'—'; if(!map[key]) map[key]={driver:p.truckDriver||'',plate:p.truckPlate||'',plans:[],_np:normPlate(p.truckPlate)}; }
      map[key].plans.push(p);
    });
    // порядок водителей ФИКСИРОВАННЫЙ по алфавиту — не прыгают при назначении планов
    return Object.keys(map).sort(function(a,b){
      return (map[a].driver||'').localeCompare(map[b].driver||'','ru');
    }).map(function(k){ return map[k]; });
  },[plans,trucks]);

  const clientColor=function(name){ var c=(clients||[]).find(function(x){ return x.name===name; }); return (c&&c.color)||'var(--accent)'; };

  // полосы рейсов: массив (один рейс = одна полоса, обрыв после обратки)
  const barsFor=function(row){
    if(!row.plans.length) return [];
    // разбиваем планы на РЕЙСЫ: рейс заканчивается планом с обраткой, следующий план — новый рейс
    var sortedAll=row.plans.slice().sort(function(a,b){ return (a.loadDate||a.date||'').localeCompare(b.loadDate||b.date||''); });
    var trips=[]; var cur=[];
    sortedAll.forEach(function(p){
      cur.push(p);
      if(p.back){ trips.push(cur); cur=[]; }   // обратка закрывает рейс
    });
    if(cur.length) trips.push(cur);
    // строим полосу для каждого рейса
    var bars=trips.map(function(tripPlans){
      var f=tripPlans[0], l=tripPlans[tripPlans.length-1];
      var mS='',mE='';
      tripPlans.forEach(function(p){ if(p.barStart&&(!mS||p.barStart<mS)) mS=p.barStart; if(p.barEnd&&(!mE||p.barEnd>mE)) mE=p.barEnd; });
      var lK=mS||f.loadDate||f.date; var sI=weekKeys.indexOf(lK);
      if(sI<0){ if(lK<weekKeys[0]) sI=0; else return null; }
      var eK=mE||l.loadDate||l.date; var eI=weekKeys.indexOf(eK);
      if(eI<0){ if(eK>weekKeys[NDAYS-1]) eI=NDAYS-1; else eI=sI; }
      var hasBack=tripPlans.some(function(p){ return !!p.back; });
      if(!mE){ if(hasBack) eI=Math.max(eI, weekKeys.indexOf(l.loadDate||l.date)+1); else if(eI===sI) eI=sI+1; }
      if(eI>NDAYS-1) eI=NDAYS-1; if(eI<sI) eI=sI;
      var bp=tripPlans.filter(function(p){return !!p.back;})[0];
      var bIdx=bp?weekKeys.indexOf(bp.loadDate||bp.date):-1;
      return {startIdx:sI,endIdx:eI,plan:f,col:clientColor(f.clientName),hasBack:hasBack,backIdx:bIdx,manual:!!(mS||mE)};
    }).filter(Boolean);
    return bars;
  };

  return ce('div',{style:{padding:'14px',maxWidth:1500,margin:'0 auto'}},
    markDrv?(function(){
      var mTruck=(trucks||[]).find(function(x){ return sameDriver(x.driver,markDrv); })||null;
      // следующая загрузка: ближайший ещё не начатый план этого водителя с координатами загрузки
      var nextLoad=null;
      (plans||[]).filter(function(p){ return sameDriver(p.truckDriver,markDrv) && !p.launched && (p.loadDate||p.date||'')>=todayK && p.clientName; })
        .sort(function(a,b){ return (a.loadDate||'').localeCompare(b.loadDate||''); })
        .some(function(p){ var c=(clients||[]).find(function(x){ return x.name===p.clientName; }); var lg=c&&normGeo(c.loadGeo);
          if(lg){ nextLoad={name:p.clientName,lat:lg.lat,lon:lg.lon,date:p.loadDate}; return true; } return false; });
      // точка выгрузки текущего рейса
      var unloadPoint=null;
      if(mTruck){ var cc=(clients||[]).find(function(x){ return x.id===mTruck.clientId||x.name===mTruck.clientName; }); var ug=cc&&normGeo(cc.unloadGeo);
        if(ug) unloadPoint={name:'выгрузка «'+cc.name+'»',lat:ug.lat,lon:ug.lon}; }
      return ce(DriverMarkModal,{driver:markDrv,mark:(driverMarks||{})[markDrv],trucks:trucks,truck:mTruck,geoIndex:geoIndex,
        nextLoad:nextLoad,unloadPoint:unloadPoint,
        onSave:function(m){ if(onSetDriverMark) onSetDriverMark(markDrv,m); setMarkDrv(null); },
        onClear:function(){ if(onSetDriverMark) onSetDriverMark(markDrv,null); setMarkDrv(null); },
        onSend:function(g){ if(mTruck&&onUpdateTruck) onUpdateTruck(mTruck.id,{goTo:Object.assign({},g,{at:new Date().toISOString(),by:role||'',ack:false,done:false})}); setMarkDrv(null); },
        onCancelGo:function(){ if(mTruck&&onUpdateTruck) onUpdateTruck(mTruck.id,{goTo:null}); setMarkDrv(null); },
        onClose:function(){setMarkDrv(null);}});
    })():null,
    edit?ce(PlanRowEditor,{row:edit,clients:clients,orders:orders,trucks:trucks,dateKey:edit.loadDate,geoIndex:geoIndex,
      onSave:function(r){ onSavePlan(r); setEdit(null); },onCancel:function(){setEdit(null);},
      onDelete:function(id){ if(onDelPlan) onDelPlan(id); setEdit(null); }}):null,
    addCell?ce(PlanRowEditor,{row:{truckDriver:addCell.driver,loadDate:addCell.dateKey},clients:clients,orders:orders,trucks:trucks,dateKey:addCell.dateKey,geoIndex:geoIndex,
      onSave:function(r){ onSavePlan(r); setAddCell(null); },onCancel:function(){setAddCell(null);}}):null,
    // шапка: навигация по неделям
    ce('div',{style:{display:'flex',alignItems:'center',gap:12,marginBottom:14,flexWrap:'wrap'}},
      ce('div',{style:{fontWeight:800,fontSize:19}},'🚛 Свой транспорт'),
      ce('input',{value:drvQ,onChange:function(e){setDrvQ(e.target.value);},placeholder:'🔍 поиск водителя…',
        style:{padding:'6px 12px',borderRadius:8,border:'1px solid var(--border2)',background:'var(--bg2)',color:'var(--text)',fontSize:13,fontFamily:'inherit',width:180}}),
      ce('button',{className:'fl-press',onClick:function(){ var v=!showPastWeek; setShowPastWeek(v); try{localStorage.setItem('egida_x2_past',v?'1':'0');}catch(e){} },
        style:{padding:'6px 12px',borderRadius:8,cursor:'pointer',fontFamily:'inherit',fontSize:12,fontWeight:700,
        border:'1px solid '+(showPastWeek?'var(--accent)':'var(--border2)'),background:showPastWeek?'color-mix(in srgb,var(--accent) 12%,transparent)':'var(--bg2)',color:showPastWeek?'var(--accent)':'var(--text3)'}},
        showPastWeek?'▲ скрыть прошлую неделю':'▼ прошлая неделя'),
      ce('div',{style:{display:'flex',alignItems:'center',gap:6,marginLeft:'auto'}},
        ce('span',{style:{fontSize:13,fontWeight:700,color:'var(--text2)'}},
          days[0].toLocaleDateString('ru',{day:'2-digit',month:'2-digit'})+' — '+days[NDAYS-1].toLocaleDateString('ru',{day:'2-digit',month:'2-digit'})),
        ce('button',{className:'fl-press',onClick:function(){
          // печать: НЕДЕЛЯ на лист, крупно и читаемо (2 листа = 2 недели)
          var pm=new Date(base.getTime()-((base.getDay()+6)%7)*864e5);
          var esc=function(s){ return String(s||'').replace(/[&<>]/g,function(c){return {'&':'&amp;','<':'&lt;','>':'&gt;'}[c];}); };
          var rowsForPrint=driverRows.filter(function(r){ return !isHiredR(r); });
          var html='<html><head><meta charset="utf-8"><title>x2 Планирование</title><style>'
            +'@page{size:A4 landscape;margin:8mm}'
            +'body{font-family:Arial,sans-serif;color:#000;margin:0}'
            +'table{border-collapse:collapse;width:100%;table-layout:fixed;page-break-after:always}'
            +'table:last-child{page-break-after:auto}'
            +'th,td{border:1px solid #666;padding:3px 4px;font-size:11px;vertical-align:top;word-break:break-word;height:26px}'
            +'th{background:#e8e8e8;text-align:center;font-size:11px}'
            +'td.n{width:130px;font-weight:bold;font-size:11px}'
            +'td.n small{font-weight:normal;color:#555;font-size:9px}'
            +'h2{font-size:15px;margin:0 0 6px}'
            +'.b{color:#0066aa;font-size:10px}'
            +'</style></head><body>';
          // две недели = две таблицы, каждая на своём листе
          [0,1].forEach(function(wk){
            var wdays=[]; for(var i=0;i<7;i++) wdays.push(new Date(pm.getTime()+(wk*7+i)*864e5));
            var wkeys=wdays.map(dk);
            html+='<h2>x2 Планирование · '+wdays[0].toLocaleDateString('ru')+' — '+wdays[6].toLocaleDateString('ru')+'</h2>';
            html+='<table><tr><th style="width:130px">Водитель</th>';
            wdays.forEach(function(d){ html+='<th>'+wd[d.getDay()]+' '+d.getDate()+'.'+(d.getMonth()+1)+'</th>'; });
            html+='</tr>';
            rowsForPrint.forEach(function(r){
              html+='<tr><td class="n">'+esc(r.driver)+(r.plate?('<br><small>'+esc(r.plate)+'</small>'):'')+'</td>';
              wkeys.forEach(function(kk){
                var pl=r.plans.find(function(p){ return (p.loadDate||p.date)===kk; });
                var cell='';
                if(pl){ cell=esc(pl.clientName||'');
                  if(pl.clientName2) cell+='<br>+ '+esc(pl.clientName2);
                  if(pl.back) cell+='<br><span class="b">↩ '+esc(pl.back)+'</span>'; }
                html+='<td>'+cell+'</td>';
              });
              html+='</tr>';
            });
          ...
          html+='</body></html>';
          var w=window.open('','_print','width=1200,height=850');
          w.document.write(html); w.document.close();
          setTimeout(function(){ w.print(); },400);
        },title:'Печать: неделя на лист, крупно',style:{padding:'6px 12px',borderRadius:8,cursor:'pointer',fontFamily:'inherit',fontSize:13,border:'1px solid var(--border2)',background:'var(--bg2)',color:'var(--text2)'}},'🖨 Печать'))),
    // сетка
    ce('div',{className:'fl-grid-wrap',style:{overflowX:'auto',border:'1px solid var(--border)',borderRadius:12}},
      ce('div',{style:{minWidth:(170+NDAYS*COLW)+'px'}},
        // заголовок дней
        ce('div',{style:{display:'grid',gridTemplateColumns:gridCols,borderBottom:'2px solid var(--border)',background:'var(--bg2)'}},
          ce('div',{style:{padding:'8px 10px',fontSize:11,fontWeight:800,color:'var(--text3)'}},'Водитель'),
          days.map(function(d,i){ var isT=dk(d)===todayK;
            return ce('div',{key:i,style:{padding:'8px 6px',textAlign:'center',fontSize:12,fontWeight:800,
              color:isT?'var(--accent)':'var(--text2)',background:isT?'color-mix(in srgb,var(--accent) 8%,transparent)':'transparent'}},
              wd[d.getDay()]+' '+d.getDate()); })),
        // строки водителей
        driverRows.length===0?ce('div',{style:{padding:30,textAlign:'center',color:'var(--text3)',fontSize:13}},'На эту неделю планов нет. Добавьте строки во вкладке «Планирование» — они появятся здесь сеткой.'):
        (function(){
          var renderDriverRow=function(row,ri){
          var bars=barsFor(row);
          // текущий рейс — тот, чьи дни включают сегодня; иначе последний. Фура только на нём.
          var todayIdx=weekKeys.indexOf(todayK);
          var activeBarIdx=-1;
          bars.forEach(function(b,i){ if(todayIdx>=0 && b.startIdx<=todayIdx && b.endIdx>=todayIdx) activeBarIdx=i; });
          if(activeBarIdx<0 && bars.length) activeBarIdx=bars.length-1;   // сегодня вне рейсов — берём последний
          var _dn=(row.driver||'').trim().toLowerCase();
          var mt=(trucks||[]).find(function(x){ return (x.driver||'').trim().toLowerCase()===_dn; })
               ||(_dn?(trucks||[]).find(function(x){ var xn=(x.driver||'').trim().toLowerCase(); return xn&&(xn.indexOf(_dn)>=0||_dn.indexOf(xn)>=0); }):null)
               ||(row.plate?(trucks||[]).find(function(x){ var n=function(s){return String(s||'').toUpperCase().replace(/[\s\-\/]/g,'');}; return n(x.plate)===n(row.plate); }):null);
          var hired=mt&&mt.isHired;
          // рендер одной полосы рейса (полоса + ручки + фура)
          var renderOneBar=function(bar,bi){
            return [
              ce('div',{key:'bar'+bi,style:{position:'absolute',top:'50%',transform:'translateY(-50%)',height:bar.hasBack?6:5,borderRadius:3,pointerEvents:'none',
                left:'calc(170px + '+(bar.startIdx*COLW)+'px + 4px)',
                width:(((bar.endIdx-bar.startIdx+1)*COLW-8)+'px'),
                background:bar.hasBack
                  ?('linear-gradient(90deg, '+bar.col+' 0%, '+bar.col+' 55%, var(--cyan) 55%, var(--cyan) 100%)')
                  :('repeating-linear-gradient(45deg, color-mix(in srgb,var(--red) 55%,transparent), color-mix(in srgb,var(--red) 55%,transparent) 6px, color-mix(in srgb,var(--red) 30%,transparent) 6px, color-mix(in srgb,var(--red) 30%,transparent) 12px)'),
                opacity:bar.hasBack?0.9:0.7,
                boxShadow:bar.hasBack?('0 0 7px color-mix(in srgb,var(--cyan) 55%,transparent)'):'none'}}),
              // ручки только для одного рейса (первого) чтобы не мешались
              ce('div',{key:'h1'+bi,style:{position:'absolute',top:2,zIndex:3,display:'flex',gap:2,
                left:'calc(170px + '+(bar.startIdx*COLW)+'px + 4px)'}},
                ce('button',{className:'fl-press fl-bar-h',title:'Начало влево',onClick:function(e){e.stopPropagation(); if(onSetBar){ onSetBar(bar.plan,weekKeys[Math.max(0,bar.startIdx-1)],weekKeys[bar.endIdx]); }},
                  style:{width:16,height:14,borderRadius:3,border:'none',cursor:'pointer',fontSize:8,padding:0,background:'color-mix(in srgb,'+bar.col+' 30%,transparent)',color:'var(--text)'}},'◀'),
                ce('button',{className:'fl-press fl-bar-h',title:'Начало вправо',onClick:function(e){e.stopPropagation(); if(onSetBar){ onSetBar(bar.plan,weekKeys[Math.min(bar.endIdx,bar.startIdx+1)],weekKeys[bar.endIdx]); }},
                  style:{width:16,height:14,borderRadius:3,border:'none',cursor:'pointer',fontSize:8,padding:0,background:'color-mix(in srgb,'+bar.col+' 30%,transparent)',color:'var(--text)'}},'▶')),
              ce('div',{key:'h2'+bi,style:{position:'absolute',top:2,zIndex:3,display:'flex',gap:2,
                left:'calc(170px + '+((bar.endIdx+1)*COLW)+'px - 40px)'}},
                ce('button',{className:'fl-press fl-bar-h',title:'Конец влево',onClick:function(e){e.stopPropagation(); if(onSetBar){ onSetBar(bar.plan,weekKeys[bar.startIdx],weekKeys[Math.max(bar.startIdx,bar.endIdx-1)]); }},
                  style:{width:16,height:14,borderRadius:3,border:'none',cursor:'pointer',fontSize:8,padding:0,background:'color-mix(in srgb,'+bar.col+' 30%,transparent)',color:'var(--text)'}},'◀'),
                ce('button',{className:'fl-press fl-bar-h',title:'Конец вправо',onClick:function(e){e.stopPropagation(); if(onSetBar){
                    var newEnd;
                    if(bar.endIdx>=NDAYS-1){ var d=new Date(monday.getTime()+(NDAYS-1)*864e5); d.setDate(d.getDate()+1); newEnd=dk(d); }  // за край ленты — следующий день
                    else newEnd=weekKeys[bar.endIdx+1];
                    onSetBar(bar.plan,weekKeys[bar.startIdx],newEnd);
                  }},
                  style:{width:16,height:14,borderRadius:3,border:'none',cursor:'pointer',fontSize:8,padding:0,background:'color-mix(in srgb,'+bar.col+' 30%,transparent)',color:'var(--text)'}},'▶')),
              // фура на этой полосе — только если это ТЕКУЩИЙ рейс (машина одна, не дублируем)
              (function(){
                if(!mt || bi!==activeBarIdx) return null;
                var _sp=splitFleteraPlate(mt.plate||''); var g=geoIndex&&geoIndex[_sp.tractor];
                if(!g) return null;
                var mtForRoute=mt;
                if(!mt.clientId && !mt.clientName && bar.plan){ mtForRoute=Object.assign({},mt,{clientName:bar.plan.clientName}); }
                var rp=routeProgress(mtForRoute,g,clients,orders);
                if(!rp||rp.frac==null) return null;
                var span=bar.endIdx-bar.startIdx+1;
                var posDays;
                if(rp.dir==='back' && bar.backIdx>=0){ var bs=bar.backIdx; posDays=bs + rp.frac*(bar.endIdx-bs+1); }
                else posDays=bar.startIdx + rp.frac*span;
                var goBack=(rp.dir==='back');
                return ce('div',{key:'truck'+bi,style:{position:'absolute',top:'50%',transform:'translate(-50%,-50%)',pointerEvents:'none',zIndex:2,transition:'left .5s ease',textAlign:'center',
                  left:'calc(170px + '+(posDays*COLW)+'px)'}},
                  ce('div',{style:{fontSize:8,fontWeight:800,color:goBack?'var(--cyan)':bar.col,marginBottom:-2,whiteSpace:'nowrap'}},goBack?'◀ обратка':'туда ▶'),
                  ce('div',{style:{fontSize:16,display:'inline-block',transform:goBack?'scaleX(-1)':'none'}},'🚚'));
              })()
            ];
          };
          return ce('div',{key:ri,className:'fl-grid-row',style:{display:'grid',gridTemplateColumns:gridCols,borderBottom:'1px solid var(--border)',position:'relative',minHeight:44}},
            // левая колонка — водитель
            ce('div',{style:{padding:'6px 10px',borderRight:'1px solid var(--border)',display:'flex',flexDirection:'column',justifyContent:'center'}},
              ce('div',{onClick:function(){ if(row.driver) setMarkDrv(row.driver); },title:'Нажмите — выходной / пересадка',
                style:{fontSize:12.5,fontWeight:700,color:'var(--text)',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis',cursor:'pointer'}},row.driver||'—'),
              (function(){ var mk2=(driverMarks||{})[row.driver]; if(!mk2) return null;
                var lbl = mk2.type==='off' ? ('🌴 выходной с '+String(mk2.from||'').slice(5).split('-').reverse().join('.')+(mk2.to?(' по '+String(mk2.to).slice(5).split('-').reverse().join('.')):''))
                                           : ('🔄 на '+(mk2.plate||'')+' с '+String(mk2.from||'').slice(5).split('-').reverse().join('.'));
                return ce('div',{style:{fontSize:9,fontWeight:700,marginTop:2,color:mk2.type==='off'?'var(--amber)':'var(--violet)',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}},lbl); })(),
              (function(){ var gt=mt&&mt.goTo; if(!gt||gt.done) return null;
                return ce('div',{style:{fontSize:9,fontWeight:800,marginTop:2,color:'var(--accent)',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}},
                  '📍 → '+(gt.name||'')+(gt.when==='after_unload'?' после выгр.':' сейчас')+(gt.extraKm>30?(' · +'+gt.extraKm+' км'):'')+' '+(gt.ack?'✅':'⏳')); })(),
              row.plate?ce('div',{style:{fontSize:10,color:'var(--text3)'}},row.plate):null,
              hired&&mt.carrier?ce('div',{style:{fontSize:9,color:'var(--amber)',fontWeight:700}},'🚚 '+mt.carrier):null,
              (function(){
                if(!mt||!mt.status||mt.status==='FREE') return null;
                var lbl=T('st_'+mt.status); if(lbl==='st_'+mt.status) lbl=T('s_'+mt.status); if(lbl==='s_'+mt.status) return null;
                var c=(SC[mt.status])||'var(--text3)';
                // расстояние до цели по GPS (как на карточке в парке)
                var distEl=null;
                var _sp=splitFleteraPlate(mt.plate||''); var g=geoIndex&&geoIndex[_sp.tractor];
                if(g){
                  var mtR=mt; if(!mt.clientId && !mt.clientName && row.plans[0]){ mtR=Object.assign({},mt,{clientName:row.plans[0].clientName}); }
                  var wn=geoWhereNow(mtR,g,clients,orders);
                  if(wn && wn.distKm!=null){
                    distEl=ce('div',{style:{fontSize:8.5,color:'var(--text3)',marginTop:1,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis',maxWidth:150}},
                      (wn.goingBack?'← до ':'до ')+(wn.targetName||'цели')+' '+wn.distKm+' км'+(wn.moving&&wn.etaH?(' (~'+(wn.etaH<1?Math.round(wn.etaH*60)+'м':wn.etaH.toFixed(1)+'ч')+')'):''));
                  }
                }
                return ce('div',null,
                  ce('div',{style:{fontSize:9,fontWeight:800,marginTop:2,color:c}},'● '+lbl),
                  distEl);
              })(),
              // температура рефа — крупно, зелёная в норме / красная вне режима
              (function(){
                if(!mt) return null;
                var cur=mt.currentTemp, req=mt.reqTemp||'';
                // контроль прекращается, если водитель поехал за обраткой БЕЗ температурного режима
                var goingBack=['BACK','BACK_GO','BACK_MOVING'].indexOf(mt.status)>=0;
                var backTemp=(mt.backCargo&&mt.backCargo.temp)||'';
                var controlOff = goingBack && !backTemp;      // обратка без температуры — не контролируем
                if(cur==null && !req) return null;
                if(controlOff){
                  return cur!=null?ce('div',{style:{fontSize:11,marginTop:3,color:'var(--text3)',whiteSpace:'nowrap'}},
                    '🌡 '+(cur>0?'+':'')+cur+'° · без режима'):null;
                }
                var useReq = goingBack&&backTemp ? backTemp : req;   // на обратке с режимом — её режим
                var okT=true;
                if(cur!=null && useReq){
                  var nums=String(useReq).replace(',','.').match(/-?\d+(?:\.\d+)?/g);
                  if(nums&&nums.length){ var lo=Math.min.apply(null,nums.map(Number)), hi=Math.max.apply(null,nums.map(Number));
                    okT = (lo===hi) ? (cur<=hi+3) : (cur>=lo-1 && cur<=hi+1); }
                }
                return ce('div',{style:{marginTop:3,lineHeight:1.15}},
                  cur!=null?ce('div',{style:{fontSize:15,fontWeight:800,color:okT?'var(--green)':'var(--red)',whiteSpace:'nowrap'}},
                    '🌡 '+(cur>0?'+':'')+cur+'°'):null,
                  useReq?ce('div',{style:{fontSize:9.5,color:'var(--text3)',whiteSpace:'nowrap'}},'нужно '+useReq):null);
              })()),
            // ячейки дней
            days.map(function(d,di){ var kk=dk(d);
              var cellPlan=row.plans.find(function(p){ return (p.loadDate||p.date)===kk; });
              return ce('div',{key:di,onClick:function(){ if(cellPlan) setEdit(cellPlan); else setAddCell({driver:row.driver,dateKey:kk}); },
                style:{borderRight:'1px solid var(--border)',cursor:'pointer',position:'relative',minHeight:44,
                background:dk(d)===todayK?'color-mix(in srgb,var(--accent) 4%,transparent)':'transparent'}},
                cellPlan?ce('div',{style:{position:'absolute',inset:'4px',borderRadius:5,padding:'3px 5px',fontSize:9.5,lineHeight:1.15,overflow:'hidden',
                  background:'color-mix(in srgb,'+clientColor(cellPlan.clientName)+' 14%,transparent)',border:'1px solid color-mix(in srgb,'+clientColor(cellPlan.clientName)+' 35%,transparent)',color:'var(--text2)'}},
                  ce('div',{style:{fontWeight:700,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis',color:clientColor(cellPlan.clientName)}},cellPlan.clientName||''),
                  cellPlan.clientName2?ce('div',{style:{fontWeight:700,fontSize:9,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis',color:clientColor(cellPlan.clientName2)}},'+ '+cellPlan.clientName2):null,
                  cellPlan.back?ce('div',{style:{color:'var(--cyan)',fontWeight:700,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}},'↩ '+cellPlan.back):null,
                  cellPlan.launched?ce('div',{style:{fontSize:8.5,color:'var(--green)',fontWeight:800,marginTop:1}},'✓ в рейсе')
                    :ce('button',{className:'fl-press',onClick:function(e){ e.stopPropagation(); if(onLaunch&&window.confirm('Отправить в рейс: '+(cellPlan.truckDriver||'')+' → '+(cellPlan.clientName||'')+'?')) onLaunch(cellPlan); },
                      style:{position:'absolute',bottom:2,right:2,padding:'1px 6px',borderRadius:4,border:'none',cursor:'pointer',fontFamily:'inherit',fontSize:8.5,fontWeight:800,color:'#fff',background:'var(--accent)'}},'▶ рейс')):null); }),
            // полосы рейсов (каждый рейс — своя полоса, обрыв после обратки)
            bars.map(function(b,bi){ return renderOneBar(b,bi); }),
            null); };
          var isHiredR=function(row){ var t=(trucks||[]).find(function(x){ return (x.driver||'').trim().toLowerCase()===(row.driver||'').trim().toLowerCase(); }); return !!(t&&t.isHired); };
          var ownR=driverRows.filter(function(r){ return !isHiredR(r); });   // привлечённые в x2 не показываем
          // разграничение по логистам: кто не «видит весь транспорт» — видит только свои машины
          var meLog=(logists||[]).find(function(l){ return l.name===role; });
          var seeAll=isBoss || (meLog&&meLog.seeAll) || !role || LOGISTS.indexOf(role)<0;
          if(!seeAll){
            ownR=ownR.filter(function(r){
              var t=(trucks||[]).find(function(x){ return (x.driver||'').trim().toLowerCase()===(r.driver||'').trim().toLowerCase() || (r.plate&&x.plate===r.plate); });
              return t && t.logist===role;
            });
          }
          if(drvQ.trim()){ var _q=drvQ.trim().toLowerCase(); ownR=ownR.filter(function(r){ return (r.driver||'').toLowerCase().indexOf(_q)>=0 || (r.plate||'').toLowerCase().indexOf(_q)>=0; }); }
          return ce('div',null,
            ownR.map(function(r,i){ return renderDriverRow(r,'o'+i); }));
        })())),
    // ── банк обраток по дням недели (как у них внизу таблицы) ──
    ce('div',{className:'fl-no-print',style:{marginTop:16}},
      ce('div',{style:{fontSize:13,fontWeight:800,marginBottom:8,color:'var(--cyan)'}},'↩ Обратки по дням — заметка для себя'),
      ce('div',{style:{overflowX:'auto',border:'1px solid var(--border)',borderRadius:12}},
        ce('div',{style:{minWidth:(NDAYS*COLW)+'px',display:'grid',gridTemplateColumns:'repeat('+NDAYS+','+COLW+'px)'}},
          days.map(function(d,di){ var kk=dk(d);
            var notes=(gridBacks&&gridBacks[kk])||[];
            // старые заметки, что раньше писались как обратки-order на этот день — тоже показываем
            var oldOrders=(orders||[]).filter(function(o){ return !o.assigned && o.date && String(o.date).slice(0,10)===kk; });
            var isT=kk===todayK;
            return ce('div',{key:di,style:{borderRight:di<NDAYS-1?'1px solid var(--border)':'none',minHeight:80}},
              ce('div',{style:{padding:'6px 8px',fontSize:11,fontWeight:800,textAlign:'center',borderBottom:'1px solid var(--border)',
                color:isT?'var(--accent)':'var(--text3)',background:isT?'color-mix(in srgb,var(--accent) 8%,transparent)':'var(--bg2)'}},
                wd[d.getDay()]+' '+d.getDate()),
              ce('div',{style:{padding:'6px 8px',display:'grid',gap:4}},
                oldOrders.map(function(o,oi){
                  return ce('div',{key:'o'+oi,style:{fontSize:10.5,lineHeight:1.25,padding:'3px 6px',borderRadius:5,display:'flex',alignItems:'center',gap:4,
                    background:'color-mix(in srgb,var(--cyan) 8%,transparent)',border:'1px solid color-mix(in srgb,var(--cyan) 25%,transparent)',color:'var(--text2)'}},
                    ce('span',{style:{flex:1,color:'var(--text)'}},o.client||o.cargo||'—'),
                    ce('button',{className:'fl-press',title:'Удалить',onClick:function(){ if(onDelOrder) onDelOrder(o.id); },
                      style:{width:16,height:16,borderRadius:4,border:'none',cursor:'pointer',fontSize:10,padding:0,background:'transparent',color:'var(--text3)'}},'✕')); }),
                notes.map(function(txt,ni){
                  return ce('div',{key:ni,style:{fontSize:10.5,lineHeight:1.3,padding:'4px 6px',borderRadius:5,display:'flex',alignItems:'flex-start',gap:4,
                    background:'color-mix(in srgb,var(--cyan) 8%,transparent)',border:'1px solid color-mix(in srgb,var(--cyan) 25%,transparent)',color:'var(--text2)'}},
                    ce('span',{style:{flex:1,color:'var(--text)',wordBreak:'break-word'}},txt),
                    ce('button',{className:'fl-press',title:'Удалить',onClick:function(){ if(onSetGridBack) onSetGridBack(kk, notes.filter(function(_,i){return i!==ni;})); },
                      style:{width:16,height:16,borderRadius:4,border:'none',cursor:'pointer',fontSize:10,padding:0,background:'transparent',color:'var(--text3)'}},'✕')); }),
                ce('button',{className:'fl-press',onClick:function(){ var v=window.prompt('Заметка-обратка на '+wd[d.getDay()]+' '+d.getDate()+':',''); if(v&&v.trim()&&onSetGridBack) onSetGridBack(kk, notes.concat([v.trim()])); },
                  style:{fontSize:10,padding:'3px 6px',borderRadius:5,cursor:'pointer',fontFamily:'inherit',fontWeight:700,
                  border:'1px dashed color-mix(in srgb,var(--cyan) 40%,transparent)',background:'transparent',color:'var(--cyan)'}},'+ заметка'))); })))),
    // ── ТУДАТКИ по дням (загрузки туда) ──
    ce('div',{className:'fl-no-print',style:{marginTop:16}},
      ce('div',{style:{fontSize:13,fontWeight:800,marginBottom:8,color:'var(--green)'}},'→ Тудатки по дням — заметка для себя'),
      ce('div',{style:{overflowX:'auto',border:'1px solid var(--border)',borderRadius:12}},
        ce('div',{style:{minWidth:(NDAYS*COLW)+'px',display:'grid',gridTemplateColumns:'repeat('+NDAYS+','+COLW+'px)'}},
          days.map(function(d,di){ var kk=dk(d);
            var tnotes=(gridTuda&&gridTuda[kk])||[];
            var isT=kk===todayK;
            return ce('div',{key:'t'+di,style:{borderRight:di<NDAYS-1?'1px solid var(--border)':'none',minHeight:80}},
              ce('div',{style:{padding:'6px 8px',fontSize:11,fontWeight:800,textAlign:'center',borderBottom:'1px solid var(--border)',
                color:isT?'var(--accent)':'var(--text3)',background:isT?'color-mix(in srgb,var(--accent) 8%,transparent)':'var(--bg2)'}},
                wd[d.getDay()]+' '+d.getDate()),
              ce('div',{style:{padding:'6px 8px',display:'grid',gap:4}},
                tnotes.map(function(txt,ni){
                  return ce('div',{key:ni,style:{fontSize:10.5,lineHeight:1.3,padding:'4px 6px',borderRadius:5,display:'flex',alignItems:'flex-start',gap:4,
                    background:'color-mix(in srgb,var(--green) 8%,transparent)',border:'1px solid color-mix(in srgb,var(--green) 25%,transparent)',color:'var(--text2)'}},
                    ce('span',{style:{flex:1,color:'var(--text)',wordBreak:'break-word'}},txt),
                    ce('button',{className:'fl-press',title:'Удалить',onClick:function(){ if(onSetGridTuda) onSetGridTuda(kk, tnotes.filter(function(_,i){return i!==ni;})); },
                      style:{width:16,height:16,borderRadius:4,border:'none',cursor:'pointer',fontSize:10,padding:0,background:'transparent',color:'var(--text3)',flexShrink:0}},'✕')); }),
                ce('button',{className:'fl-press',onClick:function(){ var v=window.prompt('Тудатка на '+wd[d.getDay()]+' '+d.getDate()+':',''); if(v&&v.trim()&&onSetGridTuda) onSetGridTuda(kk, tnotes.concat([v.trim()])); },
                  style:{fontSize:10,padding:'3px 6px',borderRadius:5,cursor:'pointer',fontFamily:'inherit',fontWeight:700,
                  border:'1px dashed color-mix(in srgb,var(--green) 40%,transparent)',background:'transparent',color:'var(--green)'}},'+ заметка'))); }))))
  );
}
// ═══════════ ЛИСТ ПЛАНИРОВАНИЯ (как бумажный лист логистов) ═══════════
// ═══ «Кто ближе» — свои машины по ДОРОЖНЫМ километрам до точки загрузки ═══
function NearestTrucks({target,targetName,trucks,geoIndex,onPick}){
  const [st,setSt]=useState({loading:true,rows:[],road:true});
  useEffect(function(){
    var cand=[];
    (trucks||[]).forEach(function(t){
      if(!t||t.isHired||!(t.driver||'').trim()) return;
      var sp=splitFleteraPlate(t.plate||''); var g=geoIndex&&geoIndex[sp.tractor];
      if(!g||g.lat==null) return;
      cand.push({t:t,p:[g.lat,g.lon],at:g.at});
    });
    if(!cand.length){ setSt({loading:false,rows:[],road:true}); return; }
    var alive=true;
    roadDistances(cand.map(function(c){return c.p;}),target).then(function(r){
      if(!alive) return;
      var rows=cand.map(function(c,i){ return {t:c.t,km:r.km[i],min:r.min[i],at:c.at}; })
        .filter(function(x){ return x.km!=null; })
        .sort(function(a,b){ return a.km-b.km; });
      setSt({loading:false,rows:rows,road:r.road});
    });
    return function(){ alive=false; };
  },[target&&target[0],target&&target[1]]);
  var stLbl=function(s){ if(!s||s==='FREE') return {t:'свободна',c:'var(--green)'};
    var l=T('st_'+s); if(l==='st_'+s) l=T('s_'+s); if(l==='s_'+s) l=s; return {t:l,c:(SC[s]||'var(--text3)')}; };
  var hrs=function(m){ if(m==null) return ''; var h=Math.floor(m/60), mm=m%60; return h?(h+' ч '+(mm?mm+' мин':'')):(mm+' мин'); };
  return ce('div',{style:{marginTop:8,border:'1px solid var(--border2)',borderRadius:11,overflow:'hidden'}},
    ce('div',{style:{padding:'8px 11px',background:'var(--bg3)',fontSize:12,fontWeight:800,color:'var(--text2)',display:'flex',justifyContent:'space-between',gap:8}},
      ce('span',null,'🧭 Кто ближе к «'+(targetName||'загрузке')+'»'),
      st.road?ce('span',{style:{color:'var(--green)',fontWeight:700}},'по дорогам'):ce('span',{style:{color:'var(--amber)',fontWeight:700}},'≈ по прямой (сервис маршрутов недоступен)')),
    st.loading?ce('div',{style:{padding:12,fontSize:12.5,color:'var(--text3)'}},'Считаю маршруты…'):
    !st.rows.length?ce('div',{style:{padding:12,fontSize:12.5,color:'var(--text3)'}},'Нет машин с GPS'):
    ce('div',{style:{maxHeight:260,overflowY:'auto'}},
      st.rows.map(function(x,i){ var s=stLbl(x.t.status);
        return ce('div',{key:x.t.id,onClick:function(){ onPick(x.t); },className:'fl-press',
          style:{display:'flex',alignItems:'center',gap:9,padding:'8px 11px',cursor:'pointer',borderTop:i?'1px solid var(--border)':'none',
          background:i===0?'color-mix(in srgb,var(--green) 8%,transparent)':'transparent'}},
          ce('span',{style:{fontSize:15,fontWeight:800,minWidth:62,color:i===0?'var(--green)':'var(--text)'}},x.km+' км'),
          ce('div',{style:{flex:1,minWidth:0}},
            ce('div',{style:{fontSize:13,fontWeight:700,color:'var(--text)',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}},x.t.driver),
            ce('div',{style:{fontSize:11,color:'var(--text3)'}},(x.t.plate||'')+(x.min!=null?(' · ~'+hrs(x.min)):''))),
          ce('span',{style:{fontSize:10.5,fontWeight:700,color:s.c,whiteSpace:'nowrap'}},s.t)); })));
}
function PlanRowEditor({row,clients,orders,trucks,dateKey,onSave,onCancel,onDelete,geoIndex}){
  row=row||{};
  const [clientName,setClientName]=useState(row.clientName||'');
  const [clientName2,setClientName2]=useState(row.clientName2||'');
  const [driver,setDriver]=useState(row.truckDriver||'');
  const [plate,setPlate]=useState(row.truckPlate||'');
  const [back,setBack]=useState(row.back||'');
  const [note,setNote]=useState(row.note||'');
  const [freeAt,setFreeAt]=useState(row.freeAt?String(row.freeAt).slice(0,16):'');
  const [confirmed,setConfirmed]=useState(!!row.confirmed);
  const [showNear,setShowNear]=useState(false);
  // подстановка машины по водителю
  const pickDriver=function(name){
    setDriver(name);
    var t=(trucks||[]).find(function(x){ return (x.driver||'')===name; });
    if(t) setPlate(t.plate||'');
  };
  const bad=!clientName.trim() && !back.trim();   // достаточно клиента ИЛИ обратки
  return ce(Modal,{title:(row.id?'✎ Строка плана':'➕ Новая строка')+' · '+fmtD(row.loadDate||dateKey),onClose:onCancel},
    ce('div',{style:{display:'grid',gap:10}},
      fld('Клиент / куда',ce('input',{value:clientName,onChange:e=>setClientName(e.target.value),list:'fl-plan-clients',placeholder:'Серволюкс, Санта, Питер…',style:mecInp})),
      ce('datalist',{id:'fl-plan-clients'},(clients||[]).map(function(c,i){ return ce('option',{key:i,value:c.name}); })),
      fld('2-й клиент / загрузка (если есть)',ce('input',{value:clientName2,onChange:e=>setClientName2(e.target.value),list:'fl-plan-clients',placeholder:'вторая загрузка туда — необязательно',style:mecInp})),
      (function(){
        var cl=(clients||[]).find(function(c){ return c.name===clientName.trim(); });
        var lg=cl&&normGeo(cl.loadGeo);
        if(!lg) return null;
        return ce('div',null,
          ce('button',{type:'button',className:'fl-press',onClick:function(){ setShowNear(!showNear); },
            style:{width:'100%',padding:'9px',borderRadius:10,cursor:'pointer',fontFamily:'inherit',fontSize:13,fontWeight:800,
            border:'1px solid color-mix(in srgb,var(--green) 45%,transparent)',background:'color-mix(in srgb,var(--green) 10%,transparent)',color:'var(--green)'}},
            showNear?'▲ скрыть':'🧭 Кто ближе к загрузке'),
          showNear?ce(NearestTrucks,{target:[lg.lat,lg.lon],targetName:cl.name,trucks:trucks,geoIndex:geoIndex,
            onPick:function(t){ setDriver(t.driver||''); setPlate(t.plate||''); setShowNear(false); }}):null);
      })(),
      ce('div',{style:{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}},
        fld('Водитель',ce('input',{value:driver,onChange:e=>pickDriver(e.target.value),list:'fl-plan-drivers',placeholder:'фамилия',style:mecInp})),
        fld('Машина',ce('input',{value:plate,onChange:e=>setPlate(e.target.value),placeholder:'гос.номер',style:mecInp}))),
      ce('datalist',{id:'fl-plan-drivers'},(trucks||[]).filter(function(t){return t.driver;}).map(function(t,i){ return ce('option',{key:i,value:t.driver}); })),
      fld('Обратка (что назад / куда)',ce('input',{value:back,onChange:e=>setBack(e.target.value),list:'fl-plan-orders',placeholder:'выберите из списка или впишите…',style:mecInp})),
      ce('datalist',{id:'fl-plan-orders'},(orders||[]).filter(function(o){return !o.assigned;}).map(function(o,i){ return ce('option',{key:i,value:(o.client||'')+(o.cargo?(' · '+o.cargo):'')+(o.to?(' → '+o.to):'')}); })),
      fld('Заметка',ce('input',{value:note,onChange:e=>setNote(e.target.value),placeholder:'напр. Жук 07.08 в обед выгрузиться в Минске',style:mecInp})),
      fld('🕐 Освободится (дата и время)',ce('input',{type:'datetime-local',value:freeAt,onChange:e=>setFreeAt(e.target.value),style:mecInp})),
      ce('label',{style:{display:'flex',alignItems:'center',gap:8,fontSize:13,color:'var(--text2)',cursor:'pointer'}},
        ce('input',{type:'checkbox',checked:confirmed,onChange:e=>setConfirmed(e.target.checked)}),
        'Подтверждён (водитель зелёным)')),
    ce('div',{style:{display:'flex',gap:9,marginTop:16}},
      ce(Btn,{onClick:onCancel,color:'var(--text3)',wide:true},'Отмена'),
      (row&&row.id&&onDelete)?ce('button',{onClick:function(){ if(window.confirm('Удалить эту строку плана?')) onDelete(row.id); },className:'fl-press',
        style:{padding:'12px 16px',border:'1px solid color-mix(in srgb,var(--red) 45%,transparent)',borderRadius:11,background:'transparent',color:'var(--red)',fontSize:13,fontWeight:800,cursor:'pointer',fontFamily:'inherit'}},'🗑 Удалить'):null,
      ce('button',{onClick:function(){ if(!bad) onSave(Object.assign({},row,{loadDate:row.loadDate||dateKey,clientName:clientName.trim(),clientName2:clientName2.trim(),truckDriver:driver.trim(),truckPlate:plate.trim(),back:back.trim(),note:note.trim(),freeAt:freeAt?new Date(freeAt).toISOString():'',confirmed:confirmed})); },
        disabled:bad,className:'fl-press',style:{flex:1,padding:'12px',border:'none',borderRadius:11,color:'#fff',fontSize:14,fontWeight:800,cursor:bad?'default':'pointer',
        background:bad?'var(--bg3)':'linear-gradient(135deg,var(--accent),var(--accent2))',opacity:bad?0.5:1}},'Сохранить')));
}
function PlanBoard({plans,clients,orders,trucks,onSavePlan,onDelPlan,onLaunch,dayNotes,onSaveDayNote,onCopyDriver}){
  const base=new Date(); base.setHours(0,0,0,0);
  const [edit,setEdit]=useState(null);
  ...
  const todayK=dk(new Date());
  // собираем ключи дней (строки YYYY-MM-DD): сегодня+11 вперёд + ВСЕ прошлые дни где есть планы
  var keySet={};
  for(var i=0;i<12;i++){ keySet[dk(new Date(base.getTime()+i*864e5))]=1; }
  (plans||[]).forEach(function(p){ var kk=p.loadDate||p.date; if(kk) keySet[kk]=1; });   // любой день с планом (прошлый или будущий)
  var allKeys=Object.keys(keySet).sort();   // строки сортируются хронологически
  const mkDate=function(kk){ var pr=kk.split('-'); return new Date(Number(pr[0]),Number(pr[1])-1,Number(pr[2])); };
  var allDays=allKeys.map(mkDate);
  const pastKeys=allKeys.filter(function(kk){ return kk<todayK; });
  const pastDays=pastKeys.map(mkDate);
  const futureDays=allKeys.filter(function(kk){ return kk>=todayK; }).map(mkDate);
  const days=showPast?allDays:futureDays;
  const clientColor=function(name){ var c=(clients||[]).find(function(x){ return x.name===name; }); return (c&&c.color)||'var(--red)'; };
  const truckOf=function(p){
    var n=function(s){return String(s||'').toUpperCase().replace(/[\s\-\/]/g,'');};
    var dn=String(p.truckDriver||'').trim().toLowerCase();
    // 1) точное совпадение по водителю (фамилия уникальнее номера)
    if(dn){ var byDriver=(trucks||[]).find(function(x){ return (x.driver||'').trim().toLowerCase()===dn; }); if(byDriver) return byDriver; }
    // 2) потом по номеру
    if(p.truckPlate){ var byPlate=(trucks||[]).find(function(x){ return n(x.plate)===n(p.truckPlate); }); if(byPlate) return byPlate; }
    // 3) частичное по водителю (фамилия — часть ФИО)
    if(dn){ var partial=(trucks||[]).find(function(x){ return (x.driver||'').trim().toLowerCase().indexOf(dn)>=0; }); if(partial) return partial; }
    return null;
  };
  const isHiredRow=function(p){ var t=truckOf(p); return !!(t&&t.isHired); };
  const byDay=function(k){ return (plans||[]).filter(function(p){ return (p.loadDate||p.date)===k; }); };
  return ce('div',{style:{padding:'14px',maxWidth:1100,margin:'0 auto'}},
    edit?ce(PlanRowEditor,{row:edit,clients:clients,orders:orders,trucks:trucks,dateKey:edit.loadDate,
      onSave:function(r){ onSavePlan(r); setEdit(null); },onCancel:function(){setEdit(null);},
      onDelete:function(id){ if(onDelPlan) onDelPlan(id); setEdit(null); }}):null,
    addDate?ce(PlanRowEditor,{clients:clients,orders:orders,trucks:trucks,dateKey:addDate,
      onSave:function(r){ onSavePlan(r); setAddDate(null); },onCancel:function(){setAddDate(null);}}):null,
    ce('div',{style:{display:'flex',alignItems:'center',gap:10,marginBottom:14,flexWrap:'wrap'}},
      ce('div',{style:{fontWeight:800,fontSize:19}},'🚚 Привлечённый транспорт'),
      ce('div',{style:{fontSize:12,color:'var(--text3)'}},'наёмные машины по дням'),
      pastDays.length?ce('button',{className:'fl-press',onClick:function(){setShowPast(!showPast);},style:{marginLeft:'auto',padding:'6px 13px',borderRadius:8,cursor:'pointer',fontFamily:'inherit',fontSize:12,fontWeight:700,
        border:'1px solid var(--border2)',background:'var(--bg2)',color:'var(--text3)'}},showPast?'▲ скрыть прошедшие ('+pastDays.length+')':'▼ показать прошедшие ('+pastDays.length+')'):null),
    days.map(function(d){
      var k=dk(d), rows=byDay(k).slice().sort(function(a,b){
        var ca=(a.clientName||'').toLowerCase(), cb=(b.clientName||'').toLowerCase();
        if(ca<cb) return -1; if(ca>cb) return 1;   // группируем по клиенту
        return (a.truckDriver||'').localeCompare(b.truckDriver||'');  // внутри клиента — по водителю
      }), isToday=k===todayK;
      // незакрытые строки на сегодня и прошлое — подсветить
      var overdue=k<=todayK;
      return ce('div',{key:k,style:{marginBottom:18}},
        ce('div',{style:{display:'flex',alignItems:'center',gap:10,marginBottom:8,paddingBottom:6,borderBottom:'2px solid '+(isToday?'var(--accent)':'var(--border)')}},
          ce('span',{style:{fontWeight:800,fontSize:15,color:isToday?'var(--accent)':'var(--text)'}},wd[d.getDay()]+' '+String(d.getDate()).padStart(2,'0')+'.'+String(d.getMonth()+1).padStart(2,'0')),
          isToday?ce('span',{style:{fontSize:11,padding:'2px 9px',borderRadius:999,background:'color-mix(in srgb,var(--accent) 18%,transparent)',color:'var(--accent)',fontWeight:800}},'сегодня'):null,
          rows.length?ce('span',{style:{fontSize:12,color:'var(--text3)'}},rows.length+' рейс.'):null,
          ce('button',{className:'fl-press',onClick:function(){setAddDate(k);},style:{marginLeft:'auto',padding:'5px 12px',borderRadius:8,cursor:'pointer',fontFamily:'inherit',fontSize:12,fontWeight:800,
            border:'1px solid color-mix(in srgb,var(--green) 45%,transparent)',background:'color-mix(in srgb,var(--green) 12%,transparent)',color:'var(--green)'}},'＋ строка')),
        ce('div',{onClick:function(){ var cur=(dayNotes&&dayNotes[k])||''; var v=window.prompt('Заметка на '+wd[d.getDay()]+' '+String(d.getDate()).padStart(2,'0')+'.'+String(d.getMonth()+1).padStart(2,'0')+' (например: на сервис — АТ 4774, ВВ 2700):',cur); if(v!==null&&onSaveDayNote) onSaveDayNote(k,v.trim()); },
          style:{cursor:'pointer',marginBottom:8,padding:'7px 11px',borderRadius:9,fontSize:12,
          background:(dayNotes&&dayNotes[k])?'color-mix(in srgb,var(--amber) 10%,transparent)':'var(--bg3)',
          border:'1px solid '+((dayNotes&&dayNotes[k])?'color-mix(in srgb,var(--amber) 40%,transparent)':'var(--border)')}},
          (dayNotes&&dayNotes[k])?ce('span',{style:{color:'var(--text)'}},'🔧 '+dayNotes[k]):ce('span',{style:{color:'var(--text3)'}},'🔧 Заметка на день (сервис и т.п.) — нажмите, чтобы добавить')),
        (function(){
          var renderRow=function(p){
            var col=clientColor(p.clientName);
            var launched=!!p.launched;
            var mt=truckOf(p);
            var hired=mt&&mt.isHired;
            return ce('div',{key:p.id,className:'fl-row-hover',style:{display:'flex',alignItems:'flex-start',gap:12,padding:'9px 12px',borderRadius:9,
              background:launched?'var(--bg2)':'var(--bg2)',opacity:launched?0.62:1,   // отправленные — приглушены, невыполненные ярче
              border:'1px solid var(--border)',
              borderLeft:'3px solid '+(launched?'var(--border2)':col)}},
              // ── слева: освобождение ──
              ce('div',{style:{minWidth:60,flexShrink:0,textAlign:'center',lineHeight:1.15}},
                p.freeAt?[ce('div',{key:'t',style:{fontSize:13,fontWeight:800,color:'var(--text)'}},new Date(p.freeAt).toLocaleTimeString('ru',{hour:'2-digit',minute:'2-digit'})),
                          ce('div',{key:'d',style:{fontSize:10,color:'var(--text3)'}},new Date(p.freeAt).toLocaleDateString('ru',{day:'2-digit',month:'2-digit'}))]
                        :ce('div',{style:{fontSize:10,color:'var(--text3)',fontStyle:'italic'}},'—')),
              // ── центр: клиент + водитель ──
              ce('div',{style:{flex:1,minWidth:0}},
                ce('div',{style:{display:'flex',alignItems:'center',gap:8,flexWrap:'wrap'}},
                  ce('span',{style:{fontWeight:800,fontSize:13.5,color:launched?'var(--text2)':col}},p.clientName||'—'),
                  hired?ce('span',{style:{fontSize:9.5,fontWeight:700,padding:'1px 6px',borderRadius:5,whiteSpace:'nowrap',
                    background:'var(--bg3)',color:'var(--text3)',border:'1px solid var(--border2)'}},'привл.'+(mt.carrier?(' '+mt.carrier):'')):null,
                  // статус водителя (по машине этой строки) — виден когда машина в рейсе
                  (function(){
                    if(!mt) return null;
                    var st=mt.status; if(!st||st==='FREE') return null;
                    var lbl=T('st_'+st); if(lbl==='st_'+st) lbl=T('s_'+st); if(lbl==='s_'+st) return null;
                    var c=(SC[st])||'var(--text3)';
                    return ce('span',{title:'статус водителя',style:{fontSize:9.5,fontWeight:800,padding:'1px 7px',borderRadius:5,whiteSpace:'nowrap',
                      background:'color-mix(in srgb,'+c+' 16%,transparent)',color:c}},lbl);
                  })()),
                ce('div',{style:{fontSize:12,color:p.truckDriver?'var(--text2)':'var(--text3)',marginTop:1,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}},
                  p.truckDriver?(p.truckDriver+(p.truckPlate?(' · '+p.truckPlate):'')):'нет водителя'),
                p.back?ce('div',{style:{fontSize:11,color:'var(--cyan)',marginTop:1,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis',maxWidth:'100%'},title:p.back},'↩ обратка: '+p.back)
                      :ce('div',{style:{fontSize:10.5,color:'var(--text3)',marginTop:1,fontStyle:'italic'}},'обратки нет — освобождение неизвестно'),
                p.note?ce('div',{style:{fontSize:11,color:'var(--text3)',marginTop:1}},'📝 '+p.note):null,
                // ── действия отдельным рядом (помещаются в узкой колонке) ──
                launched?
                  ce('div',{style:{display:'flex',alignItems:'center',gap:8,marginTop:7}},
                    ce('span',{style:{fontSize:11,color:'var(--green)',fontWeight:800}},'✓ в рейсе'),
                    ce('button',{className:'fl-press',onClick:function(){setEdit(p);},title:'Изменить и переназначить',style:{width:26,height:26,borderRadius:6,border:'1px solid var(--border2)',background:'var(--bg3)',color:'var(--text3)',cursor:'pointer',fontSize:12}},'✎'),
                    ce('button',{className:'fl-press',onClick:function(){ if(window.confirm('Убрать строку из плана? Рейс в «Рейсах» останется.')) onDelPlan(p.id); },title:'Убрать из плана',style:{width:26,height:26,borderRadius:6,border:'1px solid var(--border2)',background:'transparent',color:'var(--text3)',cursor:'pointer',fontSize:12}},'✕')):
                  ce('div',{style:{display:'flex',alignItems:'center',gap:6,marginTop:7}},
                    ce('button',{className:'fl-press',onClick:function(){ if(window.confirm('Отправить в рейс: '+(p.truckDriver||p.truckPlate||'машину')+' → '+(p.clientName||'')+'?')) onLaunch(p); },
                      style:{padding:'6px 13px',borderRadius:8,border:'none',cursor:'pointer',fontFamily:'inherit',fontSize:12,fontWeight:800,color:'#fff',background:'var(--accent)'}},'▶ В рейс'),
                    ce('button',{className:'fl-press',onClick:function(){setEdit(p);},title:'Изменить',style:{width:26,height:26,borderRadius:6,border:'1px solid var(--border2)',background:'var(--bg3)',color:'var(--text3)',cursor:'pointer',fontSize:12}},'✎'),
                    ce('button',{className:'fl-press',onClick:function(){ if(window.confirm('Удалить строку?')) onDelPlan(p.id); },title:'Удалить',style:{width:26,height:26,borderRadius:6,border:'1px solid var(--border2)',background:'transparent',color:'var(--text3)',cursor:'pointer',fontSize:12}},'🗑'),
                    p.driverSent?ce('span',{title:'данные водителю отправлены',style:{fontSize:13,color:'var(--green)',marginLeft:'auto'}},'📩'):null))); };
          // раздел «Привлечённый транспорт» — показываем ТОЛЬКО привлечённые машины
          var hired=rows.filter(function(p){ return isHiredRow(p); });
          var noBack=hired.filter(function(p){ return !p.back && (p.truckDriver||p.truckPlate); });
          if(hired.length===0) return ce('div',{style:{fontSize:12.5,color:'var(--text3)',padding:'4px 2px 8px',fontStyle:'italic'}},'— привлечённых машин на этот день нет');
          return ce('div',null,
            noBack.length?ce('div',{style:{marginBottom:10,padding:'8px 11px',borderRadius:9,maxWidth:520,
              background:'color-mix(in srgb,var(--amber) 7%,transparent)',border:'1px solid color-mix(in srgb,var(--amber) 30%,transparent)'}},
              ce('div',{style:{fontSize:10.5,fontWeight:800,color:'var(--amber)',marginBottom:4,textTransform:'uppercase',letterSpacing:.4}},'⚠ Без обратки ('+noBack.length+')'),
              ce('div',{style:{display:'flex',flexWrap:'wrap',gap:'3px 10px'}},
                noBack.map(function(p,i){ return ce('span',{key:i,style:{fontSize:11.5,color:'var(--text2)'}},
                  (p.truckDriver||p.truckPlate)+' · '+(p.clientName||'')); }))):null,
            ce('div',{style:{display:'grid',gap:5}},hired.map(renderRow)));
        })()); }));
}
...
function BossMechanic({tractors,trailers,acts,tires,bags,akb,kasko}){
  ...
  (trailers||[]).forEach(function(tr){
    var pt=(tractors||[]).find(function(x){return x.plate===tr.pairedTractor;}); var odo=trailerMileage(tr,tractors);
    ...
    items.push({plate:tr.plate,cat:'Санпаспорт',s:dateStatus(tr.sanitary),o:tr,kind:'trailer'});
  ...
  return ce('div',null,
    ...
    ce('div',{className:'fl-subtabs',style:{display:'inline-flex',gap:3,marginBottom:12,background:'var(--bg2)',border:'1px solid var(--border)',borderRadius:11,padding:3,flexWrap:'wrap'}},
...
function BossHr({drivers,vacations,waybills,history,trucks,plans}){
  const tnow=Date.now();
  ...
  const _isHiredB=function(name){ var t=(trucks||[]).find(function(x){ return (x.driver||'').trim().toLowerCase()===(name||'').trim().toLowerCase(); }); return !!(t&&t.isHired); };
  const wbBad=(drivers||[]).filter(function(d){ return !_isHiredB(d.name); }).map(function(d){ var w=(waybills||[]).find(function(x){return (x.driverName||'').trim()===(d.name||'').trim();});
    var issued=(w&&w.issued)||d.workStart||''; var since=dOnly(issued)||0;
    var n=countTrips(d.name, issued, history, plans);
...
function BossPanel(props){
  ...
  return ce('div',{style:{padding:'16px',maxWidth:1700,margin:'0 auto'}},
    ...
    sec==='hr'?ce(BossHr,{drivers:props.hrDrivers,vacations:props.hrVacations,waybills:props.hrWaybills,history:props.history,trucks:props.trucks,plans:props.plans}):null,
    ...
    sec==='temp'?ce(TempTab,{trucks:props.trucks,clients:props.clients,binds:props.binds,onBind:props.onBind,readOnly:true}):null);
}
function MechBase({trucks,tractors,trailers}){
  const moving=(trucks||[]).filter(function(t){ return MOVING.includes(t.status); });
  const findT=function(plate){ var tp=splitFleteraPlate(plate).tractor; return findMecTractor(tractors,tp); };
...
function MechanicTab(props){
  ...
  return ce('div',{style:{padding:'16px',maxWidth:1700,margin:'0 auto'}},
    ce('div',{className:'fl-subtabs',style:{display:'inline-flex',gap:3,marginBottom:16,background:'var(--bg2)',border:'1px solid var(--border)',borderRadius:11,padding:3,flexWrap:'wrap'}},
      tabs.map(function(v){ var on=view===v[0];
        return ce('button',{key:v[0],className:'fl-press',onClick:function(){setView(v[0]);},style:{padding:'8px 16px',borderRadius:9,cursor:'pointer',fontFamily:'inherit',fontSize:13,fontWeight:800,border:'none',
          background:on?'linear-gradient(135deg,var(--cyan),var(--accent))':'transparent',color:on?'#fff':'var(--text3)'}},v[1]); })),
    view==='dash'&&ce(MechDash,{tractors:tractors,trailers:trailers,acts:acts,trucks:props.trucks||[],onJump:setView}),
...
function HrDash({drivers,vacations,waybills,history,trucks,plans,onJump}){
  ...
  const tripsSince=function(name,sinceISO){ return countTrips(name,sinceISO,history,plans); };
  const _tripsSinceOld=function(name,sinceISO){ var since=dOnly(sinceISO)||0;
    return (history||[]).filter(function(e){ return e&&e.action==='dispatch'&&sameDriver(e.driver,name)&&(dOnly(e.ts)==null||dOnly(e.ts)>=since); }).length; };
  const isHiredDrv=function(name){ var t=(trucks||[]).find(function(x){ return (x.driver||'').trim().toLowerCase()===(name||'').trim().toLowerCase(); }); return !!(t&&t.isHired); };
  const wbBad=(drivers||[]).filter(function(d){ return !isHiredDrv(d.name); }).map(function(d){ var w=(waybills||[]).find(function(x){return (x.driverName||'').trim()===(d.name||'').trim();});
...
function HrWaybills({drivers,history,waybills,onSave,onDel,trucks,plans}){
  const [showAll,setShowAll]=useState(false);
  // сколько рейсов сделал водитель ПОСЛЕ выдачи путевого — по журналу отправок
  const tripsSince=function(name,sinceISO){ return countTrips(name,sinceISO,history,plans); };
  const _tripsSinceOld2=function(name,sinceISO){
    var since=dOnly(sinceISO)||0;
    return (history||[]).filter(function(e){ return e&&e.action==='dispatch'&&sameDriver(e.driver,name)
  ...
  const isHiredDrv2=function(name){ var t=(trucks||[]).find(function(x){ return (x.driver||'').trim().toLowerCase()===(name||'').trim().toLowerCase(); }); return !!(t&&t.isHired); };
  const rows=(drivers||[]).filter(function(d){ return !isHiredDrv2(d.name); }).map(function(d){
    var w=wbOf(d.name);
...
function HrBase({trucks,waybills,history,drivers}){
  ...
  const tripsFor=function(t){
    ...
    var n=countTrips(name, since, history, plans);
...
function HrTab(props){
  ...
  return ce('div',{style:{padding:'16px',maxWidth:1700,margin:'0 auto'}},
    ...
    view==='dash'&&ce(HrDash,{drivers:props.drivers,vacations:props.vacations,waybills:props.waybills,history:props.history,trucks:props.trucks,plans:props.plans,onJump:setView}),
    ...
    view==='waybills'&&ce(HrWaybills,{waybills:props.waybills,drivers:props.drivers,history:props.history,trucks:props.trucks,plans:props.plans,onSave:props.onSaveWaybill,onDel:props.onDelWaybill}),
    view==='base'&&ce(HrBase,{trucks:props.trucks,waybills:props.waybills,history:props.history,drivers:props.drivers}));
}
// ═══════════ ФОРМА ПЕРЕВОЗЧИКА (публичная ссылка ?form=carrier) ═══════════
const CARRIER_CODE='egida';   // кодовое слово от спама, можно поменять
function CarrierForm(){
  const [code,setCode]=useState('');
  const [name,setName]=useState('');
  const [phone,setPhone]=useState('');
  const [rows,setRows]=useState([{date:'',count:'1',where:'',freeNow:'yes',freeWhen:''}]);
  const [note,setNote]=useState('');
  const [sent,setSent]=useState(false);
  const [sentN,setSentN]=useState(0);
  const [err,setErr]=useState('');
  const [busy,setBusy]=useState(false);
  const fieldStyle={width:'100%',background:'var(--bg3)',border:'1px solid var(--border2)',borderRadius:11,padding:'13px 14px',color:'var(--text)',fontSize:16,fontFamily:'inherit',boxSizing:'border-box'};
  const lab=function(t){ return ce('div',{style:{fontSize:12.5,color:'var(--text3)',fontWeight:700,marginBottom:6,marginTop:14}},t); };
  const setRow=function(i,patch){ setRows(function(p){ return p.map(function(r,j){ return j===i?Object.assign({},r,patch):r; }); }); };
  const addRow=function(){ setRows(function(p){ return p.concat([{date:'',count:'1',where:'',freeNow:'yes',freeWhen:''}]); }); };
  const delRow=function(i){ setRows(function(p){ return p.length>1?p.filter(function(r,j){return j!==i;}):p; }); };
  const submit=function(){
    setErr('');
    if(code.trim().toLowerCase()!==CARRIER_CODE){ setErr('Неверное кодовое слово. Уточните его у Egida-Trans.'); return; }
    if(!name.trim()||!phone.trim()){ setErr('Заполните название и телефон.'); return; }
    var valid=rows.filter(function(r){ return Number(r.count)>0; });
    if(!valid.length){ setErr('Добавьте хотя бы одну машину.'); return; }
    setBusy(true);
    var stamp=new Date().toISOString();
    var group='grp_'+Date.now();
    var ops=valid.map(function(r){
      var id='off_'+Date.now()+Math.random().toString(36).slice(2,6);
      var rec={id:id,name:name.trim(),phone:phone.trim(),count:Number(r.count),where:(r.where||'').trim(),
        freeNow:r.freeNow==='yes',freeWhen:r.freeNow==='yes'?'':(r.freeWhen||'').trim(),date:r.date,note:note.trim(),
        ts:stamp,group:group,status:'new'};
      return dbPut('/carrier_offers/'+id,rec);
    });
    Promise.all(ops).then(function(res){
      setBusy(false);
      var okCount=res.filter(function(x){return x!==false;}).length;
      if(okCount>0){ setSentN(okCount); setSent(true); }
      else setErr('Не удалось отправить — проверьте интернет и попробуйте ещё раз.');
    }).catch(function(){ setBusy(false); setErr('Не удалось отправить — попробуйте ещё раз.'); });
  };
  if(sent) return ce('div',{style:{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',padding:20,background:'var(--bg)',color:'var(--text)'}},
    ce('div',{style:{maxWidth:420,textAlign:'center'}},
      ce('div',{style:{fontSize:54,marginBottom:12}},'✅'),
      ce('div',{style:{fontSize:20,fontWeight:800,marginBottom:8}},'Отправлено!'),
      ce('div',{style:{fontSize:14,color:'var(--text3)',lineHeight:1.6,marginBottom:20}},'Спасибо, '+name.trim()+'. Отправлено записей: '+sentN+'. Логист Egida-Trans увидит ваши машины. Появятся ещё — просто откройте эту ссылку снова.'),
      ce('button',{className:'fl-press',onClick:function(){ setSent(false); setRows([{date:'',count:'1',where:'',freeNow:'yes',freeWhen:''}]); setNote(''); },
        style:{padding:'12px 24px',borderRadius:11,border:'none',cursor:'pointer',fontFamily:'inherit',fontSize:14,fontWeight:800,color:'#fff',background:'linear-gradient(135deg,var(--accent),var(--accent2))'}},'Отправить ещё')));
  return ce('div',{style:{minHeight:'100vh',background:'var(--bg)',color:'var(--text)',padding:'24px 16px'}},
    ce('div',{style:{maxWidth:520,margin:'0 auto'}},
      ce('div',{style:{display:'flex',alignItems:'center',gap:11,marginBottom:6}},
        ce('div',{style:{width:44,height:44,borderRadius:12,background:'linear-gradient(135deg,#1e3a5f,#0f1420)',border:'1px solid var(--border2)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:22}},'🚛'),
        ce('div',null,
          ce('div',{style:{fontWeight:800,fontSize:18}},'Свободные машины'),
          ce('div',{style:{fontSize:12.5,color:'var(--text3)'}},'для Egida-Trans'))),
      ce('div',{style:{fontSize:13,color:'var(--text3)',lineHeight:1.6,marginBottom:8,marginTop:10}},
        'Укажите свободный транспорт под загрузку. Можно добавить несколько дней сразу — например, вторник 1 авто и среда 2 авто.'),
      lab('Кодовое слово'),
      ce('input',{value:code,onChange:function(e){setCode(e.target.value);},placeholder:'выдаёт Egida-Trans',style:fieldStyle}),
      lab('Перевозчик (название или имя)'),
      ce('input',{value:name,onChange:function(e){setName(e.target.value);},placeholder:'напр. ИП Иванов',style:fieldStyle}),
      lab('Телефон'),
      ce('input',{value:phone,onChange:function(e){setPhone(e.target.value);},type:'tel',placeholder:'+375…',style:fieldStyle}),
      ce('div',{style:{fontSize:12.5,color:'var(--text3)',fontWeight:700,marginTop:20,marginBottom:8}},'МАШИНЫ ПО ДНЯМ'),
      rows.map(function(r,i){ return ce('div',{key:i,style:{background:'var(--bg2)',border:'1px solid var(--border)',borderRadius:13,padding:'13px',marginBottom:10}},
        ce('div',{style:{display:'flex',alignItems:'center',gap:8,marginBottom:9}},
          ce('span',{style:{fontWeight:800,fontSize:13,color:'var(--accent)'}},'Машина '+(i+1)),
          rows.length>1?ce('button',{className:'fl-press',onClick:function(){delRow(i);},style:{marginLeft:'auto',width:28,height:28,borderRadius:8,border:'1px solid color-mix(in srgb,var(--red) 40%,transparent)',background:'transparent',color:'var(--red)',cursor:'pointer',fontSize:13}},'✕'):null),
        ce('div',{style:{display:'grid',gridTemplateColumns:'1fr 1fr',gap:9}},
          ce('div',null,ce('div',{style:{fontSize:11,color:'var(--text3)',marginBottom:4}},'Дата загрузки'),
            ce('input',{type:'date',value:r.date,onChange:function(e){setRow(i,{date:e.target.value});},style:Object.assign({},fieldStyle,{padding:'11px 12px'})})),
          ce('div',null,ce('div',{style:{fontSize:11,color:'var(--text3)',marginBottom:4}},'Сколько авто'),
            ce('input',{value:r.count,onChange:function(e){setRow(i,{count:e.target.value.replace(/[^0-9]/g,'')});},inputMode:'numeric',style:Object.assign({},fieldStyle,{padding:'11px 12px'})}))),
        ce('div',{style:{marginTop:9}},ce('div',{style:{fontSize:11,color:'var(--text3)',marginBottom:4}},'Где сейчас авто'),
          ce('input',{value:r.where,onChange:function(e){setRow(i,{where:e.target.value});},placeholder:'город / местоположение',style:Object.assign({},fieldStyle,{padding:'11px 12px'})})),
        ce('div',{style:{display:'flex',gap:7,marginTop:9}},
          ['yes','later'].map(function(v){ var on=r.freeNow===v;
            return ce('button',{key:v,className:'fl-press',onClick:function(){setRow(i,{freeNow:v});},
              style:{flex:1,padding:'10px',borderRadius:10,cursor:'pointer',fontFamily:'inherit',fontSize:13,fontWeight:800,
              border:'1px solid '+(on?'var(--accent)':'var(--border2)'),background:on?'color-mix(in srgb,var(--accent) 14%,transparent)':'var(--bg3)',color:on?'var(--accent)':'var(--text3)'}},
              v==='yes'?'Свободна сейчас':'Освободится'); })),
        r.freeNow==='later'?ce('input',{value:r.freeWhen,onChange:function(e){setRow(i,{freeWhen:e.target.value});},placeholder:'когда освободится',style:Object.assign({},fieldStyle,{padding:'11px 12px',marginTop:8})}):null); }),
      ce('button',{className:'fl-press',onClick:addRow,style:{width:'100%',padding:'12px',borderRadius:11,cursor:'pointer',fontFamily:'inherit',fontSize:13.5,fontWeight:800,
        border:'1px dashed color-mix(in srgb,var(--accent) 55%,transparent)',background:'color-mix(in srgb,var(--accent) 8%,transparent)',color:'var(--accent)'}},'＋ Добавить ещё день'),
      lab('Примечание (необязательно, общее)'),
      ce('textarea',{value:note,onChange:function(e){setNote(e.target.value);},rows:2,placeholder:'что везёте, направление, любые детали',style:Object.assign({},fieldStyle,{resize:'vertical'})}),
      err?ce('div',{style:{marginTop:14,padding:'11px 13px',borderRadius:11,background:'color-mix(in srgb,var(--red) 12%,transparent)',border:'1px solid color-mix(in srgb,var(--red) 40%,transparent)',color:'var(--red)',fontSize:13,fontWeight:700}},err):null,
      ce('button',{className:'fl-press',onClick:submit,disabled:busy,
        style:{width:'100%',marginTop:18,padding:'15px',borderRadius:13,border:'none',cursor:busy?'default':'pointer',fontFamily:'inherit',fontSize:16,fontWeight:800,color:'#fff',
        background:'linear-gradient(135deg,var(--accent),var(--accent2))',opacity:busy?0.6:1}},busy?'Отправляю…':'Отправить'),
      ce('div',{style:{height:30}})));
}
function App() {
  var _isCarrierForm=false;
  try{ _isCarrierForm=new URLSearchParams(window.location.search).get('form')==='carrier'; }catch(e){}
  if(_isCarrierForm) return ce(CarrierForm,null);
  return ce(AppMain,null);
}
function AppMain() {
  ...
  const [gridBacks,setGridBacks] = useState({});
  const [gridTuda,setGridTuda] = useState({});
  const [driverMarks,setDriverMarks] = useState({});
  const [loaded,setLoaded] = useState(false);
  const cloudOrdersRef = useRef(false);
  const cloudPlansRef = useRef(false);
  const [routeDay,setRouteDay] = useState(today());
  const [theme,setTheme] = useState(function(){ try{ return localStorage.getItem('egida_theme')||'dark'; }catch(e){ return 'dark'; } });
  ...
  // Требуемая температура водителю — пишем из карточки клиента, НЕ завися от CAP
  useEffect(function(){
    if(!loaded) return;
    var tm=setTimeout(function(){
      (trucks||[]).forEach(function(t){
        if(!t||!t.id) return;
        if(MOVING.indexOf(t.status)<0) return;                 // только машины в рейсе
        var cl=(clients||[]).find(function(c){ return c.id===t.clientId||c.name===t.clientName; });
        var req=(cl&&cl.temp)||'';
        if(!req) return;
        if(t.reqTemp===req) return;                            // уже записано
        dbPatch('/trucks/'+t.id,{reqTemp:req});
        setTrucks(function(prev){ return prev.map(function(x){ return x.id===t.id?Object.assign({},x,{reqTemp:req}):x; }); });
      });
    },1500);
    return function(){ clearTimeout(tm); };
  },[loaded,trucks,clients]);

  // Геолокация + температура машин из CAP (пишем в машину, чтобы водительское читало из базы)
  useEffect(function(){
    var load=function(){
      capFetch('odometer').then(function(r){
        if(!r.ok) return;
        var data=(r.val&&(r.val.data||r.val))||[];
        if(Array.isArray(data)){ var idx=buildGeoIndex(data); setGeoIndex(idx); cacheSet('geo_index',idx); }
      });
      // температуры рефов -> пишем факт с датчика в машину этого прицепа
      capFetch('temperature/current').then(function(r){
        if(!r.ok) return;
        var data=(r.val&&(r.val.data||r.val))||[];
        if(!Array.isArray(data)) return;
        // строим карту: нормализованный прицеп -> температура с датчика
        var tmap={};
        data.forEach(function(row){
          var p=capParseName(row.name||''); if(!p.plate) return;
          var v=null, at=null, s=row.sensors;
          if(s){
            if(Array.isArray(s)){ for(var i=0;i<s.length;i++){ if(s[i]&&s[i].value_c!=null){ v=Number(s[i].value_c); at=s[i].measured_at; break; } } }
            else if(s.value_c!=null){ v=Number(s.value_c); at=s.measured_at; }   // CAP отдаёт объект, не массив
          }
          if(v==null && row.value_c!=null){ v=Number(row.value_c); at=row.measured_at; }
          if(v!=null) tmap[p.plate]={t:v,at:at||row.measured_at||nowLocalISO()};
        });
        // обновляем машины: currentTemp (факт с датчика), reqTemp (требуемая из клиента) — всегда для машин в рейсе
        setTrucks(function(prev){ return prev.map(function(t){
          var sp=splitFleteraPlate(t.plate);
          // ищем температуру датчика: по прицепу, затем по трактору
          var td=(sp.trailer&&tmap[sp.trailer])||(sp.tractor&&tmap[sp.tractor])||null;
          var cl=(clients||[]).find(function(c){ return c.id===t.clientId; });
          var req=(cl&&cl.temp)||t.reqTemp||'';
          // если ни датчика, ни требуемой — нечего писать
          if(!td && !req) return t;
          var patch={reqTemp:req};
          if(td){ patch.currentTemp=td.t; patch.currentTempAt=td.at; }
          // не дёргаем базу если ничего не изменилось
          if(t.reqTemp===req && (!td || (t.currentTemp===td.t))) return t;
          dbPatch('/trucks/'+t.id,patch);
          return Object.assign({},t,patch);
        }); });
      });
    };
    var c=cacheGet('geo_index'); if(c) setGeoIndex(c);
    load();
  ...
  useEffect(function(){ var iv=setInterval(function(){ setClock(function(n){return n+1;}); },60000); return function(){ clearInterval(iv); }; },[]);
  ...
  const [tempBinds,setTempBinds] = useState({});
  const [geoIndex,setGeoIndex] = useState({});
  const [demands,setDemands] = useState([]);
  const [carrierOffers,setCarrierOffers] = useState([]);
  ...
  // пароли ролей: у кого задан — спрашиваем при входе
  const ROLE_PINS = { 'Настя':'2222', 'Катя':'1111', 'Механик':'3333' };
  const pickRole = (r)=>{
    var need=ROLE_PINS[r];
    // пароль логиста можно задать и в списке логистов (поле pin)
    var lg=(logists||[]).find(function(l){ return l.name===r; });
    if(lg && lg.pin) need=String(lg.pin);
    if(need){
      var got=window.prompt('Пароль для «'+r+'»:','');
      if(got===null) return;                       // отмена
      if(String(got).trim()!==String(need)){ tErr('Неверный пароль'); return; }
    }
    setRole(r); try{localStorage.setItem('egida_role',r);}catch(e){}
  ...
  useEffect(()=>{ var f=function(){
    ...
    dbFetch('/demands').then(function(r){ if(r.ok){ var v=mapVals(r.val); setDemands(v); cacheSet('demands',v); } else { var c=cacheGet('demands'); if(c&&c.length) setDemands(function(p){ return p.length?p:c; }); } });
    dbFetch('/carrier_offers').then(function(r){ if(r.ok){ setCarrierOffers(mapVals(r.val)); } });
    dbFetch('/temp_binds').then(function(r){ if(r.ok){ var v=(r.val&&typeof r.val==='object')?r.val:{}; setTempBinds(v); cacheSet('temp_binds',v); } else { var c=cacheGet('temp_binds'); if(c) setTempBinds(function(p){ return Object.keys(p).length?p:c; }); } });
  ...
  useEffect(()=>{ document.documentElement.setAttribute('data-theme',theme); try{ localStorage.setItem('egida_theme',theme); }catch(e){} },[theme]);
  ...
  useEffect(()=>{
    ...
    Promise.all([dbGet('/trucks'),dbGet('/clients'),dbGet('/orders'),dbGet('/plans')]).then(function(r){
      ...
      dbGet('/day_notes').then(function(dn){ if(alive&&dn&&typeof dn==='object') setDayNotes(dn); });
      dbGet('/grid_backs').then(function(gb){ if(alive&&gb&&typeof gb==='object') setGridBacks(gb); });
      dbGet('/grid_tuda').then(function(gt){ if(alive&&gt&&typeof gt==='object') setGridTuda(gt); });
      dbGet('/driver_marks').then(function(dm){ if(alive&&dm&&typeof dm==='object'){ var o={}; Object.keys(dm).forEach(function(k){ o[decodeURIComponent(k)]=dm[k]; }); setDriverMarks(o); } });
      setLoaded(true);
  ...
  const updTruck = (id,patch)=>{ setTrucks(prev=>prev.map(t=>{ if(t.id!==id) return t; var m=Object.assign({},t,patch);
    if(patch&&patch.status==='FORWARD' && t.status!=='FORWARD'){ m.reachedUnload=false;
      // рейс падает тому, кто нажал «В рейс» (если логист ещё не назначен вручную и патч его не задал)
      if(!patch.logist && !m.logist && role && LOGISTS.indexOf(role)>=0){ m.logist=role; }
    }
    if(patch&&patch.status==='FORWARD' && t.status!=='FORWARD' && (m.driver||'').trim()){
      ...
      // автопривязка температуры: если у клиента задан режим — сразу привязываем реф к рейсу
      (function(){
        var cl=(clients||[]).find(function(c){ return c.id===m.clientId||c.name===m.clientName; });
        var tp=(cl&&cl.temp)||'';
        if(!tp) return;
        var trailer=splitFleteraPlate(m.plate).trailer;
        if(!trailer) return;
        var hrs=(cl&&cl.routeHours)||14;
        var bind={truckId:id,truckPlate:m.plate||'',driver:(m.driver||'').trim(),
          clientName:m.clientName||(cl&&cl.name)||'',temp:tp,at:nowLocalISO(),since:nowLocalISO(),starts:'',
          expires:new Date(Date.now()+hrs*3600*1000).toISOString(),hours:hrs,auto:true};
        setTempBinds(function(prev){ var n=Object.assign({},prev); n[trailer]=bind; cacheSet('temp_binds',n); return n; });
        saveOr('/temp_binds/'+trailer, bind, 'Температура привязана автоматически');
      })();
    }
    dbPut('/trucks/'+id,m); return m; })); tShow(T('saved')); };
  // Откат: логист снял рейс/обратку — убираем последнее событие отправки этой машины
  const launchDemand = (d)=>{
    if(!d||!(d.assigned||[]).length) return;
    var cl=(clients||[]).find(function(c){ return c.name===d.clientName; });
    (d.assigned||[]).forEach(function(id){
      var t=(trucks||[]).find(function(x){ return String(x.id)===String(id); });
      if(!t) return;
      updTruck(t.id,{status:'FORWARD',
        clientId:(cl&&cl.id)||t.clientId||'',clientName:d.clientName||t.clientName||'',
        loadAt:(d.date||'')+'T08:00',
        logist:(role&&LOGISTS.indexOf(role)>=0)?role:(t.logist||''),
        note:(d.time?('Заявка: '+d.time):'')||t.note||''});
    });
    // помечаем заявку выполненной
    saveDemand(Object.assign({},d,{launched:true}));
    tShow('Отправлено в рейс: '+(d.assigned||[]).length);
  };
  const launchPlan = (p)=>{
    if(!p) return;
    if(p.launched){ tShow('Этот рейс уже создан'); return; }   // защита от повторного запуска (дубли)
    var norm=function(s){ return String(s||'').toUpperCase().replace(/[\s\-\/]/g,''); };
    var np=norm(p.truckPlate), nd=String(p.truckDriver||'').trim().toLowerCase();
    var t=(trucks||[]).find(function(x){
      if(String(x.id)===String(p.truckId)) return true;
      if(np && norm(x.plate)===np) return true;
      if(np && norm(x.plate).indexOf(np)>=0) return true;   // номер плана — часть номера машины
      if(np && np.indexOf(norm(x.plate))>=0) return true;   // или наоборот
      if(nd && (x.driver||'').trim().toLowerCase()===nd) return true;
      if(nd && (x.driver||'').trim().toLowerCase().indexOf(nd)>=0) return true; // фамилия — часть ФИО
      return false;
    });
    if(!t){ tErr('Не нашёл машину «'+(p.truckPlate||p.truckDriver||'?')+'» в парке. Проверьте номер/водителя в строке плана.'); return; }
    var cl=(clients||[]).find(function(c){ return c.name===p.clientName; });
    // если машина уже в рейсе — НЕ сбрасываем статус водителя (он сам его ставил), только FORWARD для свободной
    var alreadyMoving=MOVING.indexOf(t.status)>=0;
    var patch={status:alreadyMoving?t.status:'FORWARD',
      driver:(t.driver&&t.driver.trim())||p.truckDriver||'',   // водитель — чтобы событие путевого записалось с верной фамилией
      clientId:(cl&&cl.id)||t.clientId||'',clientName:p.clientName||t.clientName||'',
      loadAddr:(cl&&cl.loadAddr)||p.from||t.loadAddr||'',
      unloadAddr:(cl&&cl.unloadOptions&&cl.unloadOptions[0]&&cl.unloadOptions[0].addr)||p.to||t.unloadAddr||'',
      loadAt:(p.loadDate||p.date||'')+'T08:00',
      logist:(role&&LOGISTS.indexOf(role)>=0)?role:(t.logist||''),  // рейс падает тому, кто нажал «В рейс»
      clientName2:p.clientName2||'',   // вторая загрузка туда
      note:(p.clientName2?('2-я загрузка: '+p.clientName2+'. '):'')+(p.note||t.note||'')};
    // переносим обратку из плана (текст p.back) в карточку рейса
    if(p.back && String(p.back).trim()){
      patch.backCargo=Object.assign({cargo:String(p.back).trim()}, t.backCargo||{}, {cargo:String(p.back).trim()});
      if(p.backFromGeo) patch.backCargo.fromGeo=p.backFromGeo;
      if(p.backToGeo) patch.backCargo.toGeo=p.backToGeo;
    }
    updTruck(t.id,patch);
    // вариант Б: строка остаётся в плане как «✓ в рейсе» (не удаляем), защита от повторного запуска
    var pl=Object.assign({},p,{launched:true,launchedTruckId:t.id,truckPlate:t.plate||p.truckPlate||''});
    dbPut('/plans/'+p.id,pl); setPlans(function(x){ return x.map(function(y){ return y.id===p.id?pl:y; }); });
    tShow('▶ '+(t.driver||t.plate)+' → '+(p.clientName||'')+' — рейс создан');
  };
  const applyRigSwap = (o)=>{
    if(!o||!o.src||!o.dst) return;
    var sPatch={plate:o.newSrc};
    if(o.toService){ sPatch.onService=true; sPatch.status='SERVICE'; }
    updTruck(o.src.id,sPatch);
    updTruck(o.dst.id,{plate:o.newDst});
    var dstTract=String(o.dst.plate||'').split('/')[0].trim();
    var srcTract=String(o.src.plate||'').split('/')[0].trim();
    var moved=findMecTrailer(mecTrailers,capNorm(o.movedTrailer));
    var ntr=findMecTractor(mecTractors,capNorm(dstTract));
    if(moved&&ntr){
      var cur=trailerMileage(moved,mecTractors);
      var nodo=Number(ntr.odometer);
      var patch={pairedTractor:ntr.plate};
      if(isFinite(nodo)&&cur!=null){ patch.accum=String(Math.round(cur)); patch.attachBase=String(nodo); }
      updMecTrailer(moved.id,patch);
    }
    if(o.backTrailer){
      var back=findMecTrailer(mecTrailers,capNorm(o.backTrailer));
      var str2=findMecTractor(mecTractors,capNorm(srcTract));
      if(back&&str2){
        var cur2=trailerMileage(back,mecTractors);
        var nodo2=Number(str2.odometer);
        var patch2={pairedTractor:str2.plate};
        if(isFinite(nodo2)&&cur2!=null){ patch2.accum=String(Math.round(cur2)); patch2.attachBase=String(nodo2); }
        updMecTrailer(back.id,patch2);
      }
    }
    var hid='rig_'+Date.now()+Math.random().toString(36).slice(2,5);
    var ev={}; ev[hid]={ts:nowLocalISO(),plate:o.movedTrailer,driver:o.dst.driver||'',logist:role||'',
      action:'rigswap',label:'Прицеп '+o.movedTrailer+': '+(o.src.plate||'')+' → '+(o.dst.plate||'')+(o.toService?' · тягач на сервис':''),ref:String(o.dst.id)};
    dbPatch('/history',ev); setHistory(function(h){ return (h||[]).concat([ev[hid]]); });
    tShow('Сцепка пересажена');
  };
  ...
  const saveDemand = (d)=>{ var id=d.id||('dm_'+Date.now()); var obj=Object.assign({},d,{id:id});
    setDemands(function(prev){ var ex=prev.some(function(x){return x.id===id;}); var n=ex?prev.map(function(x){return x.id===id?obj:x;}):prev.concat([obj]); cacheSet('demands',n); return n; });
    saveOr('/demands/'+id,obj,'Заявка сохранена'); };
  const delDemand = (id)=>{ setDemands(function(prev){ var n=prev.filter(function(x){return x.id!==id;}); cacheSet('demands',n); return n; }); saveOr('/demands/'+id,null,'Заявка удалена'); };
  const bindTemp = (plate,bind)=>{ setTempBinds(function(prev){ var n=Object.assign({},prev); if(bind) n[plate]=bind; else delete n[plate]; cacheSet('temp_binds',n); return n; });
    saveOr('/temp_binds/'+plate, bind||null, bind?'Привязано к рейсу':'Привязка снята'); };
  ...
  // save order (обратка НЕ создаёт клиента — имя заказчика хранится только в самой обратке)
  const saveOrder = (o,isEdit)=>{
    var ord=Object.assign({},o); delete ord.newClient;
    // сохраняем имя разового заказчика прямо в обратку, чтобы не терялось
    if(o.newClient&&o.newClient.name&&!ord.client) ord.client=o.newClient.name;
  ...
  const stats = useMemo(()=>({
    free:trucks.filter(isFreeTruck).length,
    moving:trucks.filter(effMoving).length,
  ...
  const filtered = useMemo(()=>{
    ...
    return trucks.filter(t=>{
      ...
      if(stFilter==='FREE'&&!isFreeTruck(t)) return false;
  ...
  if(isMech||isHr) TABS=TABS.filter(function(x){ return ['clients','orders','future','grid','log'].indexOf(x[0])<0; });
  if(isBoss) TABS=[];
  ...
  return ce('div',{style:{minHeight:'100vh',color:'var(--text)'}},
    drawer&&TABS.length>0?ce('div',{onClick:function(){setDrawer(false);},
      style:{position:'fixed',inset:0,zIndex:900,background:'rgba(0,0,0,.5)',backdropFilter:'blur(2px)'}},
      ce('div',{onClick:function(e){e.stopPropagation();},
        style:{position:'absolute',top:0,left:0,bottom:0,width:'78%',maxWidth:300,background:'var(--bg2)',
          borderRight:'1px solid var(--border)',padding:'16px 12px',overflowY:'auto',boxShadow:'var(--shadow-lg)'}},
        ce('div',{style:{display:'flex',alignItems:'center',gap:9,marginBottom:14,paddingLeft:4}},
          ce('span',{style:{fontWeight:800,fontSize:16}},'Меню'),
          ce('button',{className:'fl-press',onClick:function(){setDrawer(false);},
            style:{marginLeft:'auto',width:32,height:32,borderRadius:9,border:'1px solid var(--border2)',background:'var(--bg3)',color:'var(--text2)',cursor:'pointer',fontSize:15}},'✕')),
        role?ce('div',{style:{padding:'8px 10px',borderRadius:10,marginBottom:12,fontSize:12.5,fontWeight:700,
          background:'color-mix(in srgb,var(--accent) 10%,transparent)',color:'var(--accent)'}},'👤 '+role):null,
        ce('div',{style:{display:'grid',gap:5}},
          TABS.map(function(x){ var id=x[0],label=x[1],on=tab===id;
            return ce('button',{key:id,className:'fl-press',onClick:function(){ setTab(id); setDrawer(false); },
              style:{width:'100%',textAlign:'left',padding:'13px 14px',borderRadius:11,cursor:'pointer',fontFamily:'inherit',
                fontSize:15,fontWeight:800,border:'1px solid '+(on?'color-mix(in srgb,var(--accent) 45%,transparent)':'transparent'),
                background:on?'color-mix(in srgb,var(--accent) 14%,transparent)':'transparent',
                color:on?'var(--accent)':'var(--text2)'}},label); })))):null,
    ...
    ce('div',{style:{background:'color-mix(in srgb,var(--bg2) 88%,transparent)',backdropFilter:'blur(10px)',
      ...
      ce('div',{style:{maxWidth:1700,margin:'0 auto',display:'flex',alignItems:'center',justifyContent:'space-between',height:60,gap:14}},
        ...
        TABS.length>0&&ce('button',{className:'fl-burger fl-press',onClick:()=>setDrawer(true),
          style:{display:'none',width:40,height:38,borderRadius:10,border:'1px solid var(--border2)',background:'var(--bg2)',
            color:'var(--text)',cursor:'pointer',fontSize:17,flexShrink:0,alignItems:'center',justifyContent:'center'}},'☰'),
        TABS.length>0&&ce('nav',{className:'fl-nav',style:{display:'flex',gap:3,flexWrap:'wrap',justifyContent:'center',flex:1}},
        ...
        ce('div',{style:{display:'flex',gap:11,alignItems:'center',flexShrink:0}},
          ...
          ce('button',{className:'fl-press',title:'Тема оформления',onClick:function(){
              var list=['dark','light','midnight','warm','contrast'];
              setTheme(function(x){ var i=list.indexOf(x); return list[(i+1)%list.length]; });
            },style:headBtn},
            (function(){
              var names={dark:'🌙 Тёмная',light:'☀️ Светлая',midnight:'🌌 Ночная',warm:'🍂 Тёплая',contrast:'⬛ Контраст'};
    ...
    tab==='park'&&ce('div',{style:{padding:'16px',maxWidth:1700,margin:'0 auto'}},
      modal==='rigswap'&&ce(RigSwapModal,{trucks:trucks,mecTrailers:mecTrailers,mecTractors:mecTractors,
        onApply:function(o){ applyRigSwap(o); setModal(null); },onClose:function(){setModal(null);}}),
      (!isBoss)&&ce(UnloadSummary,{trucks:trucks.filter(function(t){return isBoss||t.logist===role;}),
        onEngage:function(t){ setSearch(t.plate||''); if(typeof window!=='undefined'&&window.scrollTo) window.scrollTo({top:0,behavior:'smooth'}); tShow('Показана машина '+(t.plate||'')+' — оформите обратку на карточке'); }}),
      (!isBoss)&&ce('div',{style:{display:'flex',gap:7,marginBottom:14,flexWrap:'wrap',alignItems:'center'}},
        ce('button',{className:'fl-press',onClick:function(){setModal('rigswap');},style:{padding:'7px 14px',borderRadius:9,cursor:'pointer',fontFamily:'inherit',fontSize:12.5,fontWeight:800,
          border:'1px solid color-mix(in srgb,var(--cyan) 50%,transparent)',background:'color-mix(in srgb,var(--cyan) 14%,transparent)',color:'var(--cyan)'}},'🔄 Пересадка сцепки'),
      ...
      (!isBoss)&&ce('div',{style:{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(330px,1fr))',gap:12}},
        filtered.map(t=>ce(TruckCard,{key:t.id,truck:t,clients,orders,settings,role,isBoss,onUpdate:updTruck,geoIndex:geoIndex,
    ...
    tab==='moving'&&(function(){
      ...
      var dayPlans=(plans||[]).filter(function(p){return planOn(p,routeDay)&&!p.launched;});
      return ce('div',{style:{padding:'16px',maxWidth:1700,margin:'0 auto'}},
        ...
        ce('div',{style:{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(340px,1fr))',gap:12}},
          ...
          dayPlans.map(function(p){
            var pk=p.loadDate||p.date||''; var isDue=pk&&pk<=todayStr;   // день наступил или прошёл
            return ce('div',{key:'plan_'+p.id,className:'fl-card',style:{background:isDue?'color-mix(in srgb,var(--accent) 7%,transparent)':'var(--bg2)',
            border:isDue?'1px solid color-mix(in srgb,var(--accent) 55%,transparent)':'1px dashed color-mix(in srgb,var(--violet) 50%,transparent)',borderRadius:14,padding:'14px 15px',boxShadow:'var(--shadow)'}},
            ce('div',{style:{display:'flex',justifyContent:'space-between',marginBottom:6,alignItems:'center'}},
              ce('div',{style:{fontWeight:800,fontSize:13.5,color:'var(--text)'}},p.truckPlate||'—'),
              ce('span',{style:{fontSize:10.5,padding:'2px 9px',borderRadius:999,fontWeight:800,
                background:isDue?'color-mix(in srgb,var(--accent) 18%,transparent)':'color-mix(in srgb,var(--violet) 16%,transparent)',
                color:isDue?'var(--accent)':'var(--violet)'}},isDue?'⏰ ПОРА В РЕЙС':'ПЛАН')),
            ...
            ce('div',{style:{fontSize:11.5,color:isDue?'var(--accent)':'var(--violet)',fontWeight:700,marginBottom:3}},'📅 '+(p.loadDate||p.date||'')+(p.unloadDate?(' → '+p.unloadDate):'')),
            ...
            isDue?ce('div',{style:{fontSize:11,color:'var(--text3)',marginTop:7,lineHeight:1.5}},'День загрузки наступил. Запустите план — он станет активным рейсом.'):null,
            ce('div',{style:{marginTop:9,display:'flex',gap:6}},
              ce('button',{className:'fl-press',onClick:function(){ launchPlan(p); },style:{flex:1,padding:'9px',borderRadius:9,border:'none',cursor:'pointer',fontFamily:'inherit',fontSize:12.5,fontWeight:800,
                color:'#fff',background:isDue?'linear-gradient(135deg,var(--accent),var(--accent2))':'linear-gradient(135deg,var(--violet),var(--accent))'}},isDue?'▶ Запустить рейс':'▶ Создать рейс'),
    ...
    tab==='orders'&&ce('div',{style:{padding:'16px',maxWidth:1700,margin:'0 auto'}},
      ce('div',{style:{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:14,gap:12,flexWrap:'wrap'}},
        ce('h2',{style:{margin:0,fontWeight:800,color:'var(--text)',fontSize:20}},T('orders_title')),
        ce('input',{value:orderQ,onChange:e=>setOrderQ(e.target.value),placeholder:'🔍 Поиск: клиент, груз, город, температура',
          style:{flex:1,minWidth:220,maxWidth:420,background:'var(--bg2)',border:'1px solid var(--border2)',borderRadius:9,padding:'8px 13px',color:'var(--text)',fontSize:13,fontFamily:'inherit'}}),
      ...
      (function(){ var oq=(orderQ||'').toLowerCase().trim();
        var shownOrders=oq?orders.filter(function(o){ return ((o.client||'')+' '+(o.cargo||'')+' '+(o.from||'')+' '+(o.to||'')+' '+(o.temp||'')+' '+(o.note||'')).toLowerCase().indexOf(oq)>=0; }):orders;
        return shownOrders.length===0?ce('div',{style:{textAlign:'center',padding:40,color:'var(--text3)',fontSize:13}},oq?'Ничего не найдено по запросу «'+orderQ+'».':'Обраток пока нет.'):
      ce('div',{style:{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(300px,1fr))',gap:12}},
        shownOrders.map(o=>{
    ...
    // ── FUTURE / PLANNING TAB (лист логистов) ──
    tab==='grid'&&ce(PlanGrid,{plans:plans,clients:clients,orders:orders,trucks:trucks,geoIndex:geoIndex,role:role,logists:logists,isBoss:isBoss,gridBacks:gridBacks,onSetGridBack:function(k,arr){ setGridBacks(function(p){ var n=Object.assign({},p); if(arr&&arr.length) n[k]=arr; else delete n[k]; return n; }); dbPut('/grid_backs/'+k, (arr&&arr.length)?arr:null); },
      onDelOrder:function(id){ dbPut('/orders/'+id,null); setOrders(function(x){ return x.filter(function(y){ return y.id!==id; }); }); },
      gridTuda:gridTuda,onSetGridTuda:function(k,arr){ setGridTuda(function(p){ var n=Object.assign({},p); if(arr&&arr.length) n[k]=arr; else delete n[k]; return n; }); dbPut('/grid_tuda/'+k,(arr&&arr.length)?arr:null); },
      onUpdateTruck:updTruck,
      driverMarks:driverMarks,onSetDriverMark:function(nm,m){ setDriverMarks(function(p){ var n=Object.assign({},p); if(m) n[nm]=m; else delete n[nm]; return n; }); dbPut('/driver_marks/'+encodeURIComponent(nm), m||null); },
      onSavePlan:function(r){ var pl=Object.assign({id:(r&&r.id)||uid()},r); dbPut('/plans/'+pl.id,pl);
        setPlans(function(prev){ var ex=prev.some(function(x){return x.id===pl.id;}); return ex?prev.map(function(x){return x.id===pl.id?pl:x;}):prev.concat([pl]); }); tShow('Строка сохранена'); },
      onDelPlan:function(id){ dbPut('/plans/'+id,null); setPlans(function(x){ return x.filter(function(y){ return y.id!==id; }); }); },
      onAddOrder:function(o){ var ord=normOrder(Object.assign({id:uid(),kind:'back'},o)); dbPut('/orders/'+ord.id,ord); setOrders(function(x){ return x.concat([ord]); }); tShow('Обратка добавлена'); },
      onSetBar:function(pl,s,e){ var np=Object.assign({},pl,{barStart:s,barEnd:e}); dbPut('/plans/'+np.id,np); setPlans(function(x){ return x.map(function(y){ return y.id===np.id?np:y; }); }); },
      onLaunch:launchPlan}),

    tab==='future'&&ce(PlanBoard,{plans:plans,clients:clients,orders:orders,trucks:trucks,
      dayNotes:dayNotes,onSaveDayNote:function(k,v){ setDayNotes(function(p){ var n=Object.assign({},p); if(v) n[k]=v; else delete n[k]; return n; }); dbPut('/day_notes/'+k, v||null); },
      onCopyDriver:function(txt){ tShow('📋 Текст скопирован — вставьте водителю'); },
      onSavePlan:function(r){
        var wasLaunched=r&&r.launched;
        var pl=Object.assign({id:(r&&r.id)||uid()},r);
        if(wasLaunched){
          // правили уже отправленную строку — сбрасываем «в рейсе», чтобы переназначить машину заново
          pl.launched=false; pl.launchedTruckId=null;
          tShow('Строка изменена — нажмите «В рейс», чтобы переназначить машину');
        } else tShow('Строка сохранена');
        dbPut('/plans/'+pl.id,pl);
        setPlans(function(prev){ var ex=prev.some(function(x){return x.id===pl.id;}); return ex?prev.map(function(x){return x.id===pl.id?pl:x;}):prev.concat([pl]); });
      },
      onDelPlan:function(id){ dbPut('/plans/'+id,null); setPlans(function(x){ return x.filter(function(y){ return y.id!==id; }); }); },
      onLaunch:launchPlan}),
    ...
    tab==='demands'&&ce(DemandsTab,{demands:demands,trucks:trucks,clients:clients,orders:orders,onSave:saveDemand,onDel:delDemand,onLaunch:launchDemand,
      carrierOffers:carrierOffers,onDelOffer:function(id){ setCarrierOffers(function(p){return p.filter(function(x){return x.id!==id;});}); dbPut('/carrier_offers/'+id,null); },
      onArchiveOffer:function(id){ setCarrierOffers(function(p){return p.map(function(x){return x.id===id?Object.assign({},x,{status:'done'}):x;});}); dbPatch('/carrier_offers/'+id,{status:'done'}); }}),
    tab==='temp'&&ce(TempTab,{trucks:trucks,clients:clients,binds:tempBinds,onBind:bindTemp,geoIndex:geoIndex,
      onRememberTemp:function(cname,tp){ var cl=(clients||[]).find(function(c){return c.name===cname;}); if(cl&&cl.temp!==tp){ var u=Object.assign({},cl,{temp:tp}); dbPut('/clients/'+cl.id,u); setClients(function(p){return p.map(function(x){return x.id===cl.id?u:x;});}); } }}),
    tab==='boss'&&ce(BossPanel,{trucks:trucks,clients:clients,plans:plans,orders:orders,history:history,onUpdate:updTruck,search:search,setSearch:setSearch,binds:tempBinds,onBind:bindTemp,
    ...
    tab==='hr'&&ce(HrTab,{drivers:hrDrivers,vacations:hrVacations,waybills:hrWaybills,history:history,plans:plans,trucks:trucks.filter(function(t){return !t.isHired;}),
    ...
    modal&&modal.type==='editclient'&&(function(){
      ...
      return ce(EditClientModal,{client:cl,
        onSave:u=>{dbPut('/clients/'+u.id,u);setClients(p=>p.map(c=>c.id===u.id?u:c));setModal(null);tShow(T('t_client_upd'));},
        onDelete:id=>{dbPut('/clients/'+id,null);setClients(p=>p.filter(c=>c.id!==id));setModal(null);tShow('Клиент удалён');},onClose:()=>setModal(null)});
    })(),
