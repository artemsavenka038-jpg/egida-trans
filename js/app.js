    ...
    ce('div',{style:{display:'flex',alignItems:'center',gap:12,marginBottom:14,flexWrap:'wrap'}},
      ...
      ce('div',{style:{display:'flex',alignItems:'center',gap:6,marginLeft:'auto'}},
        ...
        ce('button',{className:'fl-press',onClick:function(){
          // ужимаем сетку под один лист перед печатью
          var grid=document.querySelector('.fl-grid-wrap');
          if(grid){
            var pageW=1050, pageH=720; // примерно A4 landscape в px при 96dpi минус поля
            var sx=pageW/grid.scrollWidth, sy=pageH/grid.scrollHeight;
            var sc=Math.min(1,sx,sy);
            grid.style.transformOrigin='top left'; grid.style.transform='scale('+sc+')';
            window.print();
            setTimeout(function(){ grid.style.transform=''; },500);
          } else window.print();
        },title:'Печать на 1 страницу',style:{padding:'6px 12px',borderRadius:8,cursor:'pointer',fontFamily:'inherit',fontSize:13,border:'1px solid var(--border2)',background:'var(--bg2)',color:'var(--text2)'}},'🖨 Печать'))),
    // сетка
