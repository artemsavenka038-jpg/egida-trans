      ...
      ce('div',{style:{minWidth:900}},
        ...
        (function(){
          ...
          return ce('div',{key:ri,className:'fl-grid-row',style:{display:'grid',gridTemplateColumns:'160px repeat(7,1fr)',borderBottom:'1px solid var(--border)',position:'relative',minHeight:44}},
            ...
            // полоса рейса поверх (загрузка +1 день; +1 если обратка)
            bar?ce('div',{style:{position:'absolute',top:'50%',transform:'translateY(-50%)',height:4,borderRadius:3,pointerEvents:'none',
              left:'calc(160px + '+(bar.startIdx)+' * ((100% - 160px)/7) + 4px)',
              width:'calc('+(bar.endIdx-bar.startIdx+1)+' * ((100% - 160px)/7) - 8px)',
              background:bar.hasBack?('linear-gradient(90deg, color-mix(in srgb,'+bar.col+' 55%,transparent) 0%, color-mix(in srgb,'+bar.col+' 55%,transparent) 60%, color-mix(in srgb,var(--cyan) 55%,transparent) 60%, color-mix(in srgb,var(--cyan) 55%,transparent) 100%)'):('color-mix(in srgb,'+bar.col+' 55%,transparent)'),
              opacity:0.55}}):null,
            ...
            (function(){
              ...
              return ce('div',{style:{position:'absolute',top:'50%',transform:'translate(-50%,-50%)',pointerEvents:'none',zIndex:2,transition:'left .5s ease',
                left:'calc(160px + '+posDays+' * ((100% - 160px)/7))'}},
                ce('div',{style:{fontSize:15,filter:rp.dir==='back'?'scaleX(-1)':'none',transform:rp.dir==='back'?'scaleX(-1)':'none'}},'🚚'));
            })(),
            // (статус водителя убран отсюда — накладывался на текст клиента; он виден в самой ячейке)
            null); };
