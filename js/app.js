...
function OrderModal({clients,onSave,onClose,maxId,editOrder}) {
  ...
  const [fromGeo,setFromGeo] = useState((e0&&e0.fromGeo)||'');
  const [toGeo,setToGeo] = useState((e0&&e0.toGeo)||'');
  const cl = clients.find(c=>c.id===cid)||clients[0];
...
function EditClientModal({client,onSave,onDelete,onClose}) {
  ...
  const [loadGeo,setLoadGeo] = useState(client.loadGeo||'');
  const [unloadGeo,setUnloadGeo] = useState(client.unloadGeo||'');
