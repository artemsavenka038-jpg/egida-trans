...
function PlanGrid({plans,clients,orders,trucks,geoIndex,onSavePlan,onDelPlan,onLaunch,onAddOrder,onSetBar,role,logists,isBoss,gridBacks,onSetGridBack,onDelOrder}){
  ...
  const driverRows=useMemo(function(){
    ...
    return Object.keys(map).sort(function(a,b){
      var pa=map[a].plans.length>0, pb=map[b].plans.length>0;
      if(pa!==pb) return pa?-1:1;
      return (map[a].driver||'').localeCompare(map[b].driver||'');
    ...
    ce('div',{style:{display:'flex',alignItems:'center',gap:12,marginBottom:14,flexWrap:'wrap'}},
      ...
      ce('div',{style:{display:'flex',alignItems:'center',gap:6,marginLeft:'auto'}},
        ...
        ce('button',{className:'fl-press',onClick:function(){
          // строим чистую печатную таблицу: 14 дней от понедельника этой недели, читаемо
          var pdays=[]; var pm=new Date(base.getTime()-((base.getDay()+6)%7)*864e5);
          for(var i=0;i<14;i++) pdays.push(new Date(pm.getTime()+i*864e5));
          var pkeys=pdays.map(dk);
          var esc=function(s){ return String(s||'').replace(/[&<>]/g,function(c){return {'&':'&amp;','<':'&lt;','>':'&gt;'}[c];}); };
          var html='<html><head><meta charset="utf-8"><title>Планирование</title><style>'
            +'@page{size:A4 landscape;margin:6mm}body{font-family:Arial,sans-serif;color:#000}'
            +'table{border-collapse:collapse;width:100%;table-layout:fixed}'
            +'th,td{border:1px solid #999;padding:2px 3px;font-size:8px;vertical-align:top;overflow:hidden;word-break:break-word}'
            +'th{background:#eee;text-align:center}td.n{width:90px;font-weight:bold}'
            +'h2{font-size:14px;margin:0 0 6px}</style></head><body>';
          html+='<h2>x2 Планирование · '+pdays[0].toLocaleDateString('ru')+' — '+pdays[13].toLocaleDateString('ru')+'</h2>';
          html+='<table><tr><th style="width:90px">Водитель</th>';
          pdays.forEach(function(d){ html+='<th>'+wd[d.getDay()]+' '+d.getDate()+'.'+(d.getMonth()+1)+'</th>'; });
          html+='</tr>';
          driverRows.forEach(function(r){
            if(isHiredR(r)) return;
            html+='<tr><td class="n">'+esc(r.driver)+(r.plate?('<br><span style="font-weight:normal;color:#666">'+esc(r.plate)+'</span>'):'')+'</td>';
            pkeys.forEach(function(kk){
              var pl=r.plans.find(function(p){ return (p.loadDate||p.date)===kk; });
              html+='<td>'+(pl?(esc(pl.clientName)+(pl.clientName2?('<br>+ '+esc(pl.clientName2)):'')+(pl.back?('<br>↩ '+esc(pl.back)):'')):'')+'</td>';
            });
            html+='</tr>';
          });
        ...
        },title:'Печать (читаемая таблица на 2 недели)',style:{padding:'6px 12px',borderRadius:8,cursor:'pointer',fontFamily:'inherit',fontSize:13,border:'1px solid var(--border2)',background:'var(--bg2)',color:'var(--text2)'}},'🖨 Печать'))),
    // сетка
      ...
      ce('div',{style:{overflowX:'auto',border:'1px solid var(--border)',borderRadius:12}},
        ce('div',{style:{minWidth:(NDAYS*COLW)+'px',display:'grid',gridTemplateColumns:'repeat('+NDAYS+','+COLW+'px)'}},
          days.map(function(d,di){ var kk=dk(d);
            ...
            var isT=kk===todayK;
            return ce('div',{key:di,style:{borderRight:di<6?'1px solid var(--border)':'none',minHeight:80}},
              ...
              ce('div',{style:{padding:'6px 8px',display:'grid',gap:4}},
                ...
                notes.map(function(txt,ni){
                  return ce('div',{key:ni,style:{fontSize:10.5,lineHeight:1.25,padding:'3px 6px',borderRadius:5,display:'flex',alignItems:'center',gap:4,
                    background:'color-mix(in srgb,var(--cyan) 8%,transparent)',border:'1px solid color-mix(in srgb,var(--cyan) 25%,transparent)',color:'var(--text2)'}},
                    ce('span',{style:{flex:1,color:'var(--text)'}},txt),
...
function BossHr({drivers,vacations,waybills,history,trucks}){
  const tnow=Date.now();
  ...
  const wbBad=(drivers||[]).filter(function(d){ return !_isHiredB(d.name); }).map(function(d){ var w=(waybills||[]).find(function(x){return (x.driverName||'').trim()===(d.name||'').trim();});
    ...
    var n=(history||[]).filter(function(e){ return e&&e.action==='dispatch'&&sameDriver(e.driver,d.name)&&(dOnly(e.ts)==null||dOnly(e.ts)>=since); }).length;
...
function BossPanel(props){
  ...
  return ce('div',{style:{padding:'16px',maxWidth:1700,margin:'0 auto'}},
    ...
    sec==='hr'?ce(BossHr,{drivers:props.hrDrivers,vacations:props.hrVacations,waybills:props.hrWaybills,history:props.history,trucks:props.trucks}):null,
...
function HrWaybills({drivers,history,waybills,onSave,onDel,trucks}){
...
function HrBase({trucks,waybills,history,drivers}){
  ...
  const tripsFor=function(t){
    ...
    var n=(history||[]).filter(function(e){ return e&&e.action==='dispatch'&&sameDriver(e.driver,name)&&(dOnly(e.ts)==null||dOnly(e.ts)>=since); }).length;
...
function HrTab(props){
  ...
  return ce('div',{style:{padding:'16px',maxWidth:1700,margin:'0 auto'}},
    ...
    view==='waybills'&&ce(HrWaybills,{waybills:props.waybills,drivers:props.drivers,history:props.history,trucks:props.trucks,onSave:props.onSaveWaybill,onDel:props.onDelWaybill}),
    ...
    var load=function(){
      ...
      capFetch('temperature/current').then(function(r){
        ...
        data.forEach(function(row){
          ...
          var v=null;
          if(row.sensors&&row.sensors.length){ for(var i=0;i<row.sensors.length;i++){ if(row.sensors[i]&&row.sensors[i].value_c!=null){ v=Number(row.sensors[i].value_c); break; } } }
          if(v==null && row.value_c!=null) v=Number(row.value_c);
          if(v!=null) tmap[p.plate]={t:v,at:row.measured_at||nowLocalISO()};
        });
