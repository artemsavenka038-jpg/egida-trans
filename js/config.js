...
function routeProgress(truck, geo, clients, orders){
  ...
  if(goingBack){
    dir='back';   // статус водителя — главное, направление назад всегда
    if(back && normGeo(back.fromGeo) && normGeo(back.toGeo)){
      from=normGeo(back.fromGeo); to=normGeo(back.toGeo);   // есть координаты обратки — по ним
    } else if(cl && normGeo(cl.unloadGeo) && normGeo(cl.loadGeo)){
      from=normGeo(cl.unloadGeo); to=normGeo(cl.loadGeo);   // нет — разворачиваем маршрут клиента (выгрузка -> загрузка/домой)
    }
  } else {
...
/* ═══ Дорожные расстояния для фур (OpenRouteService через /api/route) ═══
   sources: [[lat,lon],...], target: [lat,lon]
   -> Promise {road:true, km:[..], min:[..]}  по реальным дорогам
      или    {road:false, km:[..], min:[..]} оценка по прямой ×1.25, если сервис недоступен */
function roadDistances(sources, target){
  var fallback=function(){
    return { road:false,
      km: sources.map(function(p){ var d=geoDistKm(p[0],p[1],target[0],target[1]); return d==null?null:Math.round(d*1.25); }),
      min: sources.map(function(p){ var d=geoDistKm(p[0],p[1],target[0],target[1]); return d==null?null:Math.round(d*1.25/65*60); }) };
  };
  if(!sources||!sources.length) return Promise.resolve({road:true,km:[],min:[]});
  return fetch('/api/route',{method:'POST',headers:{'Content-Type':'application/json'},
    ...
    .then(function(j){ if(j&&j.ok&&Array.isArray(j.km)) return {road:true,km:j.km,min:j.min}; return fallback(); })
    .catch(function(){ return fallback(); });
}

/* ═══ Постоянные точки парка (куда отправляют машины, кроме клиентов) ═══ */
var FLEET_PLACES = [
  { id:'base', name:'База', icon:'🏠', lat:53.804425, lon:27.593407 },
  { id:'daf',  name:'ДАФ-сервис', icon:'🔧', lat:53.953328, lon:27.984318 }
];
function placeById(id){ for(var i=0;i<FLEET_PLACES.length;i++){ if(FLEET_PLACES[i].id===id) return FLEET_PLACES[i]; } return null; }
function navLink(lat,lon){ return 'https://yandex.by/maps/?rtext=~'+lat+','+lon+'&rtt=auto'; }
