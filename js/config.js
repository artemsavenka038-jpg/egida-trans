...
function geoWhereNow(truck, geo, clients, orders){
  ...
  if(back && back.toGeo){ target=normGeo(back.toGeo); targetName=(back.to||'обратка'); dir='обратно'; }
  else {
  ...
  var res={moving:moving,speed:Math.round(geo.speed||0),ageMin:geoAgeMin(geo.at),lat:geo.lat,lon:geo.lon,dir:dir};
  if(target){
    var dist=geoDistKm(geo.lat,geo.lon,target.lat,target.lon);
    if(dist!=null){
      res.distKm=Math.round(dist);
      res.targetName=targetName;
      // грубая оценка времени: при движении делим на скорость, иначе по 60 км/ч
...
function autoGeoStatus(truck, geo, clients, orders){
  ...
  if(back && (back.fromGeo||back.toGeo)){
    // едет по обратному маршруту
    loadGeo=normGeo(back.fromGeo); unloadGeo=normGeo(back.toGeo); dir='обратно';
  } else {
    // рейс «туда» — берём координаты клиента или заказа
    var cl=(clients||[]).find(function(c){ return c.id===truck.clientId||c.name===truck.clientName; });
    if(cl){ loadGeo=normGeo(cl.loadGeo); unloadGeo=normGeo(cl.unloadGeo); }
    // если у машины привязан заказ с координатами — приоритетнее
  ...
  if(!moving && dLoad!=null && dLoad<=RAD)
    return {st:'at_load',color:'var(--green)',label:'🟢 на загрузке'+(dir==='обратно'?' (обратно)':''),dir:dir};
  if(!moving && dUnload!=null && dUnload<=RAD)
    return {st:'at_unload',color:'var(--red)',label:'🔴 на выгрузке'+(dir==='обратно'?' (обратно)':''),dir:dir};
  if(moving)
    return {st:'moving',color:'var(--cyan)',label:'🔵 в пути'+(dir==='обратно'?' назад':''),dir:dir};
  // стоит, но не у известной точки — без вывода (нет данных складов)
  if(dLoad==null && dUnload==null) return null;
  return {st:'stopped',color:'var(--text3)',label:'⏸ стоит',dir:dir};
}
