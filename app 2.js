...
function BindTripModal({plate,driver,trucks,clients,onPick,onClose}){
  ...
  const pick=function(t){
    ...
    onPick({truckId:t.id,truckPlate:t.plate||'',driver:t.driver||'',
      clientName:t.clientName||(cl&&cl.name)||'',temp:(cl&&cl.temp)||'',at:nowLocalISO()});
  };
  return ce(Modal,{title:'🔗 Привязать '+plate+' к рейсу',onClose,wide:true},
    ce('div',{style:{fontSize:12,color:'var(--text3)',marginBottom:10,lineHeight:1.6}},
      'Выберите рейс — из него возьмётся требуемая температура клиента, и на карточке появится индикатор.'+(driver?(' Датчик подписан: '+driver+'.'):'')),
    ...
    list.length===0?ce('div',{style:{color:'var(--text3)',fontSize:13,padding:'18px 2px'}},
      moving.length?'Ничего не найдено.':'Сейчас нет машин в рейсе.'):
    ce('div',{style:{maxHeight:340,overflowY:'auto',border:'1px solid var(--border)',borderRadius:11}},
      list.map(function(t,i){ var cl=(clients||[]).find(function(c){ return c.id===t.clientId; });
        var tp=(cl&&cl.temp)||'';
        return ce('div',{key:t.id,className:'fl-press',onClick:function(){ pick(t); },style:{display:'flex',alignItems:'center',gap:10,padding:'10px 12px',cursor:'pointer',
          borderBottom:i<list.length-1?'1px solid var(--border)':'none',fontSize:12.5}},
          ce('span',{style:{fontWeight:800,minWidth:130}},t.plate||'—'),
          ce('span',{style:{color:'var(--text2)',minWidth:150,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}},t.driver||'—'),
          ce('span',{style:{color:'var(--text3)',flex:1,minWidth:0,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}},t.clientName||(cl&&cl.name)||'—'),
          ce('span',{style:{fontWeight:800,color:tp?'var(--cyan)':'var(--text3)',minWidth:70,textAlign:'right'}},tp?(tp+'°'):'без t°')); })),
    ce('div',{style:{marginTop:14}},ce(Btn,{onClick:onClose,color:'var(--text3)',wide:true},'Отмена')));
}
  ...
  return ce('div',{style:{background:'var(--bg2)',borderRadius:14,padding:'12px 13px',
    ...
    bound?ce('div',{style:{display:'flex',flexDirection:'column',gap:5,paddingTop:6,borderTop:'1px solid var(--border)'}},
      ce('div',{style:{fontSize:11,color:'var(--text3)',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}},
        (r.bind.truckPlate||'')+(r.bind.clientName?(' · '+r.bind.clientName):'')),
      ...
      ce('button',{className:'fl-press',onClick:function(){onBind(r);},style:{marginTop:2,padding:'7px 10px',borderRadius:9,cursor:'pointer',fontFamily:'inherit',fontSize:11.5,fontWeight:800,
  ...
  return ce('div',{style:{padding:'16px',maxWidth:1700,margin:'0 auto'}},
    ...
    ce('div',{style:{display:'flex',alignItems:'center',gap:8,marginBottom:12,flexWrap:'wrap'}},
      ...
      ce('span',{style:{fontSize:11.5,color:'var(--text3)',marginLeft:'auto'}},
        'Индикатор появляется после привязки к рейсу — норма берётся из карточки клиента.')),
