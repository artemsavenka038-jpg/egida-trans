...
// Ключ берётся ТОЛЬКО из переменной окружения Vercel (Settings → Environment Variables).
// В коде его держать нельзя: репозиторий публичный, ключ утечёт.
const CAP_BASE = "https://api.svc.cap.by/api/v6";
const CAP_KEY = process.env.CAP_KEY || "";
...
module.exports = async function handler(req, res) {
  ...
  if (!CAP_KEY) {
    res.status(500).json({
      error: "no_key",
      message: "Не задан ключ CapNavi. Vercel → проект egida-trans → Settings → Environment Variables → добавьте CAP_KEY, затем Redeploy."
    });
    return;
  }
